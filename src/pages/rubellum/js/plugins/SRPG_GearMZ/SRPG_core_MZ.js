...
;(() => {
	...
	var _srpgMenuCommandList =
		parameters['srpgMenuCommandList'] ||
		'turnEnd,autoBattle,item,skill,equip,status,WinLose,options,save,gameEnd'
	...
	Game_System.prototype.unitDistance = function (event1, event2) {
		...
		if ($gameMap.isLoopHorizontal() === true) {
			var event1X =
				event1.posX() > event2.posX()
					? event1.posX() - $gameMap.width()
					: event1.posX() + $gameMap.width()
		...
		if ($gameMap.isLoopVertical() === true) {
			var event1Y =
				event1.posY() > event2.posY()
					? event1.posY() - $gameMap.height()
					: event1.posY() + $gameMap.height()
	...
	Game_System.prototype.setupSrpgBattleScene = function (userArray, targetArray) {
		...
		$gameTemp.setSrpgDistance(
			$gameSystem.unitDistance($gameTemp.activeEvent(), $gameTemp.targetEvent())
		)
		// 攻撃側の設定
	...
	Game_System.prototype.setSrpgBattleWindowNeedRefresh = function (
		actionBattlerArray,
		targetBattlerArray
	) {
	...
	Game_BattlerBase.prototype.makeSrpgRangeListForBattle = function (event) {
		...
		for (var i = 0; i < edges.length; i++) {
			...
			for (var d = 2; d < 10; d += 2) {
				...
				if (
					$gameMap.distTo(x, y, dx, dy) >= minRange &&
					event.srpgRangeExtention(dx, dy, x, y, skill, range)
				) {
	...
	Game_BattlerBase.prototype.makeAllRangeListForBattle = function (skill, battler) {
		$gameMap.events().forEach(function (event) {
			if (
				(skill.meta.specialRange === 'allActor' &&
					event.isType() === 'actor' &&
					!event.isErased()) ||
				(skill.meta.specialRange === 'allEnemy' && event.isType() === 'enemy' && !event.isErased())
			) {
				var dx = event.posX()
				var dy = event.posY()
	...
	Game_BattlerBase.prototype.canInput = function () {
		if ($gameSystem.isSRPGMode() === true) {
			return (
				this.isAppeared() && !this.isRestricted() && !this.isAutoBattle() && !this.srpgTurnEnd()
			)
		} else {
	...
	Game_BattlerBase.prototype.isTargetInRange = function (item) {
		var targetEvent =
			this._srpgActionTiming === 1 ? $gameTemp.activeEvent() : $gameTemp.targetEvent()
	...
	Game_Battler.prototype.srpgMakeCounterActions = function (userArray, targetArray) {
		...
		if (
			userArray[0] !== targetArray[0] &&
			userArray[1].currentAction().item().meta.srpgUncounterable
		)
			return
	...
	Game_Battler.prototype.useItem = function (item) {
		if ($gameSystem.isSRPGMode() === true) {
			if (DataManager.isSkill(item)) {
				if (
					!this._usedFirstSkill ||
					_srpgAgiAttackPlusPayCost === 2 ||
					_srpgAgiAttackPlusPayCost === 4
				) {
	...
	Game_Actor.prototype.attackSkillId = function () {
		...
		// 職業
		if (this.currentClass().meta.srpgWeaponSkill)
			value = Number(this.currentClass().meta.srpgWeaponSkill)
		// スキル
	...
	Game_Actor.prototype.srpgReactionSkill = function () {
		...
		// アクター
		if (this.actor().meta.srpgReactionSkill)
			array.unshift(Number(this.actor().meta.srpgReactionSkill))
	...
	Game_Actor.prototype.srpgAlternativeSkillId = function () {
		var id = this.actor().meta.srpgAlternativeSkillId
			? this.actor().meta.srpgAlternativeSkillId
			: this.attackSkillId()
		return id
	}
	...
	Game_Actor.prototype.srpgThroughTag = function () {
		...
		// 職業
		if (
			this.currentClass().meta.srpgThroughTag &&
		...
		for (var i = 0; i < equips.length; i++) {
			var item = equips[i]
			if (
				this.isEquipValid(item) &&
				item.meta.srpgThroughTag &&
	...
	Game_Actor.prototype.expRate = function () {
		if (this.isMaxLevel()) {
			...
			var rate =
				(this.currentExp() - this.currentLevelExp()) /
	...
	Game_Enemy.prototype.srpgReactionSkill = function () {
		...
		array.unshift(_srpgDefaultReactionSkill)
		// エネミー
		if (this.enemy().meta.srpgReactionSkill)
			array.unshift(Number(this.enemy().meta.srpgReactionSkill))
	...
	Game_Enemy.prototype.srpgAlternativeSkillId = function () {
		var id = this.enemy().meta.srpgAlternativeSkillId
			? this.enemy().meta.srpgAlternativeSkillId
			: this.attackSkillId()
		return id
	}
	...
	Game_Enemy.prototype.srpgThroughTag = function () {
		...
		var weapon = $dataWeapons[Number(this.enemy().meta.srpgWeapon)]
		if (
			this.isEquipValid(weapon) &&
			weapon.meta.srpgThroughTag &&
	...
	Game_Troop.prototype.setup = function (troopId) {
		if ($gameSystem.isSRPGMode() === true) {
			...
			for (var i = 0; i < this.SrpgBattleEnemys().length; i++) {
				...
				if ($gameSystem.isSideView()) {
					enemy.setScreenXy(
						Graphics.width / 4 + 240 * i + enemy.correctionX(),
						Graphics.height / 2 + 48 + enemy.correctionY()
					)
				} else {
					enemy.setScreenXy(
						Graphics.width / 2 + enemy.correctionX(),
						Graphics.height / 2 + 32 + 96 * i + enemy.correctionY()
					)
				}
				this._enemies.push(enemy)
			}
	...
	Game_CharacterBase.prototype.makeAppearPoint = function (eventSelf, x, y, throughTag, previous) {
		...
		makeAppPointBreak: for (var i = 0; i < edges.length; i++) {
			...
			for (var num = 0; num < events.length; num++) {
				...
				if (['actor', 'enemy', 'object', 'playerEvent'].indexOf(event.isType()) < 0)
					break makeAppPointBreak
			}
			for (var d = 2; d < 10; d += 2) {
	...
	Game_Player.prototype.touchOnCancelButton = function () {
		...
		return (
			...
			this.isCancelButtonEnabled() &&
			TouchInput.x >= cancelButton.x &&
			TouchInput.y <= cancelButton.y * 2 + cancelButton.height
		)
	}
					...
					$gameMap.eventsXy(x, y).forEach(function (event) {
						if (event.isTriggerIn(triggers) && !event.isErased()) {
							...
							if (event.isType() === 'actor' || event.isType() === 'enemy') {
								...
								if (
									(targetArray[0] === 'actor' && actionArray[1].currentAction().isForFriend()) ||
									...
									if (_srpgPredictionWindowMode === 3 && !actionArray[1].canUse(skill)) {
										...
										if (_srpgPredictionWindowMode !== 3)
											$gameSystem.setSrpgStatusWindowNeedRefresh(actionArray)
	...
	Game_Interpreter.prototype.checkUserAndTarget = function (switchId, eventId1, eventId2) {
		if ($gameTemp.activeEvent() && $gameTemp.targetEvent()) {
			if (
				($gameTemp.activeEvent().eventId() === eventId1 &&
					$gameTemp.targetEvent().eventId() === eventId2) ||
				($gameTemp.activeEvent().eventId() === eventId2 &&
					$gameTemp.targetEvent().eventId() === eventId1)
			) {
	...
	Game_Interpreter.prototype.unitRevive = function (eventId) {
		...
		if (battlerArray && event && (battlerArray[0] === 'actor' || battlerArray[0] === 'enemy')) {
			...
			var xy = event.makeAppearPoint(
				event,
				event.posX(),
				event.posY(),
				battlerArray[1].srpgThroughTag()
			)
	...
	Sprite_Character.prototype.updateCharacterFrame = function () {
		...
		if (
			...
			if (battlerArray) {
				...
				if (
					(battlerArray[0] === 'actor' || battlerArray[0] === 'enemy') &&
	...
	Window_SrpgBattleResult.prototype.drawGainItem = function (x, y) {
		...
		if (items.length > 0) {
			for (var i = 0; i < items.length; i++) {
				this.drawItemName(
					items[i],
					x + width * Math.floor(0.5 + i * 0.5),
					y - this.lineHeight() * (i % 2),
					width
				)
			}
		}
	...
	Window_SrpgPrediction.prototype.setDoubleActionRate = function (user, target) {
		...
		if (
			user.currentAction().isForOpponent() &&
	...
	Window_SrpgPrediction.prototype.drawContents = function () {
		...
		this.drawSrpgBattleActionName(
			user,
			action,
			this.innerWidth / 2 + padding * 2,
			lineHeight * 0,
			true
		)
		this.drawSrpgBattleHit(hit, eva, this.innerWidth / 2 + padding * 2, lineHeight * 1)
		this.drawSrpgBattleDistance(
			user,
			action,
			this.innerWidth / 2 + 172 + padding * 2,
			lineHeight * 1
		)
		this.drawSrpgBattleDamage(
			damage,
			this.innerWidth / 2 + padding * 2,
			lineHeight * 2,
			doubleActionRate
		...
		if (
			reaction &&
			(!target.canUse(reaction.item()) || user.currentAction().item().meta.srpgUncounterable)
		) {
			reaction = null
		}
		...
		for (var i = 0; i < order.length; i++) {
			...
			if (user === battler && action && action.item()) {
				...
				if (doubleActioncheck.indexOf(battler) < 0) {
					...
					if (_srpgBattleReaction === 2 && target.cnt > 0)
						actionRate = Math.floor(actionRate * target.cnt)
					if (actionRate > 0) {
						this.drawSrpgBattleOrders(battler, reaction.item(), orderX, 17 + orderY * line)
	...
	// 使用する攻撃名の描画
	Window_SrpgPrediction.prototype.drawSrpgBattleActionName = function (
		battler,
		action,
		x,
		y,
		...
		if (action && flag === true) {
			...
			if (skill) {
				...
				if (
					DataManager.isSkill(skill) &&
					skill.id === battler.attackSkillId() &&
					!battler.hasNoWeapons()
				) {
					if (battler.isActor()) {
						var item = battler.weapons()[0]
	...
	var _SRPG_Window_BattleLog_showEnemyAttackAnimation =
    ...
    */
	...
	BattleManager.makeActionOrders = function () {
		...
		if (_srpgUseAgiAttackPlus === 'true') {
			...
			if (
				firstBattler.currentAction().isForOpponent() &&
	...
	BattleManager.endBattle = function (result) {
		...
		if ($gameParty.battleMembers()[0] && $gameParty.battleMembers()[0].isAlive())
			$gameParty.battleMembers()[0].onAllActionsEnd()
		if ($gameParty.battleMembers()[1] && $gameParty.battleMembers()[1].isAlive())
			$gameParty.battleMembers()[1].onAllActionsEnd()
		if ($gameTroop.members()[0] && $gameTroop.members()[0].isAlive())
			$gameTroop.members()[0].onAllActionsEnd()
		if ($gameTroop.members()[1] && $gameTroop.members()[1].isAlive())
			$gameTroop.members()[1].onAllActionsEnd()
	...
	Scene_Map.prototype.touchOnCancelButton = function () {
		...
		return (
			$gameSystem.isSRPGMode() &&
			this.isCancelButtonEnabled() &&
			TouchInput.x >= this._cancelButton.x &&
			TouchInput.y <= this._cancelButton.y * 2 + this._cancelButton.height
		)
	}
	...
	Scene_Map.prototype.srpgWindowOpenClose = function () {
		...
		if (flag[0]) {
			if (
				!this._mapSrpgActorCommandWindow.isOpen() &&
				!this._mapSrpgActorCommandWindow.isOpening()
			) {
			...
			if (
				this._mapSrpgActorCommandWindow.isOpen() &&
				!this._mapSrpgActorCommandWindow.isClosing()
			) {
		...
		if (flag[0]) {
			if (
				!this._mapSrpgActorCommandStatusWindow.isOpen() &&
				!this._mapSrpgActorCommandStatusWindow.isOpening()
			) {
			...
			if (
				this._mapSrpgActorCommandStatusWindow.isOpen() &&
				!this._mapSrpgActorCommandStatusWindow.isClosing()
			) {
	...
	Scene_Map.prototype.eventUnitEvent = function () {
		$gameMap
			.eventsXy($gameTemp.activeEvent().posX(), $gameTemp.activeEvent().posY())
			.forEach(function (event) {
				if (event.isType() === 'unitEvent') {
					if (event.pageIndex() >= 0) event.start()
					$gameTemp.pushSrpgEventList(event)
					$gameSystem.pushSearchedItemList([
						$gameTemp.activeEvent().posX(),
						$gameTemp.activeEvent().posY()
					])
				}
	...
	Scene_Map.prototype.selectPreviousSrpgBattleStart = function () {
		...
		if (
			(_srpgSkipTargetForSelf === 'true' && activeBattler.action(0).isForUser()) ||
	...
	Scene_Map.prototype.srpgInvokeAutoActorMove = function () {
		...
		// 代替スキルの検討
		var alternativeArray = this.checkAlternativeSkill(
			event,
			type,
			actor,
			targetType,
	...
	Scene_Map.prototype.srpgMakeCanAttackTargetsFromBestSearchRoute = function (
		activeEvent,
		battler,
		targetType
	) {
	...
	// 代替スキルの検討と決定
	Scene_Map.prototype.checkAlternativeSkill = function (
		event,
		type,
		battler,
		targetType,
	...
	Scene_Map.prototype.srpgSearchOptimalPos = function (targetEvent, battler, type) {
		...
		var candidatePos = []
		var optimalDis = -9999
		var optimalResion = $gameMap.regionId(
			$gameTemp.activeEvent().posX(),
			$gameTemp.activeEvent().posY()
		)
	...
	Scene_Map.prototype.srpgCalculateCandidatePosDis = function (pos, targetEvent) {
		...
		if ($gameMap.isLoopHorizontal() === true) {
			var pos0X =
				pos[0] > targetEvent.posX() ? pos[0] - $gameMap.width() : pos[0] + $gameMap.width()
			var disX = Math.abs(pos0X - targetEvent.posX())
			minDisX = minDisX < disX ? minDisX : disX
		}
		if ($gameMap.isLoopVertical() === true) {
			var pos1Y =
				pos[1] > targetEvent.posY() ? pos[1] - $gameMap.height() : pos[1] + $gameMap.height()
	...
	Scene_Map.prototype.srpgSearchItemPos = function (list, candidatePos) {
		...
		for (var i = 0; i < list.length; i++) {
			...
			if (pos[2] === false && $gameSystem.areTheyNoUnits(pos[0], pos[1])) {
				if (
					$gameSystem.isThereEventUnit(pos[0], pos[1]) &&
	...
	Scene_Map.prototype.srpgInvokeAutoUnitAction = function () {
		...
		event.makeRangeTable(
			event.posX(),
			event.posY(),
			actionArray[1].srpgSkillRange(skill),
			[0],
			event.posX(),
			event.posY(),
			skill
		...
		if (actionArray[1].canUse(skill)) {
			$gameTemp.setAutoMoveDestinationValid(true)
			$gameTemp.setAutoMoveDestination(
	...
	Scene_Map.prototype.preBattleSetDirection = function () {
		...
		if ($gameMap.isLoopHorizontal() === true) {
			var event1X =
				$gameTemp.activeEvent().posX() > $gameTemp.targetEvent().posX()
		...
		if ($gameMap.isLoopVertical() === true) {
			var event1Y =
				$gameTemp.activeEvent().posY() > $gameTemp.targetEvent().posY()
	...
	// ステータスウィンドウの開閉
	const _SRPG_Scene_Battle_updateStatusWindowVisibility =
        ...
        */
		...
		if (user !== target) {
			...
			if (target.currentAction() && target.currentAction().item()) {
				...
				if (reaction.isForUser() || (reaction.isForFriend() && userArray[0] !== targetArray[0])) {
	...
	Scene_Map.prototype.srpgInvokeMapSkill = function (data) {
		...
		switch (data.phase) {
			...
			case 'animation':
				...
				if (animation < 0)
					animation = user.isActor() ? user.attackAnimationId1() : user.attackAnimationId()
