...
;(function () {
	...
	Game_System.prototype.srpgMakeMoveTable = function (event) {
		...
		if (!$gameMap.isEventRunning() && $gameSystem.isBattlePhase() === 'actor_phase')
			$gameTemp.makeAuraList(event) //show aura color
	}
	...
	Scene_Map.prototype.eventBeforeBattle = function () {
		...
		if (battler.shouldPayCost()) {
			//this is used to avoid refreshing repeatedly when using AoE skills.
	...
	Game_Temp.prototype.refreshAura = function (userevent) {
		...
		$gameMap.events().forEach(function (event) {
			...
			if (unit && (unit[0] === 'actor' || unit[0] === 'enemy')) {
				unit[1].skills().forEach(function (item) {
					//check all skills
					if ($gameTemp.isAuraStateValid(item, userevent.isType(), unit[0], dx, dy))
						user.addState(Number(item.meta.SRPGAuraState))
				})
				unit[1].states().forEach(function (item) {
					//check all states
					if ($gameTemp.isAuraStateValid(item, userevent.isType(), unit[0], dx, dy))
						user.addState(Number(item.meta.SRPGAuraState))
				})
				...
				if ($gameTemp.isAuraStateValid(item, userevent.isType(), 'actor', dx, dy))
					user.addState(Number(item.meta.SRPGAuraState))
			}
		})
	}
	...
	var shoukang_Sprite_SrpgMoveTile_clearThisMoveTile =
		Sprite_SrpgMoveTile.prototype.clearThisMoveTile
