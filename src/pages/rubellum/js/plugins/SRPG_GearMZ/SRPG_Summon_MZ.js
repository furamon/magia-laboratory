...
;(function () {
	...
	Game_Interpreter.prototype.AoESummon = function (type, summonId, battlerId, level, turn) {
		...
		for (var m = 0; m < rlim; ++m) {
			for (var n = 0; n < rlim; ++n) {
				...
				if (
					$gameMap.isValid(x, y) &&
					$gameMap.inArea(m - aoe.size, n - aoe.size, aoe.size, aoe.minSize, aoe.shape, aoe.dir)
				) {
					this.summon(type, summonId, battlerId, level, turn, x, y)
				}
			}
	...
	Game_Interpreter.prototype.summon = function (
		type,
		summonId,
		battlerId,
		level,
		...
		var summonEvent = new Game_SummonEvent(
			$gameMap.mapId(),
			eventId,
			summonId,
			summoner,
    ...
    */
	...
	Game_SummonEvent.prototype.initialize = function (
		mapId,
		eventId,
		summonEventId,
		summoner,
