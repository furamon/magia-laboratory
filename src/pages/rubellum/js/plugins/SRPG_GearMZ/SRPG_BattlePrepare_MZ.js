...
;(function () {
	...
	Game_Party.prototype.initRemainingActorList = function () {
		...
		for (var i = 0; i < this._srpgPrepareAllActors.length; i++) {
			if (actorlist.indexOf(this._srpgPrepareAllActors[i]) < 0)
				list.push(this._srpgPrepareAllActors[i])
		}
	...
	Game_Party.prototype.isValidActorNumber = function () {
		return (
			this.currentActorNumber() >= this._srpgMinActor &&
			this.currentActorNumber() <= this._srpgMaxActor
		)
	}
	...
	Game_Party.prototype.allMembers = function () {
		if ($gameSystem.isSRPGMode() == true && $gameSystem.isBattlePhase() === 'battle_prepare') {
			...
			this._srpgPrepareAllActors = members.map(function (actor) {
	...
	Window_MenuStatus.prototype.drawItemImage = function (index) {
		if ($gameSystem.isSRPGMode() == true && $gameSystem.isBattlePhase() === 'battle_prepare') {
			...
			if ($gameParty.inLockedActorList(actor.actorId())) {
				this.drawIcon(
					_lockIconIndex,
					ImageManager.faceWidth - ImageManager.iconHeight - 1,
					rect.y + rect.height - ImageManager.iconHeight - 1
				)
			}
			this.changePaintOpacity(true)
		} else {
	...
	Game_Player.prototype.srpgBattlePrepareExchangePosition = function (event) {
		...
		if (
			Number(event.event().meta.id) === 0 &&
			event.eventId() !== $gameTemp.activeEvent().eventId()
		) {
			var battlerArray = $gameSystem.EventToUnit(event.eventId())
			SoundManager.playOk()
	...
	Game_Player.prototype.startMapEvent = function (x, y, triggers, normal) {
		if (
			...
			$gameSystem.isBattlePhase() === 'battle_prepare'
		) {
			...
			if ($gameSystem.isSubBattlePhase() === 'normal' && triggers[0] === 0) {
				...
				} else if (event.event().meta.type === 'actor' && event.isErased()) {
					//if there's remaining actor add actor.
	...
	Scene_Map.prototype.srpgCanNotUpdateCallMenu = function () {
		return (
			...
			($gameSystem.isBattlePhase() != 'actor_phase' &&
				$gameSystem.isBattlePhase() != 'battle_prepare')
		) //shoukang add new condition: $gameSystem.isBattlePhase() != 'battle_prepare'
	}
	...
	Scene_Map.prototype.isCancelButtonEnabled = function () {
		if (
			$gameSystem.isSubBattlePhase() === 'actor_Interaction' ||
	...
	Scene_Map.prototype.update = function () {
		...
		if ($gameSystem.isSRPGMode() && $gameSystem.isBattlePhase() === 'battle_prepare') {
			if ($gameTemp.moveList().length === 0 && !$gameMap.isEventRunning())
				$gameTemp.srpgMakePrepareTable()
	...
	Scene_Map.prototype.createPrepareWindow = function () {
		...
		this._mapSrpgPrepareWindow.x = Math.max(
			Graphics.boxWidth / 2 - this._mapSrpgPrepareWindow.windowWidth,
			0
		)
		this._mapSrpgPrepareWindow.y = Math.max(
			Graphics.boxHeight / 2 - this._mapSrpgPrepareWindow.windowHeight,
			0
		)
		...
		$gameMap.events().forEach(function (event) {
			if (event.isType() === 'actor' && !event.isErased()) {
				...
				if (
					actorArray &&
					actorArray[1] &&
					$gameParty.inRemainingActorList(actorArray[1].actorId())
				) {
	...
	Scene_Menu.prototype.onFormationOk = function () {
		if ($gameSystem.isSRPGMode() == true && $gameSystem.isBattlePhase() == 'battle_prepare') {
			...
			if ($gameParty.inLockedActorList(id) || $gameActors.actor(id).isDead()) {
				// || $gameActors.actor(id).isDeathStateAffected()
				SoundManager.playBuzzer()
	...
	Window_SrpgMemberRequirement.prototype.drawRequiredBattlerNumber = function (x, width) {
		...
		if ($gameParty.getMinActor() === 1)
			this.drawText($gameParty.getMaxActor(), (3 * width) / 4, 4, width / 4, 'right')
		else
			this.drawText(
				$gameParty.getMinActor() + '-' + $gameParty.getMaxActor(),
				2 * x + (3 * width) / 4,
				4,
				width / 4,
				'right'
