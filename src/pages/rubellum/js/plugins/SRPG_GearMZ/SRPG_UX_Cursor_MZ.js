...
;(function () {
		...
		if ($gameTemp.canAutoSelect()) {
			...
			for (var i = 0; i < list.length; i++) {
				...
				event.makeRangeTable(
					pos[0],
					pos[1],
					battlerArray[1].srpgWeaponRange(),
					[0],
					pos[0],
					pos[1],
					$dataSkills[battlerArray[1].attackSkillId()]
				)
			}
	...
	Scene_Map.prototype.updateMenuSelection = function (id) {
		...
		$gameTemp.setSrpgSpecialRange(
			user.srpgRangeExtention(event.posX(), event.posY(), user.posX(), user.posY(), item)
		)
	...
	Game_Player.prototype.triggerAction = function () {
		...
		if (
			...
			$gameMap.events().forEach(function (event) {
				if (
					!event.isErased() &&
					event.pos($gamePlayer.posX(), $gamePlayer.posY()) &&
					event.isType() === 'actor' &&
					$gameSystem.EventToUnit(event.eventId())[1].canInput()
				) {
					newId = event.eventId()
				}
			})
	...
	Game_System.prototype.findSelection = function (type, start, reverse) {
		...
		for (var id = (start + step) % max; id != start; id = (id + step) % max) {
			...
			if (!event) continue
			if (
				(type === 'actor' && $gameSystem.eventIsUsableActor(event)) ||
				(type === 'target' &&
					$gameSystem.eventIsValidTarget(event) &&
					$gameSystem.positionInRange(event.posX(), event.posY()))
			) {
