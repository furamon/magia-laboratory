...
;(() => {
	...
	const paramPlusWithWeaponTypeTraitId = uniqueTraitIdCache.allocate(
		pluginName,
		localTraitIdStart,
		'武器タイプ能力値'
	)
	...
	class ParamPlusWithEquipTypeTrait {
		...
		paramId() {
			return this.isXParam()
				? XPARAM_KEYS.indexOf(this._statusType)
				: PARAM_KEYS.indexOf(this._statusType)
		}
		trait() {
			return {
				code: this.traitId(),
	...
	function Game_Actor_EquipTypeStatusBonusTraitMixIn(gameActor) {
		...
		gameActor.paramPlusWithEquipTypeTraits = function (paramId) {
			return this.validParamPlusWithEquipTypeTraits(paramId).reduce(
				(result, trait) => result + trait.value,
				0
			)
		}
			...
			return this.equipsForEquipTypeStatusBonus()
				...
				.reduce((result, equip) => {
					const dataId = DataManager.isWeapon(equip)
						? dataIdFromTypeIdAndParamId(
								DataManager.weaponTypeIdForStatusBonusTrait(equip),
								paramId
							)
		...
		gameActor.xparamPlusWithEquipTypeTraits = function (xparamId) {
			return this.validXParamPlusWithEquipTypeTraits(xparamId).reduce(
				(result, trait) => result + trait.value,
				0
			)
		}
			...
			return this.equipsForEquipTypeStatusBonus()
				...
				.reduce((result, equip) => {
					const dataId = DataManager.isWeapon(equip)
						...
						: dataIdFromTypeIdAndParamId(
								DataManager.armorTypeIdForStatusBonusTrait(equip),
								xparamId
							)
					const traitId = DataManager.isWeapon(equip)
						? xparamPlusWithWeaponTypeTraitId.id
