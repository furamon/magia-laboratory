...
;(() => {
	...
	const settings = {
		...
		ignoreAutoLineBreakWindows: JSON.parse(pluginParameters.ignoreAutoLineBreakWindows || '[]').map(
			(e) => {
				return String(e || ``)
			}
		),
	...
	function Window_AutoLineBreakMixIn(windowClass) {
		...
		windowClass.isIgnoreAutoLineBreakWindow = function () {
			return (
				settings.ignoreAutoLineBreakWindows.includes(this.constructor.name) ||
				this._ignoreAutoLineBreakTemporary
			)
		}
		/**
		 * 自動折返しが有効かどうか
		...
		windowClass.shouldLineBreakHere = function (textState) {
			...
			return (
				this.shouldCharacterBaseLineBreakHere(textState) ||
				this.shouldWordBaseLineBreakHere(textState)
			)
		}
		windowClass.shouldCharacterBaseLineBreakHere = function (textState) {
			...
			if (size + textState.x > this.lineWidth()) {
				...
				return (
					this.isProhibitLineBreakBefore(next2Character) &&
					this.isProhibitLineBreakBefore(next3Character)
				)
			}
			// 行末禁則チェック
			if (
				next2Character &&
				this.textWidth(`${textState.lineBuffer}${nextCharacter}${next2Character}`) + textState.x >
					this.lineWidth()
			) {
		...
		windowClass.shouldWordBaseLineBreakHere = function (textState) {
			...
			const isInitialOfWord =
				textState.text[textState.index - 1] === ' ' && textState.text[textState.index] !== ' '
			const nextSpaceIndex = textState.text.indexOf(' ', textState.index + 1)
			const nextLineBreakIndex = textState.text.indexOf('\n', textState.index + 1)
			if (
				!isInitialOfWord ||
				nextSpaceIndex < 0 ||
				(nextLineBreakIndex > 0 && nextSpaceIndex > nextLineBreakIndex)
			) {
				return false
			}
	...
	function Window_BattleLog_AutoLineBreakMixIn(windowClass) {
		...
		windowClass.lineRect = function (index) {
			...
			rect.y +=
				this._newLines.slice(0, index).reduce((prev, current) => prev + current, 0) *
				this.lineHeight()
			return rect
		}
	}
