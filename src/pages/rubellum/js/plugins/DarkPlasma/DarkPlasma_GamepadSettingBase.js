...
;(() => {
	...
	const settings = {
		keyMapper: JSON.parse(
			pluginParameters.keyMapper ||
				'[{"keyCode":"77","action":"menu"},{"keyCode":"83","action":"special2"}]'
		).map((e) => {
			return ((parameter) => {
		...
		gamepadMapper: JSON.parse(
			pluginParameters.gamepadMapper || '[{"keyCode":"6","action":"special2"}]'
		).map((e) => {
			return ((parameter) => {
				const parsed = JSON.parse(parameter)
				return {
	...
	class Window_GamepadConfig extends Window_Selectable {
		...
		drawHeader() {
			...
			HEADER_TEXTS.forEach((text, index) => {
				this.drawText(
					text,
					index * (this.colsWidth() + this.colsPadding()),
					0,
					this.colsWidth(),
					'center'
				)
	...
	function convertGamepadEscapeCharacter(text) {
		...
		text = text.replace(/\x1bGAMEPAD\[(.+)\]/gi, (_, symbol) =>
			Window_Base.prototype.getManualButtonName(symbol)
		)
		return text
	}
})()
