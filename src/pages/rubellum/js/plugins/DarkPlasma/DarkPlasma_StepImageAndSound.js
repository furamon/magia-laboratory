...
;(() => {
	...
	class StepImageSetting {
		constructor(filename, meta) {
			...
			this._wet = Number(
				settingList.find((setting) => /^wet(\d+)$/.test(setting))?.match(/^wet(\d+)$/)[1] || -1
			)
			this._speed = Number(
				settingList
					.find((setting) => /^animeSpeed(\d+)$/.test(setting))
					?.match(/^animeSpeed(\d+)$/)[1] || settings.animationSpeed
			)
		}
		get filename() {
		...
		dataManager.extractMetadata = function (data) {
			...
			if (isTileset(data)) {
				...
				;[...Array(7).keys()]
					.filter(
						(terrainTag) =>
							data.meta[`ステップエフェクト${terrainTag}`] || data.meta[`StepEffect${terrainTag}`]
					)
					.forEach((terrainTag) => {
						data.stepImageSetting[terrainTag] = new StepImageSetting(
							...
							String(
								data.meta[`ステップエフェクト設定${terrainTag}`] ||
									data.meta[`StepEffectSetting${terrainTag}`]
							)
						)
					})
				;[...Array(7).keys()]
					.filter(
						(terrainTag) =>
							data.meta[`ステップサウンド${terrainTag}`] || data.meta[`StepSound${terrainTag}`]
					)
					.forEach((terrainTag) => {
						data.stepSoundSetting[terrainTag] = new StepSoundSetting(
							...
							String(
								data.meta[`ステップサウンド設定${terrainTag}`] ||
									data.meta[`StepSoundSetting${terrainTag}`]
							)
						)
					})
			}
	...
	function Game_CharacterBase_StepImageAndSoundMixIn(gameCharacterBase) {
		...
		gameCharacterBase.updateMove = function () {
			...
			if (this._moveCount % this.stepSpeed() === 0) {
				...
				if (stepImageSettingOnMap) {
					...
					if (stepImageSetting) {
						$gameTemp.requestStepImage(
							new StepImageRequest(stepImageSetting, this._realX, this._realY, this)
						)
					}
		...
		gameCharacterBase.StepSeVolumeRate = function () {
			return Math.max(
				1.0 -
					$gameMap.distance(this.x, this.y, $gamePlayer.x, $gamePlayer.y) / settings.audioDistance,
				0
			)
		}
	}
	...
	class Sprite_StepAnimation extends Sprite {
		...
		maxAnimationFrame() {
			return this.bitmap
				? Math.floor(this.bitmap.width / settings.cellWidth) * this._animationSpeed
				: 0
		}
		update() {
			super.update()
		...
		canvasX() {
			const dx =
				this._defaultX < $gameMap.displayX()
					? $gameMap.displayX() - $gameMap.width()
					: $gameMap.displayX()
			return (
				(this._defaultX - dx) * $gameMap.tileWidth() +
				settings.cellWidth / 2 +
				this._offsetX +
				this.fitStepOffsetX()
			)
		}
		canvasY() {
			const dy =
				this._defaultY < $gameMap.displayY()
					? $gameMap.displayY() - $gameMap.height()
					: $gameMap.displayY()
			return (
				(this._defaultY - dy) * $gameMap.tileHeight() +
				settings.cellWidth / 2 +
				this._offsetY +
				this.fitStepOffsetY()
			)
