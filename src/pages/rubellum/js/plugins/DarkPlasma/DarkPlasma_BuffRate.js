...
;(() => {
	...
	const settings = {
		maxHp: ((parameter) => {
			...
			pluginParameters.maxHp ||
				'{"buffRate1":"25", "buffRate2":"50", "debuffRate1":"25", "debuffRate2":"50"}'
		),
		maxMp: ((parameter) => {
			const parsed = JSON.parse(parameter)
			return {
	...
	const buffRates = paramNames.map((paramName) => [
		0,
		settings[paramName].buffRate1,
		settings[paramName].buffRate2
	])
