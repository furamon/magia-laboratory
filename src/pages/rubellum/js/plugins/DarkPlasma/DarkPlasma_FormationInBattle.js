...
;(() => {
	...
	const settings = {
		...
		cooldownWithForceFormation:
			String(pluginParameters.cooldownWithForceFormation || true) === 'true'
	}
	...
	function Scene_Battle_FormationMixIn(sceneBattle) {
		...
		sceneBattle.createFormationBattleMemberWindow = function () {
			this._formationBattleMemberWindow = new Window_FormationBattleMember(
				this.formationBattleMemberWindowRect()
			)
		...
		sceneBattle.createFormationWaitingMemberWindow = function () {
			this._formationWaitingMemberWindow = new Window_FormationWaitingMember(
				this.formationWaitingMemberWindowRect()
			)
	...
	function Window_PartyCommand_FormationMixIn(windowClass) {
		...
		windowClass.makeCommandList = function () {
			...
			if (!this.isFormationEnabled() && formationCooldown.isDuringCooldown()) {
				commandName += settings.cooldownFormat.replace(
					/\{turn\}/gi,
					`${formationCooldown.cooldownTurnCount()}`
				)
			}
