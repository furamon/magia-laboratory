...
;(() => {
	...
	const fixParameterTraitId = uniqueTraitIdCache.allocate(
		pluginName,
		localTraitId,
		'通常能力値固定'
	)
	const fixXParameterTraitId = uniqueTraitIdCache.allocate(
		pluginName,
		localTraitId + 1,
		'追加能力値固定'
	)
	const fixParameterVariableTraitId = uniqueTraitIdCache.allocate(
		pluginName,
		localTraitId + 2,
		'通常能力値固定(変数)'
	)
				...
				lines
					.map((line) => {
						...
						if (fixParameterSetting && fixParameterSetting[1] && fixParameterSetting[2]) {
							const traitId = traitIdFromSetting(
								fixParameterSetting[1].trim(),
								fixParameterSetting[2].trim()
							)
							if (!traitId) {
								return null
							}
