...
;(() => {
	...
	const sealItemByIdTraitId = uniqueTraitIdCache.allocate(
		pluginName,
		localTraitId + 1,
		'アイテム禁止'
	)
	const sealHealItemTraitId = uniqueTraitIdCache.allocate(
		pluginName,
		localTraitId + 2,
		'回復アイテム禁止'
	)
	const sealResurrectionItemTraitId = uniqueTraitIdCache.allocate(
		pluginName,
		localTraitId + 3,
		'蘇生アイテム禁止'
	)
		...
		dataManager.isResurrectionItem = function (item) {
			return item.effects.some(
				(effect) =>
					...
					effect.dataId === Game_BattlerBase.prototype.deathStateId()
			)
		}
	}
	...
	function Game_Actor_SealItemMixIn(gameActor) {
		...
		gameActor.isItemSealed = function (item) {
			return (
				...
				(this.traits(sealResurrectionItemTraitId.id).length > 0 &&
					DataManager.isResurrectionItem(item)) ||
