...
;(() => {
	...
	class Sprite_Falling extends Sprite {
		...
		setup() {
			...
			this._animationFrame = Math.randomInt(
				this.fallSetting().cols * this.fallSetting().animationSpeed
			)
			this.updateFrame()
		}
		update() {
			...
			this._animationFrame =
				(this._animationFrame + 1) % (this._fallSetting.cols * this._fallSetting.animationSpeed)
		...
		initialLifeTime() {
			return (
				this.fallSetting().minimumLifeTime +
				Math.floor(Math.random() * this.fallSetting().lifeTimeRange)
			)
		}
		calcScale() {
	...
	function Spriteset_Map_FallImagesMixIn(spritesetMap) {
		...
		spritesetMap.needCreateFallImage = function () {
			return (
				!!fallImageStatus?.startRequested ||
				(!!fallImageStatus?.isFalling && this._fallImageSprites.length === 0)
			)
		}
		spritesetMap.createFallImage = function () {
			...
			this._fallImageSprites = [...Array(image.count).keys()].map(
				(_) => new Sprite_Falling(image.id)
			)
