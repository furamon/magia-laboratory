...
;(() => {
		...
		return class extends sceneClass {
			...
			battleMemberWindowWidth() {
				...
				return settings.characterHeight > DEFAULT_CHARACTER_SIZE
					...
					: (settings.characterWidth + characterSpacing) *
							Math.floor(($gameParty.maxBattleMembers() + 1) / 2) +
							32
			}
			/**
			 * @return {Rectangle}
			...
			setupFormationWindows() {
				this.formationWaitingMemberWindow().setActivateAnotherWindow(() =>
					this.activateBattleMemberWindow()
				)
				this.formationWaitingMemberWindow().setStatusWindow(this.formationStatusWindow())
				this.formationWaitingMemberWindow().setStatusParamsWindow(
					this.formationStatusParamsWindow()
				)
				...
				this.formationBattleMemberWindow().setActivateAnotherWindow(() =>
					this.activateWaitingMemberWindow()
				)
			...
			activateWaitingMemberWindow() {
				if (this.formationWaitingMemberWindow().maxItems() > 0) {
					...
					this.formationWaitingMemberWindow().smoothSelect(
						this.targetIndexOfActivateWaitingMember()
					)
			...
			targetIndexOfActivateWaitingMember() {
				let rowOffset =
					this.formationBattleMemberWindow().row() - this.formationBattleMemberWindow().topRow()
				let targetIndex = () =>
					(this.formationWaitingMemberWindow().topRow() + rowOffset) *
					this.formationWaitingMemberWindow().maxCols()
			...
			targetIndexOfActivateBattleMember() {
				let rowOffset =
					this.formationWaitingMemberWindow().row() - this.formationWaitingMemberWindow().topRow()
				let colOffset = this.formationBattleMemberWindow().maxCols() - 1
				let targetIndex = () =>
					(this.formationBattleMemberWindow().topRow() + rowOffset) *
						this.formationBattleMemberWindow().maxCols() +
					colOffset
		...
		createButtons() {
			if (ConfigManager.touchUI) {
				...
				this._cancelButton.x =
					Graphics.boxWidth - this.cancelButtonWidth() - settings.cancelButtonRightMergin
	...
	class Window_FormationMember extends Window_StatusBase {
		...
		itemRect(index) {
			const x =
				this.rectangleOffsetX() +
				(index % this.maxColsForRect()) * this.itemWidth() -
				this.scrollBaseX()
		...
		update() {
			 ...
			 */
			if (
				this._bitmapsMustBeRedraw.length > 0 &&
				this._bitmapsMustBeRedraw.every((bitmap) => bitmap.isReady())
			) {
				this.refresh()
				this._bitmapsMustBeRedraw = []
			}
	...
	class Window_FormationBattleMember extends Window_FormationMember {
		...
		isAtRightEnd() {
			return (
				this.index() % this.maxCols() === this.maxCols() - 1 || this.index() === this.maxItems() - 1
			)
		}
		isAtLeftEnd() {
	...
	class Window_FormationWaitingMember extends Window_FormationMember {
		actor() {
			return this.index() >= 0
				? $gameParty.allMembers()[this.index() + $gameParty.battleMembers().length]
				: undefined
		}
		pendingIndexInParty() {
