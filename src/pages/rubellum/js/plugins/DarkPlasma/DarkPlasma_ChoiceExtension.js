...
;(() => {
	...
	Game_Interpreter.prototype.currentCommandIsMergedChoice = function () {
		const mergedChoice = this._mergedChoices.find(
			(mergedChoice) => mergedChoice.index === this._index
		)
	...
	Game_Interpreter.prototype.mergeSequencialChoices = function () {
		...
		for (
			...
			this._mergedChoices.push(
				new MergedSequencialChoicesIndex(commandIndex, sequencialChoiceIndex)
			)
			this._list[commandIndex].parameters[0].forEach((choice, index) => {
				...
				if (isDisplayed) {
					this._choiceBranches.push(
						new ChoiceBranchIndex(
							this.findChoiceBranchCommandIndex(commandIndex, index),
							branchIndex
						)
					)
					branchIndex++
	...
	Game_Interpreter.prototype.findChoiceEndIndex = function (fromIndex) {
		...
		return (
			this._list
				...
				.findIndex(
					(command) =>
						fromCommand.indent === command.indent && command.code === EVENT_COMMAND.CHOICE_END
				) + fromIndex
		)
	}
	...
	Game_Interpreter.prototype.findChoiceStartIndex = function (fromIndex) {
		...
		return (
			...
			this._list
				...
				.findIndex(
					(command) =>
						fromCommand.indent === command.indent && command.code === EVENT_COMMAND.CHOICE_START
				)
		)
	}
	...
	Game_Interpreter.prototype.command402 = function (params) {
		const choiceBranchIndex = this._choiceBranches.find(
			(choiceBranch) => choiceBranch.commandIndex === this._index
		)
