...
;(() => {
	...
	function Game_Character_MoveToPointMixIn(gameCharacter) {
		...
		gameCharacter.moveRouteListTo = function (x, y) {
			...
			while (openNodes.length > 0) {
				...
				;[2, 4, 6, 8]
					...
					.forEach((dir) => {
						...
						const openNode = openNodes.find(
							(openNode) => openNode.x === newNode.x && openNode.y === newNode.y
						)
						if (openNode) {
							...
							closedNodes.every(
								(closedNode) => closedNode.x !== newNode.x || closedNode.y !== newNode.y
							)
						) {
							openNodes.push(newNode)
						}
			...
			for (
				let node = closedNodes.find((node) => node.isGoal(x, y));
				node && node.parent;
				node = node.parent
			) {
