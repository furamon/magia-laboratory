...
*/
const DotMoveSystem_FunctionExPluginName = document.currentScript
...
;(function (DotMoveSystem) {
	...
	;(function (FunctionEx) {
		...
		const PP = PluginParamsParser.parse(
			PluginManager.parameters(DotMoveSystem_FunctionExPluginName),
			typeDefine
		)
		/*
		 * ● 定数定義
		...
		Game_Player.prototype.initMembers = function () {
			...
			this._transferOffsetX =
				PP.PlayerInfo.TransferOffsetX == null ? 0 : PP.PlayerInfo.TransferOffsetX
		...
		Game_Follower.prototype.initMembers = function () {
			...
			this._transferOffsetX =
				PP.FollowerInfo.TransferOffsetX == null ? 0 : PP.FollowerInfo.TransferOffsetX
			this._transferOffsetY =
				PP.FollowerInfo.TransferOffsetX == null ? 0 : PP.FollowerInfo.TransferOffsetX
		}
		/*
		...
		DotMoveSystem.CharacterMover.prototype.dotMoveByDeg = function (
			deg,
		...
		Game_CharacterBase.prototype.originDistancePerFrame =
		 ...
		 * ● 壁スライド許可/禁止切り替え
		 */
		DotMoveSystem.CharacterDotMoveProcess.prototype.dotMoveByDeg = function (
			deg,
			dpf = this._character.distancePerFrame()
		) {
			this._dpf = dpf
			...
			const margin =
				this._character.distancePerFrame() / DotMoveSystem.DotMoveUtils.MOVED_MARGIN_UNIT
			let moved = true
		...
		const _CharacterMover_continuousMoveProcess =
			DotMoveSystem.CharacterMover.prototype.continuousMoveProcess
		 ...
		 * ● 当たり判定付きジャンプ
		 */
		Game_CharacterBase.prototype.smartJump = function (
			xPlus,
			yPlus,
			baseJumpPeak = 10,
			through = false
		...
		Game_CharacterBase.prototype.smartJumpByDeg = function (
			deg,
			far,
			baseJumpPeak = 10,
			through = false
		...
		Game_CharacterBase.prototype.smartJumpAbs = function (
			x,
			y,
			baseJumpPeak = 10,
			through = false
		...
		DotMoveSystem.CharacterCollisionChecker.prototype.getMassRects = function (x, y) {
			switch (this.getMassCollisionType(x, y)) {
				...
				case 9:
					return [
						new DotMoveSystem.DotMoveRectangle(x, y, 0.5, 1),
						new DotMoveSystem.DotMoveRectangle(x + 0.5, y + 0.5, 0.5, 0.5)
					]
				case 10:
					return [
						new DotMoveSystem.DotMoveRectangle(x, y, 0.5, 1),
						new DotMoveSystem.DotMoveRectangle(x + 0.5, y, 0.5, 0.5)
					]
				case 11:
					return [
						new DotMoveSystem.DotMoveRectangle(x + 0.5, y, 0.5, 1),
						new DotMoveSystem.DotMoveRectangle(x, y, 0.5, 0.5)
					]
				case 12:
					return [
						new DotMoveSystem.DotMoveRectangle(x + 0.5, y, 0.5, 1),
						new DotMoveSystem.DotMoveRectangle(x, y + 0.5, 0.5, 0.5)
					]
			}
		...
		const _CharacterCollisionChecker_isNoTargetMass =
			DotMoveSystem.CharacterCollisionChecker.prototype.isNoCheckMass
		DotMoveSystem.CharacterCollisionChecker.prototype.isNoCheckMass = function (
			ix,
			iy,
			d,
			massRange
			...
			if (
				this.getMassCollisionType(ix, iy) >= 1 &&
				this.getMassCollisionType(ix, iy) <= END_TRIANGLE_ID
			) {
				return false
			}
			return _CharacterCollisionChecker_isNoTargetMass.call(this, ix, iy, d, massRange)
		}
		DotMoveSystem.CharacterCollisionChecker.prototype.checkCollisionMass = function (
			subjectRect,
			d,
			ix,
			iy
			...
			if (!this.checkPassMass(ix, iy, d)) {
				...
				for (const massRect of massRects) {
					const result = this.checkCollidedRect(
						...
						subjectRect.clone(),
						massRect,
						new DotMoveSystem.MassInfo(ix, iy)
					)
		...
		DotMoveSystem.CharacterCollisionChecker.prototype.checkPassMass = function (ix, iy, d) {
			...
			const prevPoint = DotMoveSystem.DotMoveUtils.prevPointWithDirection(
				new DotMoveSystem.DotMovePoint(ix, iy),
				d
			)
			if (
				this.getMassCollisionType(prevPoint.x, prevPoint.y) >= 1 &&
				this.getMassCollisionType(prevPoint.x, prevPoint.y) <= END_TRIANGLE_ID
			) {
				return true
			}
		...
		const _CharacterCollisionChecker_checkCollisionXCliff =
			DotMoveSystem.CharacterCollisionChecker.prototype.checkCollisionXCliff
		DotMoveSystem.CharacterCollisionChecker.prototype.checkCollisionXCliff = function (
			subjectRect,
			x1,
			x2,
			iy,
			...
			if (
				this.getMassCollisionType(x1, iy) >= 1 &&
				this.getMassCollisionType(x1, iy) <= END_TRIANGLE_ID &&
				this.getMassCollisionType(x2, iy) >= 1 &&
				this.getMassCollisionType(x2, iy) <= END_TRIANGLE_ID
			) {
				return []
			}
			return _CharacterCollisionChecker_checkCollisionXCliff.call(this, subjectRect, x1, x2, iy, d)
		}
		const _CharacterCollisionChecker_checkCollisionYCliff =
			DotMoveSystem.CharacterCollisionChecker.prototype.checkCollisionYCliff
		DotMoveSystem.CharacterCollisionChecker.prototype.checkCollisionYCliff = function (
			subjectRect,
			y1,
			y2,
			ix,
			...
			if (
				this.getMassCollisionType(ix, y1) >= 1 &&
				this.getMassCollisionType(ix, y1) <= END_TRIANGLE_ID &&
				this.getMassCollisionType(ix, y2) >= 1 &&
				this.getMassCollisionType(ix, y2) <= END_TRIANGLE_ID
			) {
				return []
			}
		...
		DotMoveSystem.CharacterCollisionChecker.prototype.calcMassTriangle = function (
			id,
			characterRect,
			direction,
			ix,
		...
		const _CharacterCollisionChecker_checkCollisionCliff =
			DotMoveSystem.CharacterCollisionChecker.prototype.checkCollisionCliff
		DotMoveSystem.CharacterCollisionChecker.prototype.checkCollisionCliff = function (
			subjectRect,
			massRange,
			d
		) {
		...
		const _CharacterCollisionChecker_checkCollisionMass =
			DotMoveSystem.CharacterCollisionChecker.prototype.checkCollisionMass
		DotMoveSystem.CharacterCollisionChecker.prototype.checkCollisionMass = function (
			subjectRect,
			d,
			ix,
		...
		DotMoveSystem.CharacterCollisionChecker.prototype.checkCollisionMassLeftUp = function (
			subjectRect,
			d,
			ix,
			iy
		...
		DotMoveSystem.CharacterCollisionChecker.prototype.checkCollisionMassDownLeft = function (
			subjectRect,
			d,
			ix,
			iy
		...
		DotMoveSystem.CharacterCollisionChecker.prototype.checkCollisionMassRightDown = function (
			subjectRect,
			d,
			ix,
			iy
		...
		DotMoveSystem.CharacterCollisionChecker.prototype.checkCollisionMassUpRight = function (
			subjectRect,
			d,
			ix,
			iy
		...
		DotMoveSystem.CharacterDotMoveProcess.prototype.calcUp = function (dis) {
			...
			if (collisionResults.length >= 1) {
				if (this.checkCollisionResultIsAllTriangleMass(collisionResults, LEFT_UP_TRIANGLE_ID)) {
					...
					this.checkCollisionResultIsAllTriangleMass(collisionResults, UP_RIGHT_TRIANGLE_ID)
				) {
		...
		DotMoveSystem.CharacterDotMoveProcess.prototype.calcRight = function (dis) {
			...
			if (collisionResults.length >= 1) {
				if (this.checkCollisionResultIsAllTriangleMass(collisionResults, UP_RIGHT_TRIANGLE_ID)) {
					...
					this.checkCollisionResultIsAllTriangleMass(collisionResults, RIGHT_DOWN_TRIANGLE_ID)
				) {
					const dis2 = this.calcDistance(DotMoveSystem.Degree.UP_RIGHT)
					return this.calcUpRight(dis2)
				}
		...
		DotMoveSystem.CharacterDotMoveProcess.prototype.calcLeft = function (dis) {
			...
			if (collisionResults.length >= 1) {
				if (this.checkCollisionResultIsAllTriangleMass(collisionResults, LEFT_UP_TRIANGLE_ID)) {
					...
					this.checkCollisionResultIsAllTriangleMass(collisionResults, DOWN_LEFT_TRIANGLE_ID)
				) {
					const dis2 = this.calcDistance(DotMoveSystem.Degree.LEFT_UP)
					return this.calcLeftUp(dis2)
				}
		...
		DotMoveSystem.CharacterDotMoveProcess.prototype.checkCollisionResultIsAllTriangleMass =
			function (collisionResults, type) {
