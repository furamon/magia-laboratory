...
;(() => {
	...
	var MapActorStatusPluginName = document.currentScript
		? decodeURIComponent(document.currentScript.src.match(/^.*\/(.+)\.js$/)[1])
		: 'MapActorStatus'
	var MapActorStatusPluginParams = PluginParamsParser.parse(
		PluginManager.parameters(MapActorStatusPluginName)
	)
	var ActorContainer = class extends PIXI.Container {
		...
		createMpGauge() {
			...
			this._mpGauge.y =
				ActorContainer.GAUGE_START_Y + this._hpGauge.height + ActorContainer.GAUGE_SPACE_Y
		...
		updateActorContainers() {
			...
			if (this._lastMembers.length < members.length) {
				for (let i = this._lastMembers.length; i < members.length; i++) {
					const actorContainer = new ActorContainer(
						i * MapActorStatusPluginParams.StatusAreaSpace,
						0
					)
	...
	Spriteset_Map.prototype.createPartyContainer = function () {
		this._partyContainer = new PartyContainer(
			MapActorStatusPluginParams.StatusAreaX,
			MapActorStatusPluginParams.StatusAreaY
		)
