...
;(() => {
	...
	function mixin(dest, src) {
		for (const name of Object.getOwnPropertyNames(src.prototype)) {
			...
			const value =
				Object.getOwnPropertyDescriptor(src.prototype, name) || /* @__PURE__ */ Object.create(null)
			Object.defineProperty(dest.prototype, name, value)
		}
	}

	// ts/CharacterImageEx/CharacterImageEx.ts
	var CharacterImageExPluginName = document.currentScript
		? decodeURIComponent(document.currentScript.src.match(/^.*\/(.+)\.js$/)[1])
		: 'CharacterImageEx'
	var CharacterImageExPluginParams = PluginParamsParser.parse(
		PluginManager.parameters(CharacterImageExPluginName)
	)
	...
	PluginManager.registerCommand(CharacterImageExPluginName, 'SetAngle', function (args) {
		...
		const angle =
			params.AngleByVariable > 0 ? $gameVariables.value(params.AngleByVariable) : params.Angle
	...
	PluginManager.registerCommand(CharacterImageExPluginName, 'SetScale', function (args) {
		...
		const character = this.findCharacterBySpecification(params.CharacterSpecification)
		const scaleX =
			params.ScaleXByVariable > 0 ? $gameVariables.value(params.ScaleXByVariable) : params.ScaleX
		const scaleY =
			params.ScaleYByVariable > 0 ? $gameVariables.value(params.ScaleYByVariable) : params.ScaleY
	...
	PluginManager.registerCommand(CharacterImageExPluginName, 'SetPicture', function (args) {
		...
		const frame = new Rectangle(
			params.PictureFrame.X,
			params.PictureFrame.Y,
			params.PictureFrame.Width,
			params.PictureFrame.Height
		)
	...
	PluginManager.registerCommand(CharacterImageExPluginName, 'SetSubPriority', function (args) {
		...
		const character = this.findCharacterBySpecification(params.CharacterSpecification)
		const subPriority =
			params.SubPriorityByVariable > 0
				? $gameVariables.value(params.SubPriorityByVariable)
				: params.SubPriority
