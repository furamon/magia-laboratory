...
;(() => {
	...
	const HIDE_SWITCH_AB_BUTTONS_IF_GAMEPAD_UNCONNECTED =
		pluginParams.hideSwitchABButtonsIfGamepadUnconnected === 'true'
	...
	ConfigManager.useSwitchABButtons = function () {
		return (
			USE_SWITCH_AB_BUTTONS &&
			(!HIDE_SWITCH_AB_BUTTONS_IF_GAMEPAD_UNCONNECTED || Input.isAnyGamepadConnected())
		)
	}
	...
	Object.defineProperty(Input, 'gamepadMapper', {
		get: function () {
			return USE_SWITCH_AB_BUTTONS && ConfigManager.switchABButtons
				? this.switchedGamepadMapper
				: this.defaultGamepadMapper
		},
		set: function (value) {
	...
	Window_Message.prototype.processEscapeCharacter = function (code, textState) {
		if (USE_FAST_MESSAGE && ConfigManager.fastMessage) {
			switch (code) {
				case '.':
					if (WAIT_TIME_FOR_FAST_MESSAGE !== 0)
						this.startWait(Math.floor(15 / WAIT_TIME_FOR_FAST_MESSAGE))
					return
				case '|':
					if (WAIT_TIME_FOR_FAST_MESSAGE !== 0)
						this.startWait(Math.floor(60 / WAIT_TIME_FOR_FAST_MESSAGE))
					return
				case '!':
	...
	Window_Options.prototype.addBooleanOptions = function () {
		...
		if (ConfigManager.useSwitchABButtons())
			this.addCommand(SWITCH_AB_BUTTONS_NAME, 'switchABButtons')
	...
	Window_Options.prototype.isBooleanSymbol = function (symbol) {
		return ['alwaysDash', 'commandRemember', 'touchUI', 'switchABButtons', 'fastMessage'].includes(
			symbol
		)
	}
	...
	Window_Options.prototype.changeNumberBySymbol = function (
		symbol,
		forward,
		skip,
		wrap,
