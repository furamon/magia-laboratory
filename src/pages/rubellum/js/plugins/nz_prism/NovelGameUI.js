...
;(() => {
	...
	const SKIP_BUTTON_UNSELECTED_DISABLED_IMAGE_NAME =
		pluginParams.skipButtonUnselectedDisabledImageName
	const SKIP_BUTTON_SELECTED_DISABLED_IMAGE_NAME = pluginParams.skipButtonSelectedDisabledImageName
	const SKIP_BUTTON_UNSELECTED_ENABLED_IMAGE_NAME =
		pluginParams.skipButtonUnselectedEnabledImageName
	...
	const HIDE_UI_VISIBLE_PICTURE_IDS = JSON.parse(pluginParams.hideUiVisiblePictureIds).map((str) =>
		Number(str)
	)
	const HIDE_UI_INVISIBLE_PICTURE_IDS = JSON.parse(pluginParams.hideUiInvisiblePictureIds).map(
		(str) => Number(str)
	)
	...
	Game_Screen.prototype.showPicture = function (
		...
		if (HIDE_UI_VISIBLE_PICTURE_IDS.includes(pictureId))
			this._pictures[this.realPictureId(pictureId)].hide()
	}
	...
	Game_Interpreter.prototype.update = function () {
		while (this.isRunning()) {
			...
			if (
				isDescendant &&
				!$gameMessage.isHideUiMode() &&
				$gameSystem.areControlButtonsEnabled() &&
				this.updateInput()
			)
				break
	...
	const _Window_Message_prototype_processEscapeCharacter =
	...
	Window_MenuCommand.prototype.addOriginalCommands = function () {
		...
		if (USE_LOG && ADD_LOG_COMMAND)
			this.addCommand(LOG_COMMAND_NAME, 'backLog', !$gameSystem.isBackLogEmpty())
	}
	...
	const _Window_Options_prototype_restoreDefaultValues =
	...
	Sprite_ControlButton.prototype.bitmapName = function () {
		switch (this._buttonType) {
			case OPTIONS_KEY:
				return this._hovered
					? OPTIONS_BUTTON_SELECTED_IMAGE_NAME
					: OPTIONS_BUTTON_UNSELECTED_IMAGE_NAME
			case SAVE_KEY:
			...
			case SKIP_KEY:
				if ($gameTemp.skippableLabelIndex() >= 0) {
					return this._hovered
						? SKIP_BUTTON_SELECTED_ENABLED_IMAGE_NAME
						: SKIP_BUTTON_UNSELECTED_ENABLED_IMAGE_NAME
				} else {
					return this._hovered
						? SKIP_BUTTON_SELECTED_DISABLED_IMAGE_NAME
						: SKIP_BUTTON_UNSELECTED_DISABLED_IMAGE_NAME
				}
			case AUTO_KEY:
				if ($gameMessage.isAutoMode()) {
					return this._hovered
						? AUTO_BUTTON_SELECTED_ON_IMAGE_NAME
						: AUTO_BUTTON_UNSELECTED_ON_IMAGE_NAME
				} else {
					return this._hovered
						? AUTO_BUTTON_SELECTED_OFF_IMAGE_NAME
						: AUTO_BUTTON_UNSELECTED_OFF_IMAGE_NAME
				}
			case HIDE_UI_KEY:
				return this._hovered
					? HIDE_UI_BUTTON_SELECTED_IMAGE_NAME
					: HIDE_UI_BUTTON_UNSELECTED_IMAGE_NAME
			case GAME_END_KEY:
				return this._hovered
					? GAME_END_BUTTON_SELECTED_IMAGE_NAME
					: GAME_END_BUTTON_UNSELECTED_IMAGE_NAME
		}
	}
