...
;(function () {
	...
	Game_Player.prototype.reserveTransferToOriginalMap = function () {
		...
		if (param.autoTransparent) {
			...
			this._followers
				.data()
				.forEach((follower) => follower.setTransparent(this._originalTransparent))
		}
	...
	Window_MenuCommand.prototype.makeSubCommandList = function () {
		...
		$gameTemp.iterateMenuParents((subCommands, parentName) => {
			...
			if (this.checkSubCommands('isVisible')) {
				const commandName = this._maskedName[parentName]
	...
	Window_MenuSubCommand.prototype.standardFontSize = function () {
		return (
			userSetting.subCommandWindow.fontSize || Window_Command.prototype.standardFontSize.call(this)
		)
	}

	Window_MenuSubCommand.prototype.standardPadding = function () {
		return (
			userSetting.subCommandWindow.padding || Window_Command.prototype.standardPadding.call(this)
		)
	}
	...
	class Game_MenuSubCommand {
		...
		isEnable() {
			return (
				!$gameSwitches.value(this._data.DisableSwitchId) &&
