...
;(function () {
	...
	class StandPictureParam {
		...
		createCondition() {
			const conditions = []
			conditions.push(
				(file) => !file.HpUpperLimit || file.HpUpperLimit >= this._actor.hpRate() * 100
			)
			conditions.push(
				(file) => !file.HpLowerLimit || file.HpLowerLimit <= this._actor.hpRate() * 100
			)
			...
			conditions.push(
				(file) =>
					!file.Inputting ||
					this._actor.isInputCommand(file.InputCommand, file.InputSkillType) ||
					!$gameParty.inBattle()
			)
	...
	Scene_Skill.prototype.findStandPictureMember = function () {
		...
		return param.MenuActorOnly
			? [this.actor()]
			: Scene_Base.prototype.findStandPictureMember.call(this)
	}
	...
	Scene_Shop.prototype.findStandPictureMember = function () {
		...
		const member = Scene_Base.prototype.findStandPictureMember.call(this)
		return member.map((actor) =>
			tempActors?.has(actor.actorId()) ? tempActors.get(actor.actorId()) : actor
		)
	}
	...
	if (window.Scene_CustomMenu) {
		Scene_CustomMenu.prototype.findStandPictureMember = function () {
			...
			return param.MenuActorOnly && changeable
				? [this.actor()]
				: Scene_Base.prototype.findStandPictureMember.call(this)
		}
	}
