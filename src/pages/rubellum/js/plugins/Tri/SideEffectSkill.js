...
;(() => {
	...
	Game_Action.prototype.isValidSideEffectForSuccess = function () {
		return (
			...
			!PluginManagerEx.findMetaValue(this.item(), ['HitOnly', '成功時のみ'])
		)
	}
	...
	Game_Action.prototype.isValidSideEffectForFailure = function () {
		return (
			!this._successForSideEffect ||
			!PluginManagerEx.findMetaValue(this.item(), ['MissOnly', '失敗時のみ'])
		)
	}
	...
	Game_Action.prototype.isValidSideEffectForEffective = function () {
		return (
			...
			!PluginManagerEx.findMetaValue(this.item(), ['EffectiveOnly', '弱点時のみ'])
		)
	}

	Game_Action.prototype.isValidSideEffectForResist = function () {
		return (
			this._resistForSideEffect ||
			!PluginManagerEx.findMetaValue(this.item(), ['ResistOnly', '耐性時のみ'])
		)
	}
	...
	Game_Action.prototype.isValidSideEffectForCritical = function () {
		return (
			...
			!PluginManagerEx.findMetaValue(this.item(), ['CriticalOnly', '会心時のみ'])
		)
	}
	...
	Game_Action.prototype.isValidSideEffectForInvalid = function () {
		return (
			...
			!PluginManagerEx.findMetaValue(this.item(), ['InvalidOnly', '無効時のみ'])
		)
	}
	...
	Game_Action.prototype.isValidSideEffectForReflection = function () {
		return (
			...
			!PluginManagerEx.findMetaValue(this.item(), ['ReflectionOnly', '反射時のみ'])
		)
	}

	Game_Action.prototype.isValidSideEffectForTag = function () {
		const id = this.item().id
		const allInvalidate = this.subject()
			.traitObjects()
			.some((obj) =>
				PluginManagerEx.findMetaValue(obj, [`SESkillInvalidate`, `スキル副作用無効化`])
			)
		if (allInvalidate) {
			return false
		}
		if (DataManager.isSkill(this.item())) {
			return this.subject()
				...
				.every(
					(obj) =>
						!PluginManagerEx.findMetaValue(obj, [
							`SESkillInvalidate_${id}`,
							`スキル副作用無効化_${id}`
						])
				)
		} else {
