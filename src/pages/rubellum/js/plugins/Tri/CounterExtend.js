...
;(function () {
	...
	class Game_CounterAction extends Game_Action {
		...
		setup(triggerAction, target) {
			if (
				triggerAction.isCounter() ||
				this.friendsUnit().members().contains(target) ||
				!this.subject().canMove()
			) {
				return
			}
		...
		findParams() {
			...
			tagList.forEach((tag) => {
				...
				const counter = param.CounterList.find(
					(item, index) => tagList.contains(item.Id) || tagIndex === index
				)
				if (counter) {
		...
		checkCondition(skill, triggerAction, counter) {
			...
			const evasion =
				PluginManagerEx.findMetaValue(triggerSkill, ['CounterEvasion', '反撃回避']) || 0
			const frequency =
				skill.Frequency +
				subject.traitObjects().reduce((prev, traitObject) => {
			...
			conditions.push(
				() => skill.ElementCondition && !triggerAction.hasElement(skill.ElementCondition)
			)
			conditions.push(
				() =>
					skill.WeakCondition &&
					!this.hasWeakResistance(triggerAction, subject, skill.WeakCondition)
			)
			conditions.push(() => skill.SwitchCondition && !$gameSwitches.value(skill.SwitchCondition))
			conditions.push(
				() =>
					skill.MemoTagCondition && !this.hasMemoTag(triggerSkill, subject, skill.MemoTagCondition)
			)
