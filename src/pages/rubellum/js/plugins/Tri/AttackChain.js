...
;(function () {
	...
	param.enemyUnitImageSuffix = getParamString([
		'EnemyUnitImageSuffix',
		'敵用単位画像ファイル接尾辞'
	])
	...
	Game_Unit.prototype.addChainDamage = function (damageValue) {
		...
		if (
			this.isCountMaxChain() &&
			(this._chainDamage > this._maxChainDamage || !this._maxChainDamage)
		) {
	...
	Game_Unit.prototype.getChainRate = function (addRate) {
		return (
			(100 + this.getChainCount() * param.damageRate * addRate).clamp(
				0,
				param.maxRate || Infinity
			) / 100
		)
	}
	...
	Game_Action.prototype.isChangeSkillDueToChain = function () {
		return (
			this.getMetaNumberForAttackChain(['スキル変化連携数', 'SkillChangeChain']) <=
			this.friendsUnit().getChainCount()
		)
	}
	...
	Game_Item.prototype.getChangeMessage = function () {
		...
		var changeMessage =
			getMetaValues(originalItem, ['スキル変化メッセージ', 'SkillChangeMessage']) ||
			param.skillChangeMessage
		if (changeMessage) {
	...
	Sprite_ChainCount.prototype.createUnitBitmap = function () {
		...
		if (fileName) {
			...
			this._unitEnemyBitmap = ImageManager.loadSystem(
				fileName + param.enemyUnitImageSuffix || '',
				0
			)
		}
	}
	...
	Sprite_ChainDamage.prototype.getFontSize = function () {
		return (
			param.damageFontSize || Math.floor(Sprite_ChainCount.prototype.getFontSize.call(this) * 0.75)
		)
	}
