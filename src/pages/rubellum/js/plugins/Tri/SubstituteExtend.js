...
;(function () {
	...
	Game_BattlerBase.prototype.isSubstitute = function () {
		...
		const result =
			_Game_BattlerBase_isSubstitute.apply(this, arguments) && this.isSubstituteExtend()
	...
	Game_BattlerBase.prototype.isValidSubstituteRestriction = function () {
		const restrictionId = this.getSubstituteMetaInfo(
			['SE_TargetRestriction', 'SE_身代わり対象限定'],
			true
		)
		if (restrictionId) {
	...
	Game_BattlerBase.prototype.isValidSubstituteSwitch = function () {
		const switchId = this.getSubstituteMetaInfo(
			['SE_SubstituteSwitch', 'SE_身代わりスイッチ'],
			true
		)
		if (switchId) {
	...
	Game_BattlerBase.prototype.isEqualSubstituteRestrictionId = function (restrictionId) {
		const restrictionTargetId = this.getSubstituteMetaInfo(
			['SE_SubstituteTarget', 'SE_身代わり対象者'],
			true
		)
