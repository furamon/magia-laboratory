		...
		if (action.isForOpponent()) {
			if (action.isForAll()) {
				for (const target of targets) {
					...
					if (
						!targetEnemyInfo ||
						targetEnemyInfo.inspirationLevel < setTargetEnemyInfo.inspirationLevel
					) {
		...
		if (targetEnemyInfo) {
			for (const skillId in skillInfos) {
				...
				if (actorIds.includes(subject._actorId) && !subject.hasSkill(skillId)) {
					if (
						!skillInfo.derivedSkillId ||
						skillInfo.derivedSkillId === 0 ||
						skillInfo.derivedSkillId === action._item._itemId
					) {
			...
			if (inspirationSkills.length) {
				const inspirationSkill =
					inspirationSkills[Math.floor(Math.random() * inspirationSkills.length)]
