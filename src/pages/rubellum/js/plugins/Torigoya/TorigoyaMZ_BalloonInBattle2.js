...
;(function () {
	...
	function readParameter() {
		...
		return {
			...
			advancedDamageLifeTime: pickIntegerValueFromParameter(
				parameter,
				'advancedDamageLifeTime',
				30
			),
			advancedInputLifeTime: pickIntegerValueFromParameter(parameter, 'advancedInputLifeTime', -1),
			advancedVictoryLifeTime: pickIntegerValueFromParameter(
				parameter,
				'advancedVictoryLifeTime',
				-1
			),
			advancedLayerPosition: pickStringValueFromParameter(
				parameter,
				'advancedLayerPosition',
				'normal'
			)
		}
	}
	...
	class Window_BattleBalloon extends Window_Base {
		...
		_updateTrackingBattlerSprite() {
			...
			const battlerY =
				this._battlerPosition.y + this._battlerSprite.torigoyaBalloonInBattle_balloonY()
			...
			if (this.y + this.height > Graphics.height)
				this.y = Graphics.height - this.height - this._tailY()
	...
	;(() => {
		...
		function getTalkSet(talkSetId) {
			if (!talkSetCache.has(talkSetId)) {
				const config = Torigoya.BalloonInBattle.parameter.talkConfig.find(
					(config) => config.id === talkSetId
				)
		...
		Game_Battler.prototype.torigoyaBalloonInBattle_setMessageParameter = function (
			type,
			message,
			options = {}
		) {
		...
		const upstream_Window_BattleLog_displayActionResults =
		...
		Window_BattleLog.prototype.torigoyaBalloonInBattle_checkTalk = function (
			subject,
			target,
			result
		) {
		...
		// ミス
		Window_BattleLog.prototype.torigoyaBalloonInBattle_checkTalkForMissed = function (
			subject,
			target,
			result
		) {
		...
		// 回避
		Window_BattleLog.prototype.torigoyaBalloonInBattle_checkTalkForEvasion = function (
			subject,
			target,
			result
		) {
		...
		// ダメージによる表示
		Window_BattleLog.prototype.torigoyaBalloonInBattle_checkTalkForDamage = function (
			subject,
			target,
			result
		) {
		...
		// HP回復による表示
		Window_BattleLog.prototype.torigoyaBalloonInBattle_checkTalkForRecovery = function (
			subject,
			target,
			result
		) {
		...
		PluginManager.registerCommand(
			Torigoya.BalloonInBattle.name,
			'changeTalkSet',
			commandChangeTalkSetId
		)
		PluginManager.registerCommand(
			Torigoya.BalloonInBattle.name,
			'resetTalkSet',
			commandResetTalkSetId
		)
		PluginManager.registerCommand(
			Torigoya.BalloonInBattle.name,
			'talkActorByType',
			commandTalkActorByType
		)
		PluginManager.registerCommand(
			Torigoya.BalloonInBattle.name,
			'talkActorByText',
			commandTalkActorByText
		)
		PluginManager.registerCommand(
			Torigoya.BalloonInBattle.name,
			'talkEnemyByType',
			commandTalkEnemyByType
		)
		PluginManager.registerCommand(
			Torigoya.BalloonInBattle.name,
			'talkEnemyByText',
			commandTalkEnemyByText
		)
	})()
})()
