 ...
 * @param ロード画面でニューゲーム
 * @parent ロード画面
 * @desc ロード画面の一番下のスロットをニューゲーム用スロットにする
 * @type boolean
 * @default true
 *
 * @param ロード画面にタイトル画像
...
;(() => {
	...
	// フェード時間
	const keke_titleFadeTime = parameters['タイトル']
		? Math.round(Number(parameters['タイトル']) / 2) || 1
		: ''
	const keke_loadFadeTime = parameters['ロード']
		? Math.round(Number(parameters['ロード']) / 2) || 1
		: ''
	const keke_newGameAfterFadeTime = parameters['ニューゲーム後']
		? Number(parameters['ニューゲーム後']) || 1
		: ''
	const keke_loadAfterFadeTime = parameters['ロード後'] ? Number(parameters['ロード後']) || 1 : ''
	const keke_gameEndFadeTime = parameters['ゲーム終了']
		? Math.round(Number(parameters['ゲーム終了']) / 2) || 1
		: ''
	...
	function checkTitleSkip(sm, sceneClass) {
		...
		if (sm._skipedTitleKe) {
			return false
		}
		if (isForceTitle(sm, sm._scene)) {
			return false
		}
		if (keke_skipTo == 'ロード画面' && sm._loadWentKe && !keke_noBackTitle) {
			return false
		}
		return true
	}
	...
	function isForceTitle(sm, scene) {
		if (sm._forceTitledKe) {
			return true
		}
		if (!keke_titlePressing) {
			return false
		}
		// 強制タイトル時の処理
	...
	Scene_Map.prototype.needsFadeIn = function () {
		return (
			_Scene_Map_needsFadeIn.apply(this) ||
			SceneManager.isPreviousScene(Scene_Boot) ||
			SceneManager.isPreviousScene(Scene_GameEnd)
		)
	}
	...
	//- ロード時、最後のスロットをニューゲームスロットに(コア追加)
	const _Scene_Load_onSavefileOk = Scene_Load.prototype.onSavefileOk
	Scene_Load.prototype.onSavefileOk = function () {
		const savefileId = this.savefileId()
		if (keke_newGameSlot && savefileId == this._listWindow.maxItems() - 1) {
	...
	Window_SavefileList.prototype.drawItem = function (index) {
		if (validNewGameSlot() && index == this.maxItems() - 1) {
	...
	function createLoadBackground(scene) {
		scene._backSprite1 = new Sprite(ImageManager.loadTitle1($dataSystem.title1Name))
		scene._backSprite2 = new Sprite(ImageManager.loadTitle2($dataSystem.title2Name))
	...
	Scene_Title.prototype.fadeSpeed = function () {
		if (keke_titleFadeTime) {
			return keke_titleFadeTime
		}
	...
	Scene_Load.prototype.fadeSpeed = function () {
		if (keke_loadFadeTime) {
			return keke_loadFadeTime
		}
		return Scene_Base.prototype.fadeSpeed.call(this)
	}
	...
	Scene_GameEnd.prototype.fadeSpeed = function () {
		if (keke_gameEndFadeTime) {
			return keke_gameEndFadeTime
		}
		return Scene_Base.prototype.fadeSpeed.call(this)
	}
	...
	Scene_Map.prototype.fadeSpeed = function () {
		...
		if (newGameAfter) {
			newGameAfter = false
			if (keke_newGameAfterFadeTime) {
				return keke_newGameAfterFadeTime
			}
		}
		// ロード後
		if (loadAfter) {
			loadAfter = false
			if (keke_loadAfterFadeTime) {
				return keke_loadAfterFadeTime
			}
		}
		return Scene_Base.prototype.fadeSpeed.call(this)
	}
