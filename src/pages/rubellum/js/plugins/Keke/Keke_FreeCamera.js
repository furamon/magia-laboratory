...
;(() => {
	...
	//- ズームの呼び出し(公開)
	Game_Screen.prototype.callZoomKe = function (
		chara,
		time,
		scale,
		offsetX,
	...
	PluginManager.registerCommand(pluginName, 'ズーム除外', (args) => {
		...
		const charas = Array.from(
			new Set([...getCharasByName(names, self), ...getCharasById(ids, self)])
		)
		// ズーム除外を設定
	...
	Sprite_Character.prototype.initialize = function (character) {
		...
		if (character._eventId && !$gameMap._pcStartedKeFrcm && character._pageIndex >= 0) {
			...
			runPluginCmdStarting(
				character.list(),
				[/ズーム除外/, /カメラ操作/, /スワイプモード開始/, /スワイプモード終了/],
				'開始時に実行'
			)
	...
	function isFocusChara(chara) {
		const p = getCameraParam()
		if (!p) {
			return chara._followers ? true : false
		}
	...
	function scaleMap(p) {
		...
		if (!spriteset || !spriteset._tilemap || !spriteset._tilemap.parent) {
			return
		}
		const scale = $gameScreen.zoomScale()
		spriteset._tilemap.width =
			Math.ceil((Graphics.width / scale) * 1) + spriteset._tilemap._margin * 2
		spriteset._tilemap.height =
			Math.floor((Graphics.height / scale) * 1) + spriteset._tilemap._margin * 2
	...
	function isChildScene(sprite) {
		...
		if (parent.constructor.name.includes('Scene_')) {
			return true
		}
		...
		if (parent2.constructor.name.includes('Scene_')) {
			return true
		}
		return false
	}
	...
	Game_Map.prototype.updateScroll = function () {
		...
		if (this.isScrolling()) {
			...
			if (this._displayX != lastX) {
				p.x += (this._displayX - lastX) * $gameMap.tileWidth()
			}
			if (this._displayY != lastY) {
				p.y += (this._displayY - lastY) * $gameMap.tileHeight()
			}
		}
	}
	...
	function updateZoomNo(sprite) {
		...
		sprite._zoomNoScale =
			zoomScale < 1
				? Math.max(1, (1 / zoomScale) * chara._zoomNoKe)
				: Math.min(1, (1 / zoomScale) * chara._zoomNoKe)
		// 補正
		sprite.scale.x *= sprite._zoomNoScale
		sprite.scale.y *= sprite._zoomNoScale
		sprite.z = 100
	...
	function updateSwipeMode() {
		...
		// 二本指タッチ判定
		if (isPinchZoom) {
			if (TouchInput._currentState.cancelled) {
				twinTouched = true
			}
			if (TouchInput.isReleased() && twinTouched) {
				twinTouched = false
			}
		}
		// スワイプの更新
	...
	function updateSwipe(p) {
		...
		// キースワイプの更新
		if (keySwiping) {
			swiped = swiped || updateKeySwipe(p)
		}
		// スワイプしたら慣性を停止
		if (swiped) {
			p.inertiaDura = 0
	...
	function updateSwipeBack(p) {
		...
		if (!p.swipeBackPosWhenMove && !p.swipeBackZoomWhenMove) {
			return false
		}
		// プレイヤーが移動したか
		if (!checkPlayerMoved(p)) {
			return false
		}
		// スワイプパラムの初期化
		initSwipeParam(p)
	...
	function inKeySwipe() {
		...
		if (!inSwipeMode() || !swipeKey || swipeKey == 'なし') {
			return false
		}
	...
	function getSwipeVol(p) {
		...
		if (TouchInput.isPressed()) {
			...
			if (p.touchXPre) {
				...
				if (swipeX) {
					addSwipeX(p, swipeX)
					clearTimeout(timeout_initSwipeX)
				} else {
					p.swipeX = null
					timeout_initSwipeX = setTimeout(initSwipeVol, playTime, p, 'x')
				}
				const swipeY = TouchInput.y - p.touchYPre
				if (swipeY) {
					addSwipeY(p, swipeY)
					clearTimeout(timeout_initSwipeY)
				} else {
					p.swipeY = null
					timeout_initSwipeY = setTimeout(initSwipeVol, playTime, p, 'y')
				}
			}
	...
	function updatePinchZoom(p) {
		if (!twinTouched || twinTouchWidths.length < 2) {
			return false
		}
	...
	function updateSmoothZoom(p) {
		if (!p.smoothZooms || !p.smoothZooms.length) {
			return false
		}
		let zoomed = false
		let del = false
	...
	function changeZoomScale(vol, p) {
		...
		gs._zoomScale += vol
		if (p.zoomScaleMax && gs._zoomScale > p.zoomScaleMax) {
			gs._zoomScale = p.zoomScaleMax
			isMax = true
		}
		if (p.zoomScaleMin && gs._zoomScale < p.zoomScaleMin) {
			gs._zoomScale = p.zoomScaleMin
			isMax = true
		}
		return isMax
	}
	...
	function canSwipe() {
		const p = getCameraParam()
		if (!inSwipeMode() || $gameMap.isEventRunning() || $gameMap._noSwipeModeKe) {
			return false
		}
		if (p.duration || inTouchLayerPressDsms()) {
			return false
		}
		return true
	}
	...
	function inTouchLayerPressDsms() {
		const gt = $gameTemp
		if (typeof gt.isPressedTouchLayerKeDsms != 'function') {
			return false
		}
		...
		for (const id of idList) {
			...
			if (id == 0) {
				self =
					self ||
					getPlcmEvent() ||
					(charaSpritePlcm ? charaSpritePlcm._character : null) ||
					$gamePlayer
	...
	function isSameChara(a, b) {
		...
		if (a._isSwipeDummyKe && !b._isSwipeDummyKe) {
			return false
		}
		if (b._isSwipeDummyKe && !a._isSwipeDummyKe) {
			return false
		}
	...
	Game_InterpreterStartingKeAcms.prototype.setupChild = function (list, eventId) {
		if (!this._childInterpreter) {
			this._childInterpreter = new Game_InterpreterStartingKeAcms(this._depth + 1)
		}
