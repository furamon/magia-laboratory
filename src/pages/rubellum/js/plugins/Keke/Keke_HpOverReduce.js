...
;(() => {
	...
	BattleManager.endBattle = function (result) {
		...
		$gameParty.allMembers().forEach((actor) => {
			...
			// 最大HP削減の消去
			let delReduce =
				actor._delMhpReduceKe != null ? actor._delMhpReduceKe : keke_delMhpReduceAtBattleEnd
			if (delReduce) {
				delMhpReduce(actor)
			}
			// リフレッシュ
	...
	function applyHpOver(subject, newHp) {
		...
		if (!subject._hpOverNowKe) {
			subject._hpOverNowKe = { num: 0 }
		}
		...
		if (max >= 0) {
			subject._hp = Math.min(subject._hp, Math.round(subject.mhp * max))
		}
		if (d.ignore) {
			subject._reduceMhpKe = reduceMhp
		}
		// 超過数を加算
		n.num++
		// 超過データ数を保存
	...
	function applyMhpReduce(subject, target, value) {
		...
		// 削減セット
		if (target._reduceMhpKe == null) {
			target._reduceMhpKe = target.mhp
		}
	...
	Game_BattlerBase.prototype.param = function (paramId) {
		...
		if (paramId == 0) {
			val = this._reduceMhpKe != null ? this._reduceMhpKe : val
		}
		return val
	}
