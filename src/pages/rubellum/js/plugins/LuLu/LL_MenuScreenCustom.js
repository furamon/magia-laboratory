...
;(() => {
	...
	Scene_MenuBase.prototype.createPageButtons = function () {
		...
		if (leftInputMode) {
			this._pageupButton.x =
				Graphics.boxWidth - this._pageupButton.width - this._pagedownButton.width - 8
		} else {
	...
	Window_StatusBase.prototype.exDrawActorSimpleStatus = function (actor, x, y, width) {
		...
		if (actorNameLH > -1)
			this.drawActorName(actor, x + actorNameX, y + lineHeight * actorNameLH, width - actorNameX)
		if (actorLevelLH > -1)
			this.drawActorLevel(actor, x + actorLevelX, y + lineHeight * actorLevelLH)
		if (actorIconLH > -1)
			this.drawActorIcons(actor, x + actorIconX, y + lineHeight * actorIconLH, width - actorIconX)
		if (actorClassLH > -1)
			this.drawActorClass(
				actor,
				x + actorClassX,
		...
		if (actorGaugeLH > -1)
			this.placeBasicGauges(actor, x + actorGaugeX, y + lineHeight * actorGaugeLH)
	}
	...
	Window_MenuStatus.prototype.drawItemImage = function (index) {
		...
		if (mPicture && showStandingPicture) {
			...
			// ピンチ判定
			if (
				ExMenuScreenBase.getHpRate(actor.actorId()) > Number(mPicture.pinchPercentage) ||
	...
	Window_MenuHelp.prototype.drawLeftBlock = function () {
		...
		this.drawText(
			...
			this.contents.measureTextWidth(leftBlockLabel),
			y,
			oneThirdWidth * 1.5 - this.contents.measureTextWidth(leftBlockLabel),
			leftBlockAlign
		)
	}
	...
	Window_MenuHelp.prototype.drawRightBlock = function () {
		...
		this.drawText(
			...
			this.contents.measureTextWidth(rightBlockLabel) + oneThirdWidth * 1.5,
			y,
			oneThirdWidth * 1.5 - this.contents.measureTextWidth(rightBlockLabel),
			rightBlockAlign
		)
	}
	...
	Window_MenuHelp.prototype.drawCurrency = function () {
		...
		if (currencyWindowPosition == 'helpWindowRightBottom') {
			...
			this.drawText(
				...
				this.contents.measureTextWidth(rightBottomBlockLabel) + oneThirdWidth * 1.5,
				y,
				oneThirdWidth * 1.5 - this.contents.measureTextWidth(rightBottomBlockLabel),
				rightBottomBlockAlign
			)
		}
	}
	...
	Window_MenuHelp.prototype.drawMenuHelp = function () {
		...
		if (leftBottomBlockLabel != '') {
			...
			this.drawText(
				...
				this.contents.measureTextWidth(leftBottomBlockLabel),
				y,
				oneThirdWidth * 1.5 - this.contents.measureTextWidth(leftBottomBlockLabel),
				leftBottomBlockAlign
			)
		} else {
			// メニューヘルプを表示
