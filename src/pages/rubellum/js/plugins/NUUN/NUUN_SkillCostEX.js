...
;(() => {
	...
	Game_BattlerBase.prototype.skillMpCost = function (skill) {
		...
		cost += skill.meta.SkillMPCostMR
			? Math.floor((this.mmp * Number(skill.meta.SkillMPCostMR)) / 100)
			: 0
		cost += skill.meta.SkillMPCostR
			? Math.floor((this._mp * Number(skill.meta.SkillMPCostR)) / 100)
			: 0
	...
	Game_BattlerBase.prototype.skillHpCost = function (skill) {
		...
		cost += skill.meta.SkillHPCostMR
			? Math.floor((this.mhp * Number(skill.meta.SkillHPCostMR)) / 100)
			: 0
		cost += skill.meta.SkillHPCostR
			? Math.floor((this._hp * Number(skill.meta.SkillHPCostR)) / 100)
			: 0
		return cost
	}
	...
	Game_BattlerBase.prototype.skillTpCost = function (skill) {
		...
		cost += skill.meta.SkillTPCostR
			? Math.floor((this._tp * Number(skill.meta.SkillTPCostR)) / 100)
			: 0
		return cost
	}
	...
	Game_BattlerBase.prototype.skillGoldCost = function (skill) {
		...
		if (!this.isEnemy()) {
			...
			cost += skill.meta.SkillGoldCostR
				? Math.floor(($gameParty.gold() * Number(skill.meta.SkillGoldCostR)) / 100)
				: 0
		}
		return cost
	}
	...
	Game_BattlerBase.prototype.skillExpCost = function (skill) {
		...
		if (this.isActor()) {
			...
			cost += skill.meta.SkillExpCostR
				? ((this.currentExp() - this.currentLevelExp()) * Number(skill.meta.SkillExpCostR)) / 100
				: 0
		}
		return cost
	}
	...
	Game_BattlerBase.prototype.canSkillVarCostR = function (skill) {
		...
		if (cost) {
			if (cost[1] > 0) {
				...
				return (
					$gameVariables.value(cost[0]) >=
					Math.floor(($gameVariables.value(cost[0]) * cost[2]) / 100)
				)
			}
		}
		return true
			...
			while (true) {
				...
				if (match) {
					...
					switch (data[0]) {
						case 'I':
							list.push({
								item: $dataItems[parseInt(data[1])],
						...
						case 'W':
							list.push({
								item: $dataWeapons[parseInt(data[1])],
								quantity: parseInt(data[2]),
						...
						case 'A':
							list.push({
								item: $dataArmors[parseInt(data[1])],
								quantity: parseInt(data[2]),
								mode: parseInt(data[3])
							})
							break
