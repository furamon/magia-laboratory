...
;(() => {
	...
	const PopUpAdvantageousState =
		NUUN_Base_Ver >= 113
			? DataManager.nuun_structureData(parameters['PopUpAdvantageousState'])
			: null
	const PopUpBadState =
		NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['PopUpBadState']) : null
	const AppliBuffPopUp =
		(NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['AppliBuffPopUp']) : null) ||
		[]
	const PopUpBuff =
		NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['PopUpBuff']) : null
	const PopUpDebuff =
		NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['PopUpDebuff']) : null
	const DefeatShowPopup = eval(parameters['DefeatShowPopup'] || 'false')
	const PopUpDefeat =
		NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['PopUpDefeat']) : null
	const StealShowPopup = eval(parameters['StealShowPopup'] || 'false')
	const PopUpSteal =
		NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['PopUpSteal']) : null
	const PopUpGoldSteal =
		NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['PopUpGoldSteal']) : null
	const SkillLearningShowPopup = eval(parameters['SkillLearningShowPopup'] || 'false')
	const PopUpSkillLearning =
		NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['PopUpSkillLearning']) : null
	...
	Game_Battler.prototype.popupBuffIconIndex = function (id) {
		if (id < 10) {
			...
			return (
				Game_BattlerBase.ICON_DEBUFF_START + Math.max(-this._buffs[id - 10] - 1, 0) * 8 + id - 10
			)
		}
	}
		...
		for (const state of states) {
			if (this.displayDeadPopup(target, state) && !state.meta.NoPopUp && !state.meta.AddNoPopUp) {
				...
				if (data) {
					...
					const img = this.isBadState(state)
						? state.meta.BadStatePopupImg
						: state.meta.StatePopupImg
					this.setupPopup(
						target,
						data,
						state.id,
						data.PopUpText.format(name),
						color,
						iconIndex,
						255,
						'State',
			...
			if (
				...
				if (data) {
					...
					this.setupPopup(
						...
						data,
						state.id,
						data.RemovePopUpText.format(name),
						color,
						iconIndex,
						PopUpReleaseOpacity,
						'RState',
			...
			for (const paramId of buffs) {
				...
				if (find) {
					...
					if (data && (find.BuffPopUpMode === 0 || find.BuffPopUpMode === 3)) {
						...
						const iconIndex =
							data.PopupIconIndex > 0 ? data.PopupIconIndex : target.popupBuffIconIndex(id)
						this.setupPopup(
							target,
							data,
							id,
			...
			for (const paramId of buffs) {
				...
				if (find) {
					...
					if (data && (find.StatePopUpMode === 0 || find.StatePopUpMode === 2)) {
						...
						const name = find.PopUpName ? find.PopUpName : TextManager.param(paramId)
						const iconIndex =
							data.PopupIconIndex > 0 ? data.PopupIconIndex : target.removePopupBuffIconIndex(id)
						this.setupPopup(
							target,
							data,
							id,
		...
		if (SkillLearningShowPopup) {
			...
			if (data) {
				for (const skill of skills) {
					...
					const iconIndex =
						data.PopupIconIndex > 0 ? data.PopupIconIndex : $dataSkills[skill.id].iconIndex
	...
	Window_BattleLog.prototype.setupPopup = function (
		target,
		data,
		id,
		format,
	...
	Sprite_PopUpEX.prototype.fontFace = function () {
		return this.popupData && this.popupData.fontFace
			? this.popupData.fontFace
		...
		if (result && $gameParty.inBattle()) {
			this.iterateActorEx(params[0], params[1], (actor) => {
				...
				const sprite = SceneManager._scene._spriteset
					.battlerSprites()
					.find((sprite) => sprite._battler === actor)
				if (sprite) {
	...
	Game_Interpreter.prototype.command333 = function (params) {
		...
		if (result && $gameParty.inBattle()) {
			this.iterateActorEx(params[0], params[1], (enemy) => {
				...
				const sprite = SceneManager._scene._spriteset
					.battlerSprites()
					.find((sprite) => sprite._battler === enemy)
				if (sprite) {
					sprite.updateDamagePopup()
				}
				enemy.clearResult()
	...
	function _setupStatePopUpColor(state, data) {
		return state.meta.PopUpColor
			? isNaN(Number(state.meta.PopUpColor))
