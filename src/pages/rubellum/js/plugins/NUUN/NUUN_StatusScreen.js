...
;(() => {
	...
	const PageList =
		(NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['PageList']) : null) || []
	const ParamList_1Page =
		(NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['ParamList_1Page']) : null) ||
		[]
	const ParamList_2Page =
		(NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['ParamList_2Page']) : null) ||
		[]
	const ParamList_3Page =
		(NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['ParamList_3Page']) : null) ||
		[]
	const ParamList_4Page =
		(NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['ParamList_4Page']) : null) ||
		[]
	const ParamList_5Page =
		(NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['ParamList_5Page']) : null) ||
		[]
	const BackGroundImg =
		NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['BackGroundImg'])[0] : null
	...
	const ElementResist =
		NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['ElementResist']) : []
	const StateResist =
		NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['StateResist']) : []
	...
	const ElementRadarChartFramecolor =
		DataManager.nuun_structureData(parameters['ElementRadarChartFramecolor']) || 0
	const ElementRadarChartLineColor =
		DataManager.nuun_structureData(parameters['ElementRadarChartLineColor']) || 0
	const ElementRadarChartMainColor1 =
		DataManager.nuun_structureData(parameters['ElementRadarChartMainColor1']) || 0
	const ElementRadarChartMainColor2 =
		DataManager.nuun_structureData(parameters['ElementRadarChartMainColor2']) || 0
	...
	const StateRadarChartFramecolor =
		DataManager.nuun_structureData(parameters['StateRadarChartFramecolor']) || 0
	const StateRadarChartLineColor =
		DataManager.nuun_structureData(parameters['StateRadarChartFramecolor']) || 0
	const StateRadarChartMainColor1 =
		DataManager.nuun_structureData(parameters['StateRadarChartMainColor1']) || 0
	const StateRadarChartMainColor2 =
		DataManager.nuun_structureData(parameters['StateRadarChartMainColor2']) || 0
	...
	Window_Status.prototype.nuunStatusDrawBlock = function () {
		...
		for (const data of list) {
			...
			const width =
				data.ItemWidth && data.ItemWidth > 0 ? data.ItemWidth : this.widthMode(data.WideMode, rect)
	...
	Window_Status.prototype.nuunStatusDrawActorIcons = function (actor, x, y, width, list) {
		...
		if (dataEval) {
			...
			icons = actor
				.allIcons()
				.filter((icon) => states.some((id) => $dataStates[id].iconIndex === icon))
	...
	Window_Status.prototype.drawEquip = function (list, actor, x, y, width) {
		...
		for (let i = 0; i < e1uipsLength; i++) {
			...
			if (
				slotName &&
				this.isShowSlot(actor, index) &&
				(!showEquips || (showEquips && showEquips.some((data) => data === slotName)))
			) {
				let sw = 0
				let iconWidth = 0
				if (list.Back) {
	...
	Sprite_StatusExpGauge.prototype.drawValue = function () {
		...
		if (ExpPercent) {
			currentValue = this._battler.isMaxLevel()
				? '100%'
				: NuunManager.numPercentage(this.currentPercent(), EXPDecimal, DecimalMode) + '%'
		} else {
