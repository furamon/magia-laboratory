...
;(() => {
	...
	const ActorCommand =
		(NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['ActorCommand']) : null) || []
					...
					switch (key) {
						...
						case 'escape':
							...
							this.addCommand(
								text,
								key,
								data.CommandEval ? eval(data.CommandEval) : BattleManager.canEscape()
							)
							break
						case 'attack':
							if (dataFunc) {
							...
							this.addCommand(
								text,
								'attack',
								data.CommandEval ? eval(data.CommandEval) : this._actor.canAttack()
							)
							break
						case 'skill':
							if (dataFunc) {
						...
						case 'guard':
							...
							this.addCommand(
								TextManager.guard,
								'guard',
								data.CommandEval ? eval(data.CommandEval) : this._actor.canGuard()
							)
							break
						case 'item':
							if (dataFunc) {
