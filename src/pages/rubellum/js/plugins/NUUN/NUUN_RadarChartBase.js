...
;(() => {
	...
	Sprite_NUUN_RadarChart.prototype.setup = function (
		battler,
		chartType,
		data,
		radius,
	...
	Sprite_NUUN_RadarChart.prototype.setupColor = function (
		framecolor,
		lineColor,
		mainColor1,
		mainColor2
	...
	Sprite_NUUN_RadarChart.prototype.drawRadarChartMain = function (data, x, y, width, radius) {
		...
		data.forEach((a, i) => {
			...
			dataPoint.push(
				new Point(this.chartStatusPointX(x, angle, rate), this.chartStatusPointY(y, angle, rate))
			)
		})
	...
	Sprite_NUUN_RadarChart.prototype.drawRadarChartFrame = function (data, x, y, width, radius) {
		...
		data.forEach((a, i) => {
			const angle = this.getAngle(i, data)
			dataPoint.push(
				new Point(this.chartFramePointX(x, radius, angle), this.chartFramePointY(y, radius, angle))
			)
		})
	...
	Sprite_NUUN_RadarChart.prototype.drawRadarChartLine = function (data, x, y, width, radius) {
		data.forEach((a, i) => {
			const angle = this.getAngle(i, data)
			let point = new Point(
				this.chartFramePointX(x, radius, angle),
				this.chartFramePointY(y, radius, angle)
			)
			this.drawLine(point.x, point.y, width, this._lineColor)
			point = new Point(
				this.chartFramePointX(x, radius * 1.22, angle),
				this.chartFramePointY(y, radius * 1.22, angle)
			)
