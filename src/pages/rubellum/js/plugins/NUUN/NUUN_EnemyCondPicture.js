...
;(() => {
	...
	const EnemyCondPictureData =
		NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['EnemyCondPictureData']) : []
	const CommonEnemyImg =
		NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['CommonEnemyImg']) : []
	...
	Game_Enemy.prototype.battlerHue = function () {
		return !isNaN(this._battleStyleGraphicHue)
			? this._battleStyleGraphicHue
			: _Game_Enemy_battlerHue.call(this)
	}
	...
	Sprite_Enemy.prototype.setEnemyGraphic = function (enemy, bitmap) {
		...
		if (name && this.addApngChild && this.loadApngSprite(name)) {
			...
			if (this.isDead()) {
				this.revertToNormal()
				this._enemyImgesOpacity = this.isEnemyGraphicDead()
					? this.opacity - enemy.getBattleStyleOpacity()
					: this.opacity - 0
