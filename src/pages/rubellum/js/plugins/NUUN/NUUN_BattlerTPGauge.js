...
;(() => {
	...
	Sprite_Enemy.prototype.setTpGaugePosition = function () {
		if (this._battlerTp) {
			...
			const x =
				(enemy.meta.TPGaugeX ? Number(enemy.meta.TPGaugeX) : 0) +
				Gauge_X +
				this._enemy.getTpGaugePositionX()
			const y =
				(enemy.meta.TPGaugeY ? Number(enemy.meta.TPGaugeY) : 0) +
				Gauge_Y +
				this._enemy.getTpGaugePositionY()
			this._battlerTp.x = x
