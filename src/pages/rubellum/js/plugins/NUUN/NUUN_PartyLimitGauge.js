...
;(() => {
	...
	const PartyGauge_LabelColor =
		DataManager.nuun_structureData(parameters['PartyGauge_LabelColor']) || 16
	...
	const EnemyGauge_LabelColor =
		DataManager.nuun_structureData(parameters['EnemyGauge_LabelColor']) || 16
	const DamageAmount =
		NuunManager.getEvalCode(parameters['DamageAmount']) || 'Math.floor(25 * damage / a.mhp)'
	...
	Game_Battler.prototype.chargeLimitByAttack = function (item, critical) {
		if (critical) {
			evalStr =
				item && item.meta.LimitCriticalStr ? String(item.meta.LimitCriticalStr) : CriticalAmount
		} else {
	...
	if (!Imported.NUUN_SkillCostShowEX) {
		...
		Window_SkillList.prototype.drawItem = function (index) {
			...
			if (skill) {
				...
				this.multiCost =
					cost > 0 && (this._actor.skillTpCost(skill) > 0 || this._actor.skillMpCost(skill) > 0)
	...
	Sprite_Gauge.prototype.drawValue = function () {
		if (this._statusType === 'limit') {
			if (
				(this._unit === 'actor' && PartyLimitValueVisible) ||
				(this._unit === 'enemy' && EnemyLimitValueVisible)
			) {
	...
	Sprite_PartyGauge.prototype.gaugeColor1 = function () {
		return this._isGaugeData
			? getColorCode(this.changeGaugeColor1())
			: getColorCode(PartyGaugeColor1)
	}

	Sprite_PartyGauge.prototype.gaugeColor2 = function () {
		return this._isGaugeData
			? getColorCode(this.changeGaugeColor2())
			: getColorCode(PartyGaugeColor2)
	}
