...
;(() => {
	...
	const SupportActorSV =
		(NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['SupportActorSV']) : null) ||
		[]
	...
	Game_Actor.prototype.supportActorDeathCallActor = function () {
		if (this._supportActorDeadCallActor) {
			if (
				this._supportActorCallActor > 0 &&
				$gameActors.actor(this.getSupportActorCallActor()).isDead()
			) {
				this.removeSupportActor()
			}
		}
	...
	Game_Party.prototype.membersInSupportActorNum = function (members) {
		//メンバー内のサポートアクター人数
	...
	Game_Party.prototype.normalOnlyBattleMembers = function (members) {
		...
		const concatMembers = this.allMembers()
	...
	Game_Party.prototype.supportActorMembers = function () {
		//全てのサポートアクターを取得
		return this.allMembers().filter((member) => member.getSupportActor())
	}

	Game_Party.prototype.supportActorWithinMembers = function () {
		//バトルメンバー内のサポートアクターを取得
		return this.allMembers()
			.slice(0, this.maxBattleMembers())
			.filter((member) => member.getSupportActor())
	}
