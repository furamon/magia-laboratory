...
;(() => {
	...
	Game_TpbCooldowns.prototype._itemKey = function (usableItem) {
		return usableItem ? usableItem.meta.CooldownType || usableItem.id : null
	}
	...
	Game_Battler.prototype.tpbCooldownContainer = function (usableItem) {
		...
		return usableItem.meta.PartyCooldown ? this.friendsUnit().tpbCooldowns() : this._tpbCooldowns
	}
	...
	Game_Battler.prototype.updateTpb = function () {
		...
		if (this._tpbCooldowns && (this.isAlive() || param_DeadCooldownType === 'active')) {
	...
	Game_Actor.prototype.onBattleStartCooldowns = function (isGroup = false) {
		this.skills().forEach((skill) => {
			...
			if (startTime > 0 && !!skill.meta.PartyCooldown === isGroup) {
	...
	const windowCooldownModule = (obj) => {
		...
		obj.isEnabled = function (item) {
			return (
				_base_isEnabled.apply(this, arguments) && this._actor && !this._actor.isTpbCooldown(item)
			)
		}
