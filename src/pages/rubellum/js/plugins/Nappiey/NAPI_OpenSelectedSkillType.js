...
;(() => {
	...
	Game_Interpreter.prototype.pluginCommand = function (command, args) {
		...
		if (command === 'OpenSelectedSkillType' || command === '指定スキルタイプメニュー') {
			...
			pluginArgs.forEach((e) => {
				if (e[0].toLowerCase() === 'actor' || e[0] === 'アクターID') {
					NAPI.osstActorId = Number(e[1])
				}
				if (e[0].toLowerCase() === 'member' || e[0] === 'メンバーIndex') {
					NAPI.osstActorId = $gameParty.allMembers()[Number(e[1])].actorId()
				}
				if (e[0].toLowerCase() === 'type' || e[0] === 'スキルタイプID') {
					NAPI.osstSkillTypeId = Number(e[1])
				}
				if (e[0].toLowerCase() === 'typeindex' || e[0] === 'スキルタイプIndex') {
					NAPI.osstSkillTypeIndex = Number(e[1])
				}
			})
			NAPI.OpenSelectedSkillType()
		}
	...
	if (Utils.RPGMAKER_NAME === 'MZ') {
		PluginManager.registerCommand(pluginName, 'OpenSelectedSkillType', (args) => {
			...
			if (Number(args.actorId)) {
				NAPI.osstActorId = Number(args.actorId)
			}
