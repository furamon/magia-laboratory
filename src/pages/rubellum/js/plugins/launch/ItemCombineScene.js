...
;(() => {
	...
	const execCombine = (materialNum, candidateNum) => {
		...
		const result = metaToItem(typeOfNum(candidateNum), candidate.meta[material.meta.Material])
		if (!result) {
			return null
		}
	...
	const debugOutput = () => {
		...
		for (const materialItemId of Object.keys(materials)) {
			...
			for (const objItemId of candidates[materialItemId]) {
				...
				if (objItem) {
					combineItem = metaToItem(typeOfNum(objItemId), objItem.meta[materialItem.meta.Material])
					if (combineItem) {
						number++
	...
	Scene_ItemCombine.prototype.update = function () {
		...
		this._navigationWindow.setPhase(this.phase, this._candidateWindow.itemNum())
	}
	...
	Scene_ItemCombine.prototype.createCandidateWindow = function () {
		...
		this._candidateWindow.setHandler('cancel', this.onCandidateCancel.bind(this))
	...
	Scene_ItemCombine.prototype.processAllWindows = function () {
		this._materialWindow.refresh()
		this._candidateWindow._catEnabled = this._materialWindow.isCurrentItemEnabled()
		this._candidateWindow.deselect()
	...
	Window_Candidate.prototype.includes = function (item) {
		...
		return (
			...
			Object.keys(candidates[this._category]).includes(itemToNum(typeOfItem(item), item.id))
		)
	}
