...
;(() => {
	...
	const strBattleVoice = parameters['Battle Voice Name at Option'] || 'Battle Voice'
	...
	PluginManager.registerCommand(pluginName, 'stateChange', (args) => {
		const voiceToAdd = join2voices(args.voiceAtStateAdded1, args.voiceAtStateAdded2)
		const voiceToRemove = join2voices(args.voiceAtStateRemoved1, args.voiceAtStateRemoved2)
	...
	const playEnemyVoice = (enemy, type) => {
		...
		let ev
		const voices = (ev = $gameSystem.enemyVoices) ? ev[enemy.enemyId()] || {} : {}
	...
	const _voiceFileName = (setVoice, battlerMeta, type) => {
		...
		switch (type) {
			...
			case 'recover':
				...
				break
			case 'friendmagic':
				name = split(a.friendMagic || m.friendMagicVoice || a.magic || m.magicVoice)
				break
			case 'magic':
	...
	const normalFirstVoice = () => {
		if (!BattleManager._surprise) {
			const candidates = $gameParty.aliveBattleMembers().filter((actor) => {
				return (
					actor.actor().meta.firstVoice != null || (actor.battleVoices && actor.battleVoices.first)
				)
			})
			if (candidates.length > 0) {
				return { type: 'first', battlers: candidates }
	...
	BattleManager.processVictory = function () {
		const candidates = $gameParty.aliveMembers().filter((actor) => {
			return actor.actor().meta.victoryVoice || (actor.battleVoices && actor.battleVoices.victory)
		})
		if (candidates.length > 0) {
	...
	Scene_Battle.prototype.commandEscape = function () {
		const candidates = $gameParty.aliveMembers().filter((actor) => {
			return actor.actor().meta.runawayVoice || (actor.battleVoices && actor.battleVoices.runaway)
		})
		if (candidates.length > 0) {
			const index = Math.randomInt(candidates.length)
			const actor = candidates[index]
