...
;(() => {
	...
	//
	const realMaxColsMenuWnd = () => (maxColsMenuWnd ? maxColsMenuWnd : $gameParty.members().length)
	...
	Sprite_Gauge.prototype.bitmapWidth = function () {
		return this.isSceneNeedsAdjustGauge()
	...
	const _Window_SkillStatus_createInner = Window_SkillStatus.prototype.createInnerSprite
	...
	Sprite_Name.prototype.isSceneNeedsAdjustGauge = Sprite_Gauge.prototype.isSceneNeedsAdjustGauge =
		function () {
			const scene = SceneManager._scene.constructor
			return [Scene_Menu, Scene_Item, Scene_Skill].includes(scene) && !this.defaultGauge
		}
	...
	Window_MenuStatus.prototype.drawItemImage = function (index) {
		...
		if (bitmap) {
			...
			const dx = bitmap.width > rect.width ? rect.x : rect.x + (rect.width - bitmap.width) / 2
			const dy = bitmap.height > rect.height ? rect.y : rect.y + (rect.height - bitmap.height) / 2
	...
	Window_StatusBase.prototype.drawActorHpMV = function (actor, x, y, width) {
		...
		this.drawCurrentAndMaxMV(actor.hp, actor.mhp, x, y, width, hpColor, normalColor)
	}
	...
	Window_StatusBase.prototype.drawActorMpMV = function (actor, x, y, width) {
		...
		this.drawCurrentAndMaxMV(actor.mp, actor.mmp, x, y, width, mpColor, normalColor)
	}
	...
	Window_Base.prototype.drawCurrentAndMaxMV = function (current, max, x, y, width, color1, color2) {
		const labelWidth = this.textWidth(Math.max(TextManager.hpA, TextManager.mpA))
	...
	Window_MenuCommand.prototype.cursorDown = function (wrap) {
		if (wrap) {
			...
			if (maxItems % maxCols !== 0 && nextIndex >= maxItems && index < maxItems) {
				if (index === maxItems - 1) {
	...
	Window_MenuCommand.prototype.cursorUp = function (wrap) {
		if (wrap) {
			...
			const maxCols = this.maxCols()
			const nextIndex = index - maxCols
			if (maxItems % maxCols !== 0 && nextIndex < 0 && index < maxCols) {
				const indexLeftDown = Math.floor((maxItems - 1) / maxCols) * maxCols
				const newIndex = Math.min(indexLeftDown + nextIndex.mod(maxCols), maxItems - 1)
