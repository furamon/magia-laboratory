...
;(() => {
	...
	Scene_Menu.prototype.refreshSubMemberWindow = function () {
		...
		if (numLines > 0) {
			...
			this._subMemberWindow.y =
				Graphics.boxHeight - this._goldWindow.height - this._subMemberWindow.height
	...
	//
	const subMemberVarList = [submemberVar1, submemberVar2, submemberVar3, submemberVar4]
	let subMembers = []
	...
	Game_Follower.prototype.actor = function () {
		...
		if (
			hasFooter() &&
			this._memberIndex === Math.min(followerMaxSize(), numDispActor() + numSubMembers() + 1)
		) {
		...
		if (DisplayOnMap && this._memberIndex <= (lastId += numSubMembers())) {
			return subMemberPosition(this._memberIndex + numSubMembers() - lastId - 1)
		}
		return null
	}
})()
