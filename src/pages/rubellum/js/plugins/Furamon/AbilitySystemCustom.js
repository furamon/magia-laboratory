...
@default {"StatusAbilityWindowWidth": "300", "StatusAbilityWindowHeight": "200","EquipAbilitiesWindowColumns": "1"}
@desc
...
@param EquipAbilitiesWindowColumns
@text 装備アビリティ欄列数
@type number
@default 1
@desc
...
@param HasAbilitiesWindowColumns
@text 所持アビリティ欄列数
@type number
@default 2
@desc
...
@text ウィンドウサイズ
@type struct<WindowSize>
@default {"StatusAbilityWindowWidth":"300","StatusAbilityWindowHeight":"200","EquipAbilitiesWindowColumns":"1","HasAbilitiesWindowColumns":"2"}
@desc
各種ウィンドウのサイズを設定します。
...
const AbilitySystemClassAlias = (() => {
	...
	const FACE_HEIGHT_WHEN_ENABLE_CONST = 120
	...
	class Window_AbilitiesBase extends Window_Selectable {
		...
		costWidth() {
			if (EnableCost) return this.textWidth('00')
			return 0
		}
	...
	class Window_EquipAbilities extends Window_AbilitiesBase {
		...
		maxCols() {
			return WindowSize.EquipAbilitiesWindowColumns
		}
	}
	...
	class Window_HasAbilities extends Window_AbilitiesBase {
		...
		maxCols() {
			return WindowSize.HasAbilitiesWindowColumns
		}
	...
	class Window_StatusAbility extends Window_StatusBase {
		...
		drawActorFace() {
			...
			super.drawActorFace(
				this._actor,
				this.width - ImageManager.faceWidth - 20,
				faceY + 12,
		...
		drawActorName() {
			const nameRect = this.itemLineRect(0)
			nameRect.width = this.width - this.padding * 2
			super.drawActorName(this._actor, 12, 6, nameRect.width)
			// 追加：多重職業のレベルを表示
			this.drawText(this._actor.currentAdditionalClass(0).name, nameRect.x + 110, 6, 360)
		}
		...
		drawActorTotalCost() {
			...
			this.drawText(costText, x + 100, y, width, 'left')
		}
	...
	Game_Actor.prototype.findNewSkills = function (
		lastSkills,
		opt = { includeHasAbilitySkills: false }
	) {
		const newSkills = this.skills({
			includeHasAbilitySkills: opt.includeHasAbilitySkills
		})
	...
	Window_MenuCommand.prototype.addOriginalCommands = function () {
		...
		if (Text.MenuAbilitySystemText !== '')
			this.addCommand(Text.MenuAbilitySystemText, 'ability', this.isEnabledAbilityCommand())
	}
