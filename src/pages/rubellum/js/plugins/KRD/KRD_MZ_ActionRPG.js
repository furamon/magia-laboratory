...
;(() => {
	...
	Scene_Map.prototype.createMapPlayerSprite = function () {
		...
		const characterSprites = SceneManager._scene._spriteset._characterSprites
		const index = characterSprites.findIndex(
			(cs) => cs._character.constructor.name === 'Game_Player'
		)
		const cs = characterSprites[index]
	...
	Scene_Map.prototype.createMapFollowerSprite = function () {
		...
		if ($gamePlayer.followers().isVisible()) {
			const characterSprites = SceneManager._scene._spriteset._characterSprites
			const csIndexList = characterSprites
				.map((cs, index) => {
	...
	Game_Temp.prototype.moveForward = function (waitMode, eventId) {
		...
		if (character) {
			const moveRoute = {
				list: [{ code: Game_Character.ROUTE_MOVE_FORWARD, indent: null }, { code: 0 }],
				repeat: false,
				skippable: false,
				wait: !!waitMode
			}
	...
	Game_Interpreter.prototype.anyCollision = function () {
		...
		return enemyBall
			? this.playerCollision(skillId, eventId)
			: this.enemyCollision(skillId, eventId)
	}
	...
	Game_Interpreter.prototype.enemyCollision = function (skillId, eventId) {
		...
		const key = SELF_AFTER_ENEMY_DAMAGE
			? [$gameMap.mapId(), targetId, SELF_AFTER_ENEMY_DAMAGE]
			: null
	...
	Game_Party.prototype.canMapDamagePopup = function () {
		return (
			!$gameParty.inBattle() && (this.isAlwaysPopup() || (USE_DAMAGE_POPUP && this.inMapBattle()))
		)
	}
