...
;(() => {
	...
	AudioManager.updateBgmParameters = function (bgm) {
		if (bgm) {
			const percent =
				SPECIAL_DIRECTORY && bgm.name.match(SPECIAL_DIRECTORY) ? SPECIAL_VOLUME : PERCENT_BGM
	...
	AudioManager.updateBgsParameters = function (bgs) {
		if (bgs) {
			const percent =
				SPECIAL_DIRECTORY && bgs.name.match(SPECIAL_DIRECTORY) ? SPECIAL_VOLUME : PERCENT_BGS
	...
	AudioManager.updateMeParameters = function (me) {
		if (me) {
			const percent =
				SPECIAL_DIRECTORY && me.name.match(SPECIAL_DIRECTORY) ? SPECIAL_VOLUME : PERCENT_ME
	...
	AudioManager.updateSeParameters = function (buffer, se) {
		if (se) {
			const percent =
				SPECIAL_DIRECTORY && se.name.match(SPECIAL_DIRECTORY) ? SPECIAL_VOLUME : PERCENT_SE
