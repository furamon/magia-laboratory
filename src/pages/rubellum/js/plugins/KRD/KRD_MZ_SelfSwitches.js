...
;(() => {
	...
	Game_Temp.prototype.setSelfSwitchesSomeMap = function (alphabet, tag, value, argMapIdList) {
		...
		for (const paramMapId of paramMapIdList) {
			const data = $gameSystem._eventIdObjectList.find(
				(e) => e.mapId === Number(paramMapId) && e.tag === tag
			)
	...
	Game_Temp.prototype.eventIdList = function (tag) {
		return $gameMap
			.events()
			.filter((event) => event.event().meta[tag])
			.map((e) => e.eventId())
	}
	...
	Scene_Map.prototype.setEventIdObjectList = function () {
		$gameSystem._eventIdObjectList = $gameSystem._eventIdObjectList
			? $gameSystem._eventIdObjectList
			: []

		for (const tag of MAP_TAG_LIST) {
			if (
				!$gameSystem._eventIdObjectList.some((e) => e.mapId === $gameMap.mapId() && e.tag === tag)
			) {
