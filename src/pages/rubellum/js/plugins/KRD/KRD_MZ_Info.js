	...
	class Scene_InfoBase extends Scene_MenuBase {
		...
		canInfo(item, show, subSymbol) {
			return (
				show &&
				item &&
				!item.meta?.KRD_notData &&
				item.name !== '' &&
				!this.getDbJsonOneData(subSymbol, item.id, 'notData')
			)
		}
	...
	class Window_InfoText extends Window_InfoTextBase {
		drawInfo(symbol, index, i) {
			...
			if (symbolIndex !== '' && i >= 0) {
				...
				const found = KRD_INFO.command[i].data.find(
					(data, index) => symbolIndex === subSymbol + index
				)
		...
		drawInfoText(found, x = 0, y = DOWN_LETTER) {
			if (found) {
				...
				if (name4draw && alphabet) {
					...
					this.drawTextSeparate(
						text4draw,
						x,
						y + BETWEEM_TITLE_TO_TEXT,
						this.innerWidth,
						alphabet[0],
						alphabet[1]
					)
		...
		drawEnemyData2Cols(found, x = 0, y = DOWN_LETTER) {
			...
			ENEMY_PARAMS.forEach((paramIndex, i) => {
				if (i % 2 === 0) {
					this.drawTextExFontSize(
						TextManager.param(paramIndex) + ' ' + enemy.param(paramIndex),
						x,
						y,
						width
					)
					...
					if (data) {
						...
						if (i % 2 === 0) {
							...
							if (data[drop.dataId].iconIndex > 0) {
								...
								this.drawTextExFontSize(
									itemName,
									x + xx + iconPlusX + ImageManager.iconWidth,
									y,
									width
								)
							} else {
			...
			if (found) {
				...
				if (data) {
					...
					this.drawTextExFontSize(
						text4draw,
						x,
						y + this.lineHeight() + BETWEEM_TITLE_TO_TEXT,
						this.innerWidth
					)
				}
			}
