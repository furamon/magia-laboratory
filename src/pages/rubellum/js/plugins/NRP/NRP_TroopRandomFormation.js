...
;(function () {
	...
	Sprite_Battler.prototype.makeAutoPositionWithBruteForce = function (
		...
		for (let gridData of gridList) {
			const distanceData = makeDistanceData(this, gridData.x, gridData.y, sprites, bestDistance)
	...
	Sprite_Battler.prototype.makeAutoPositionAtRandom = function (
		...
		for (let i = 0; i < pMaxTryNoForEnemy; i++) {
			...
			const distanceData = makeDistanceData(this, randomX, randomY, sprites, bestDistance)
	...
	function getDistanceValue(a, b) {
		...
		if (distanceX < 0 && distanceY < 0) {
			...
			// ■または、bの上半分が隠れていない場合
			if ((ay <= bCenterY && aCenterY <= bHeadY) || (by <= aCenterY && bCenterY <= aHeadY)) {
