...
;(function () {
	...
	Spriteset_Base.prototype.isAnimationPlaying = function () {
		...
		if (
			...
			this._animationSprites.every(
				(sprite) => sprite.dynamicAnimation && sprite.dynamicAnimation.isNoWaitInAction()
			)
		) {
			return false
		}
	...
	Sprite_Battler.prototype.isAnimationRequestWait = function () {
		...
		if (this.isAnimationRequested()) {
			...
			const noWait = this.animations().every(
				(animation) => animation.dynamicAnimation && animation.dynamicAnimation.isNoWaitInAction()
			)
			if (!noWait) {
				return true
			}
	...
	Sprite_Animation.prototype.targetPosition = function (renderer) {
		...
		if (arcX) {
			this.x +=
				(-arcX / Math.pow(arrival2 / 2, 2)) * Math.pow(Math.min(t2, arrival2) - arrival2 / 2, 2) +
				arcX
		}
		if (arcY) {
			this.y +=
				(-arcY / Math.pow(arrival2 / 2, 2)) * Math.pow(Math.min(t2, arrival2) - arrival2 / 2, 2) +
				arcY
		}
