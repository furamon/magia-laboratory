...
;(function () {
	...
	function convertEscapeCharacters(originalText) {
		...
		tmpText = tmpText.replace(/\x1bV\[(\d+)\]/gi, (_, p1) => $gameVariables.value(parseInt(p1)))
		tmpText = tmpText.replace(/\x1bV\[(\d+)\]/gi, (_, p1) => $gameVariables.value(parseInt(p1)))
		tmpText = tmpText.replace(/\x1bN\[(\d+)\]/gi, (_, p1) => actorName(parseInt(p1)))
		tmpText = tmpText.replace(/\x1bP\[(\d+)\]/gi, (_, p1) => partyMemberName(parseInt(p1)))
