...
;(function () {
	...
	PluginManager.registerCommand(PLUGIN_NAME, 'PutEventsByTile', function (args) {
		...
		coordinates = coordinates.concat(
			makeTargetCoordinates($dataAutotileTable, autotileTypes, range)
		)
	...
	function putEvent(coordinates, templateId, isCheckOpen) {
		...
		// 判定用に仮のイベントを作成する。
		const templateEvent = new Game_PrefabEvent(
			$gameMap._mapId,
			null,
			originalEventId,
			point.x,
	...
	function isProhibitedRegion(x, y) {
		return pProhibitedRegion && $gameMap.regionId(x, y) == pProhibitedRegion
	}
})()
