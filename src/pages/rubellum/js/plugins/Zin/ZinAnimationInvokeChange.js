...
;(() => {
	...
	ZinAnimInvokeChange.initialize = function () {
		...
		this._invokeColor[0] = Number(
			PluginManager.parameters('ZinAnimationInvokeChange')['invokeColorRed']
		)
		this._invokeColor[1] = Number(
			PluginManager.parameters('ZinAnimationInvokeChange')['invokeColorGreen']
		)
		this._invokeColor[2] = Number(
			PluginManager.parameters('ZinAnimationInvokeChange')['invokeColorBlue']
		)
	...
	Sprite_Animation.prototype.setup = function (targets, animation, mirror, delay, previous) {
		...
		for (const timing of this._animation.flashTimings) {
			if (
				timing.color[0] == ZinAnimInvokeChange._invokeColor[0] &&
				timing.color[1] == ZinAnimInvokeChange._invokeColor[1] &&
				timing.color[2] == ZinAnimInvokeChange._invokeColor[2]
			) {
	...
	Sprite_AnimationMV.prototype.setup = function (targets, animation, mirror, delay) {
		...
		for (const timing of this._animation.timings) {
			if (
				timing.flashColor[0] == ZinAnimInvokeChange._invokeColor[0] &&
				timing.flashColor[1] == ZinAnimInvokeChange._invokeColor[1] &&
				timing.flashColor[2] == ZinAnimInvokeChange._invokeColor[2]
			) {
				this._zinInvokeTiming = timing.frame
				return
			} else {
	...
	Window_BattleLog.prototype.displayAction = function (subject, item) {
		...
		if (
			ZinAnimInvokeChange._textSkip &&
			this._methods.length > 0 &&
			this._methods[this._methods.length - 1].name == 'wait'
		) {
			this._methods.pop()
		}
	}

	const Window_BattleLog_prototype_displayActionResults =
	...
	Window_BattleLog.prototype.displayHpDamage = function (target) {
		...
		if (
			...
			this._methods.length > 0 &&
			this._methods[this._methods.length - 1].name == 'addText'
		) {
			this._methods.pop()
		}
	}
	...
	const Window_BattleLog_prototype_displayAddedStates =
	...
	const Window_BattleLog_prototype_displayRemovedStates =
