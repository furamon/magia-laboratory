...
;(() => {
	...
	ZinTween.changeColorCode = function (colorCode, target) {
		...
		if (colorCode.includes('rgba')) {
			if (target !== undefined && target.tweenOpacity !== undefined) {
				var rgba = colorCode
					.substring(colorCode.indexOf('(') + 1, colorCode.indexOf(')'))
					.split(',')
					.map(Number)
	...
	ZinTween.GetColorCode = function (colorStruct, includingAlpha = true) {
		if (includingAlpha) {
			return (
				'rgba(' +
				String(colorStruct.r) +
				',' +
				String(colorStruct.g) +
				',' +
				String(colorStruct.b) +
				',' +
				String(colorStruct.a) +
				')'
			)
		} else {
			return (
	...
	ZinTween_Object.prototype.update = function (target) {
		...
		switch (this._wrapMode) {
			...
			case WrapMode.Pingpong:
				...
				if (this._frameCount >= (this._endTime + this._delay) * 60) {
					this.refresh(
						(this._endTime * 120 - this._frameCount + this._delay * 60) / (this._endTime * 60),
						target
					)
				} else {
					this.refresh((this._frameCount - this._delay * 60) / (this._endTime * 60), target)
	...
	ZinTween_Sequence.prototype.isPlaying = function () {
		if (this._tweenList.length == 0) return false
		else return true
	}
	...
	ZinTween.setMoveByFrom = function (
		zto,
		duration,
		x,
		y,
	...
	ZinTween_Alpha.prototype.refresh = function (t, target) {
		ZinTween_Object.prototype.refresh.apply(this, arguments)
		target.tweenOpacity = Math.round(
			this._startA + ZinTween.ease(t, this._easeType) * (this._endA - this._startA)
		)
		this.setOpacity(target, target.tweenOpacity)
	}
	...
	ZinTween_Color.prototype.refresh = function (t, target) {
		ZinTween_Object.prototype.refresh.apply(this, arguments)
		for (let i = 0; i < 4; i++) {
			target.tweenColor[i] = Math.round(
				this._startV[i] + ZinTween.ease(t, this._easeType) * (this._endV[i] - this._startV[i])
			)
		}
		this.setColor(target, target.tweenColor)
	}
	...
	ZinTween_Scale.prototype.start = function (target) {
		...
		if (this._xTweenFlag) {
			...
			this.setX(
				target,
				this.x(target) - (this.xScale(target) - oldScaleX) * this.width(target) * 0.5 * this._alignX
			)
		}
		if (this._yTweenFlag) {
			...
			this.setY(
				target,
				this.y(target) -
					(this.yScale(target) - oldScaleY) * this.height(target) * 0.5 * this._alignY
			)
		}
	}
	...
	ZinTween_Rotation.prototype.width = function (target) {
		if (target.ztWidth === undefined) return 100
		else return target.ztWidth
	}

	ZinTween_Rotation.prototype.height = function (target) {
		if (target.ztHeight === undefined) return 100
		else return target.ztHeight
	}
	...
	*/
	...
	ZinTween_Picture_Move.prototype.width = function (target) {
		if (target.ztWidth) return target.ztWidth * target._scaleX * 0.01
		else return 100
	}

	ZinTween_Picture_Move.prototype.height = function (target) {
		if (target.ztHeight) return target.ztHeight * target._scaleY * 0.01
		else return 100
	}
	...
	ZinTween_Picture_Scale.prototype.width = function (target) {
		if (target.ztWidth === undefined) return 100
		else return target.ztWidth // * target._scaleX * 0.01;
	}

	ZinTween_Picture_Scale.prototype.height = function (target) {
		if (target.ztHeight === undefined) return 100
		else return target.ztHeight // * target._scaleY * 0.01;
	}
