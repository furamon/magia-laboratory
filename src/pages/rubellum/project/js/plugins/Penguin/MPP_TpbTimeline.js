...
;(() => {
	...
	class Sprite_TimelineIcon extends Sprite {
		...
		createBitmap() {
			const { 'Origin X': originX = 0, 'Origin Y': originY = 0 } = this.iconParam()
		...
		updatePosition() {
			const { Type: type = 'Horizontal', 'Start X': startX = 6 } = param_Timeline
		...
		timeWidth() {
			const { 'Wait Width': waitWidth = 320, 'Action Width': actionWidth = 128 } = param_Timeline
			if (this._value > 1) {
	...
	class Sprite_EnemyTimelineIcon extends Sprite_TimelineIcon {
		...
		drawImage(battlerBitmap) {
			...
			const { 'Origin X': ox = 0, 'Origin Y': oy = 0 } = this.battlerOriginParams()
		...
		updateSymbolPosition() {
			const { 'Symbol X': symbolX = 0, 'Symbol Y': symbolY = 0 } = this.iconParam()
	...
	class Sprite_TimelineBar extends PIXI.Container {
		...
		createActionBarBitmap() {
			const { 'Start X': startX = 6, 'Action Width': actionWidth = 128 } = param_Timeline
		...
		updateActionBarFrame() {
			if (this._actionBarSprite) {
				const { 'Start X': startX = 4, 'Action Width': actionWidth = 128 } = param_Timeline
	...
	Scene_Battle.prototype.updateTimeline = function () {
		this._timelineSprite.visible =
			timelineVisible && this._messageWindow.isClosed() && !!BattleManager._phase
	}
