...
;(() => {
	...
	Game_Interpreter.prototype.evalChoice = function (condition) {
		try {
			...
			const realCondition = condition.replace(/V\[(\d+)\]/gi, (_, n) => $gameVariables.value(+n))
	...
	Game_Interpreter.prototype.extractHelpTexts = function (index) {
		...
		if (command.code === 108 && param_ChoiceHelpCommands.includes(command.parameters[0])) {
	...
	Game_Interpreter.prototype.nextCommandOfChoice = function (index) {
		const i = this._list
			.slice(index)
			.findIndex((command) => command.indent === this._indent && command.code === 404)
	...
	Game_Interpreter.prototype.pluginCommand = function (command, args) {
		...
		if (mzCommand) {
			const args2 = Object.assign({}, ...mzCommand.keys.map((key, i) => ({ [key]: args[i] })))
	...
	Window_ChoiceList.prototype.updatePlacement = function () {
		...
		const { choiceX: x, choiceY: y, choiceWidth: width, choiceHeight: height } = $gameMessage
		if (width >= 0) {
	...
	Window_ChoiceList.prototype.processCancel = function () {
		...
		if (cancelType >= 0 && this.isCancelEnabled() && !this.isCommandEnabled(cancelType)) {
	...
	Window_Message.prototype.isLowerChoice = function () {
		...
		return $gameMessage.isChoice() && $gameMessage.isChoiceInMessage() && textState.x !== startX
	}
