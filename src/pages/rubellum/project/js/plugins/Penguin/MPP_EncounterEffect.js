...
;(() => {
	...
	EncounterEffect.centerCharacter = function () {
		return this._character || $gameMap.getInterpreterCharacter(0) || $gamePlayer
	}
	...
	EncounterEffect.removeOuterPoint = function (points) {
		const { minX, maxX, minY, maxY } = this.windowBounds
		return points.filter(({ x, y }) => x >= minX && x <= maxX && y >= minY && y <= maxY)
	}

	EncounterEffect.uniqPoints = function (points) {
		return points.filter((p1, i) => points.findIndex((p2) => p2.equals(p1)) === i)
	}
	...
	EncounterEffect.createRoundPoints = function (startAngle, innerR, currentR) {
		...
		const randomAngle = (n) => {
			return (startAngle + baseAngle * n + (baseAngle * rRate * Math.random()) / 100) % 360
		}
		const randomRadius = () => {
			return innerR + currentR * (1 - (cRate * (0.75 - Math.random())) / 100)
		}

		return [...Array(splitX).keys()].map((i) => this.calculatePoint(randomAngle(i), randomRadius()))
	}
