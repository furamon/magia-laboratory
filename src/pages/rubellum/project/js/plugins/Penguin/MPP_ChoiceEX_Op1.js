...
;(() => {
	...
	Game_System.prototype.deleteCustomChoice = function (id, text) {
		...
		if (customChoices) {
			this._customChoices[id] = customChoices.filter((params) => params.text !== text)
		...
		if ($gameMessage.isTextOnly()) {
			if (Utils.RPGMAKER_NAME === 'MV') {
				...
				if (this.nextEventCode() === 357) {
					...
					if (nextParams[0] === pluginName && nextParams[1] === 'showCustomChoices') {
						this._index++
						PluginManager.callCommand(this, nextParams[0], nextParams[1], nextParams[3])
					}
				}
			}
		}
