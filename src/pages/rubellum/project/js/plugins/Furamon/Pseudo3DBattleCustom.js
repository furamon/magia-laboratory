...
;(() => {
	...
	const pluginName = 'Pseudo3DBattleCustom'
	...
	const param_Battleback2PivotYFixes = JSON.parse(
		parameters['Battleback2 Pivot Y Fixes'] || '[]',
		reviverParse
	)
	...
	Pseudo3DBattle.clearDriftParams = function () {
		...
		this._drifts = Object.assign(
			...Object.keys(this.driftBase).map((key) => ({ [key]: new EasingValue(type, delay) }))
		)
		this._driftRate = 1
	}
	...
	Pseudo3DBattle.addParams = function (params) {
		if (params.priority > 0 || params.remove || this.differentParams(params)) {
	...
	Pseudo3DBattle.differentParams = function (params) {
		const presentParams = this._moveParamsList.find((obj) => obj.priority === params.priority)
		return (
			!presentParams ||
			['x', 'y', 'altitude', 'scale', 'skew'].some((prop) => presentParams[prop] !== params[prop])
		)
	}

	Pseudo3DBattle.setParams = function (params) {
		const priority = params.priority || 0
		const index = this._moveParamsList.findIndex((obj) => obj.priority === priority)
		if (index >= 0) {
	...
	BattleManager.callP3dEndBattlerActionsMethod = function () {
		if (this.isTpb() && !$gameTemp.isCommonEventReserved() && !$gameTroop.isEventRunning()) {
	...
	Game_Interpreter.prototype.update = function () {
		...
		if ($gameParty.inBattle() && lastRunning && !$gameTroop.isEventRunning()) {
	...
	if (Sprite_Animation.prototype.updateEffectGeometryOnRender) {
		const _Sprite_Animation_updateEffectGeometryOnRender =
			Sprite_Animation.prototype.updateEffectGeometryOnRender
	...
	Sprite_Battleback.prototype.applyPseudo3dPlacement = function () {
		...
		const groundY = this._homeY - this._effectsOffsetY - 50
	...
	Spriteset_Battle.prototype.getBattlebackPivotY = function () {
		...
		return [...number_range(0, h)].reverse().find((n) => data[n * 4 + 3] > 192) || 0
	}
