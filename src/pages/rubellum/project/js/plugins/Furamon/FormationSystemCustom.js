...
const FormationSystemClassAlias = (() => {
	...
	class WindowModule_DataListTitle {
		...
		static createTitleSprite() {
			const bitmap = new Bitmap(
				...
				WindowModule_DataListTitle.TITLE_HEIGHT
			)
	...
	const params = PluginParamsParser.parse(
		PluginManager.parameters(FormationSystemPluginName),
		typeDefine
	)
	...
	const UseBattleBackgroundInMenu =
		params.UseBattleBackgroundInMenu != null ? params.UseBattleBackgroundInMenu : true
	...
	class Scene_Formation extends Scene_MenuBase {
		...
		createEquipFormationListWindow() {
			this._equipFormationListWindow = new Window_EquipFormationList(
				this.equipFormationListWindowRect()
			)
			...
			this._equipFormationListWindow.setHandler(
				'cancel',
				this.onEquipFormationListCancel.bind(this)
			)
			this._equipFormationListWindow.setHandler(
				'select',
				this.onEquipFormationListSelect.bind(this)
			)
		...
		formationDetailWindowRect() {
			const equipFormationListWindowRect = this.equipFormationListWindowRect()
			const x = this.isRightInputMode()
				? 0
				: equipFormationListWindowRect.x + equipFormationListWindowRect.width
			const y = equipFormationListWindowRect.y
			const w =
				Graphics.boxWidth - (this.isRightInputMode() ? equipFormationListWindowRect.width : x)
	...
	Game_Party.prototype.learnFormation = function (formationId) {
		if (
			this._equipFormationIds.includes(formationId) ||
			this._hasFormationIds.includes(formationId)
		)
			return
	...
	Game_Party.prototype.changeEquipFormations = function (slotIndex, targetFormation) {
		...
		if (hasFormationId != null)
			this._hasFormationIds = this._hasFormationIds.filter((id) => id !== hasFormationId)
	...
	Game_Party.prototype.changeCurrentSlot = function (slotIndex) {
		if (slotIndex === this._currentFormationSlot || this._equipFormationIds[slotIndex] == null)
			return false
	...
	Window_PartyCommand.prototype.insertCommand = function (
		index,
		name,
		symbol,
		enabled = true,
	...
	Window_PartyCommand.prototype.makeCommandList = function () {
		...
		if (EnabledBattleFormationChange)
			this.insertCommand(1, Text.MenuFormationText, 'battleFormation')
	}
	...
	Scene_Battle.prototype.createBattleFormationListWindow = function () {
		...
		this._battleFormationListWindow.setHandler(
			'cancel',
			this.onBattleFormationListCancel.bind(this)
		)
		this._battleFormationListWindow.setHandler(
			'select',
			this.onBattleFormationListSelect.bind(this)
		)
	...
	class FormationStateUtils {
		...
		static isChangeValid(erasedStateId) {
			return (
				FormationStateUtils.isInvalidState(erasedStateId) &&
				!FormationStateUtils.isFormationInvalid()
			)
		}
	...
	Window_MenuCommand.prototype.addOriginalCommands = function () {
		...
		if (Text.MenuFormationText !== '')
			this.addCommand(
				Text.MenuFormationText,
				'battleFormation',
				this.isEnabledBattleFormationMenu()
			)
	}
	...
	PluginManager.registerCommand(FormationSystemPluginName, 'LearnFormation', (args) => {
		...
		const formationId = params.VariableId
	...
	PluginManager.registerCommand(FormationSystemPluginName, 'ChangeEquipFormations', (args) => {
		const params = PluginParamsParser.parse(args, {
			...
			HasFormationId: 'number'
		})
		const equipSlotIndex = params.EquipSlotIndex
		const targetFormation =
			params.HasFormationId >= 0 ? $dataFormations[params.HasFormationId] : null
