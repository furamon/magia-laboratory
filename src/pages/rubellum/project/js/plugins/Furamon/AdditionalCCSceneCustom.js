...
/*:ja
 * @target MZ
 * @plugindesc 多重職業用の転職画面（アビリティシステム連携版）
 * @author 砂川赳（http://newrpg.seesaa.net/） 改変：Furamon
 * @base NRP_AdditionalClasses
 * @orderAfter NRP_AdditionalClasses
 ...
 * @url http://newrpg.seesaa.net/article/483582942.html
 *
 * @help AbilitySystemの後ろに置くこと。
 *
 * --- 以下原文コピペ、Thanks to 砂川赳氏（http://newrpg.seesaa.net/） ---
 *
 * 多重職業用の転職画面を実装します。
...
;(function () {
	...
	Windows_ClassSlot.prototype.setHeight = function () {
		let height =
			$gameSystem.windowPadding() * 2 + this.maxItems() * (this.lineHeight() + this.itemPadding())
		// 最大でも画面幅を超えない
	...
	function isRepeated(keyName) {
		if (this._isEscapeCompatible(keyName) && this.isRepeated('escape')) {
			...
			return (
				...
				(this._pressedTime === 0 ||
					(this._pressedTime >= 0 && this._pressedTime % this.keyRepeatInterval === 0))
			)
		}
	}
