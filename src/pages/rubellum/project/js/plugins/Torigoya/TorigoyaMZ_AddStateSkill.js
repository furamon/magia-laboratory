...
;(function () {
	...
	function applyPlugin() {
		...
		BattleManager.torigoyaAddStateSkill_readParameterForSubject = function (item) {
			...
			const removeStates = readStateIdsFromMeta(
				item.meta['RemoveState'] || item.meta['ステート削除']
			)
		...
		BattleManager.torigoyaAddStateSkill_readParameterForUnit = function (item) {
			const addStates = readStateIdsFromMeta(
				item.meta['AddStateUnit'] || item.meta['ステート追加全体']
			)
			const removeStates = readStateIdsFromMeta(
				item.meta['RemoveStateUnit'] || item.meta['ステート削除全体']
			)
			if (addStates.length === 0 && removeStates.length === 0) return false
