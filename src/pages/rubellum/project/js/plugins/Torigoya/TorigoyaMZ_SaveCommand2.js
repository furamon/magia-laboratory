...
;(function () {
	...
	;(() => {
		 ...
		 * @returns {Promise<*>}
		 */
		Game_Interpreter.prototype.torigoyaSaveCommand_requestSave = function (
			savefileId,
			skipUpdateTimestamp
		) {
			Torigoya.SaveCommand2.requestCall = () => {
				if (skipUpdateTimestamp) {
					if (DataManager._globalInfo && DataManager._globalInfo[savefileId]) {
						Torigoya.SaveCommand2.backupLastTimestamp =
							DataManager._globalInfo[savefileId].timestamp || 0
					} else {
		...
		PluginManager.registerCommand(
			Torigoya.SaveCommand2.name,
			'saveWithVariable',
			commandSaveWithVariable
		)
		...
		PluginManager.registerCommand(
			Torigoya.SaveCommand2.name,
			'deleteWithVariable',
			commandDeleteWithVariable
		)
		PluginManager.registerCommand(
			Torigoya.SaveCommand2.name,
			'deleteLastSlot',
			commandDeleteLastSlot
		)
	})()
})()
