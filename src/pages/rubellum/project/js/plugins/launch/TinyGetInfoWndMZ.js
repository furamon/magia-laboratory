...
;(() => {
	...
	Spriteset_Base.prototype.createInfoWindow = function (id, type, text, value, includeEquip) {
		if (this.getInfoWndFactory.length > 0) {
			return this.getInfoWndFactory.shift().setup(id, type, text, value, includeEquip)
		} else {
			return new Window_GetInfo(id, type, text, value, includeEquip)
		}
	}

	Spriteset_Base.prototype.addGetInfoWindow = function (id, type, text, value, includeEquip) {
	...
	Window_GetInfo.prototype.initialize = function (id, type, text, value, includeEquip) {
	...
	const getRealValue = (type, id, value, includeEquip) => {
		...
		if (type >= 0 && type <= 2) {
			...
			if (value < 0) {
				const numItems = $gameParty.numItems(item) + (includeEquip ? numItemEquip(item) : 0)
				if (-value > numItems) {
					value = -numItems
				}
			}
	...
	Window_GetInfo.prototype.setup = function (id, type, text, value, includeEquip) {
		...
		if (type >= 0 && type <= 2) {
			...
			if (defaultInfoTextType === 'firstLine' || data.meta.info) {
				if (doesUseControlChar) {
					this.drawTextEx(noteDescription(data), 384, 21, Graphics.boxWidth - 384)
				} else {
					this.drawText(noteDescription(data), 384, 21, Graphics.boxWidth - 384, 'left')
				}
			}
		} else if (type === 3) {
			this.contents.paintOpacity = 255
