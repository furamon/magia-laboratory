...
;(() => {
	...
	Game_Action.prototype.evaluateWithTarget = function (target) {
		...
		if (this.isForOpponent()) {
			...
			if (this.isForAll()) {
				...
				value = isCrisis(target) ? value + (0.8 - hpRate) * 2 : Math.min(value, 1 - hpRate) ** 2 / 2
			}
		}
	...
	Game_Action.prototype.evaluate = function () {
		...
		if (this.isHpRecover() && this.isForAll()) {
			let numToAttention = $gameParty.aliveMembers().filter((a) => isAttention(a)).length
	...
	Game_Action.prototype.hasItemAnyValidEffects2 = function (target) {
		return this.item().effects.some((effect) => this.testItemEffect2(target, effect))
	}
