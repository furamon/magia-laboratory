...
*/
const DotMoveSystemPluginName = document.currentScript
...
;(function (DotMoveSystem) {
	...
	class DotMoveRectangle {
		...
		isCollidedRect(rect) {
			if (
				((this.x > rect.x && this.x < rect.x2) ||
					(this.x2 > rect.x && this.x2 < rect.x2) ||
					(rect.x >= this.x && rect.x2 <= this.x2)) &&
				((this.y > rect.y && this.y < rect.y2) ||
					(this.y2 > rect.y && this.y2 < rect.y2) ||
					(rect.y >= this.y && rect.y2 <= this.y2))
			) {
				return true
			}
			return false
	...
	class AStarUtils {
		...
		static computeRoute(character, startX, startY, goalX, goalY, searchLimit) {
			...
			const start = new AStarNode(
				...
				startX,
				startY,
				$gameMap.distance(startX, startY, goalX, goalY),
				0
			)
	...
	class CharacterCollisionChecker {
		...
		checkCharacter(x, y, d, character) {
			...
			const subjectRect = new DotMoveRectangle(
				x,
				y,
				this._character.width(),
				this._character.height()
			)
			const targetRect = new DotMoveRectangle(
				targetX,
				targetY,
				character.width(),
				character.height()
		...
		checkCollisionMass(subjectRect, d, ix, iy) {
			if (!this.checkPassMass(ix, iy, d)) {
				...
				const result = this.checkCollidedRect(
					d,
					subjectRect.clone(),
					massRect,
					new MassInfo(ix, iy)
		...
		checkCollisionXCliff(subjectRect, x1, x2, iy, d) {
			...
			for (let ix = x1; ix < x2; ix++) {
				const ix2 = $gameMap.roundX(ix)
				if (
					!this.checkPassMass(ix2, iy2, 4) &&
					!this.checkPassMass($gameMap.roundX(ix2 + 1), iy2, 6)
				) {
					const massRect1 = new DotMoveRectangle(ix2, iy2, 1, 1)
					const massRect2 = new DotMoveRectangle($gameMap.roundX(ix2 + 1), iy2, 1, 1)
					const result1 = this.checkCollidedRect(
						d,
						subjectRect.clone(),
						massRect1,
						new MassInfo(massRect1.x, massRect1.y)
					)
					const result2 = this.checkCollidedRect(
						d,
						subjectRect.clone(),
						massRect2,
						new MassInfo(massRect2.x, massRect2.y)
					)
					if (result1 && result2) {
		...
		checkCollisionYCliff(subjectRect, y1, y2, ix, d) {
			...
			for (let iy = y1; iy < y2; iy++) {
				const iy2 = $gameMap.roundY(iy)
				if (
					!this.checkPassMass(ix2, iy2, 8) &&
					!this.checkPassMass(ix2, $gameMap.roundY(iy2 + 1), 2)
				) {
					const massRect1 = new DotMoveRectangle(ix2, iy2, 1, 1)
		...
		calcSubjectCharacterMassRange(x, y) {
			...
			if (this._overComplementMode) {
				const subjectOrigRect = new DotMoveRectangle(
					this._origX,
					this._origY,
					subjectRect.width,
					subjectRect.height
				...
				if ($gameMap.isLoopHorizontal()) {
					...
					if (
						subjectRect.x < subjectOrigRect.width &&
						subjectOrigRect.x >= $gameMap.width() - subjectOrigRect.width
					) {
						subjectRect.x += $gameMap.width()
					} else if (
						subjectRect.x >= $gameMap.width() - subjectRect.width &&
						subjectOrigRect.x < subjectRect.width
					) {
				...
				if ($gameMap.isLoopVertical()) {
					...
					if (
						subjectRect.y < subjectOrigRect.height &&
						subjectOrigRect.y >= $gameMap.height() - subjectOrigRect.height
					) {
						subjectRect.y += $gameMap.height()
					} else if (
						subjectRect.y >= $gameMap.height() - subjectRect.height &&
						subjectOrigRect.y < subjectRect.height
					) {
		...
		checkCollidedRect(d, subjectRect, targetRect, targetObject) {
			...
			if ($gameMap.isLoopHorizontal()) {
				...
				if (
					targetRect.x < subjectRect.width &&
					subjectRect.x >= $gameMap.width() - subjectRect.width
				) {
					targetRect.x += $gameMap.width()
				} else if (
					targetRect.x >= $gameMap.width() - targetRect.width &&
					subjectRect.x < targetRect.width
				) {
					subjectRect.x += $gameMap.width()
					origX += $gameMap.width()
				}
			...
			if ($gameMap.isLoopVertical()) {
				...
				if (
					targetRect.y < subjectRect.height &&
					subjectRect.y >= $gameMap.height() - subjectRect.height
				) {
					targetRect.y += $gameMap.height()
				} else if (
					targetRect.y >= $gameMap.height() - targetRect.height &&
					subjectRect.y < targetRect.height
				) {
					subjectRect.y += $gameMap.height()
			...
			if (this._overComplementMode) {
				return this.checkCollidedRectOverComplement(
					origX,
					origY,
					d,
					subjectRect,
		...
		checkCollidedRectOverComplement(origX, origY, d, subjectRect, targetRect, targetObject) {
			switch (d) {
				...
				case 6:
					if (
						DotMoveUtils.isFloatLt(origX + subjectRect.width, targetRect.x) &&
						subjectRect.x2 > targetRect.x2
					) {
				...
				case 2:
					if (
						DotMoveUtils.isFloatLt(origY + subjectRect.height, targetRect.y) &&
						subjectRect.y2 > targetRect.y2
					) {
	...
	class CharacterDotMoveProcess {
		...
		correctDistance(pos, distance, dir) {
			...
			const collisionResults = this.checkCollision(nextX, nextY, dir, {
				origX,
				origY,
				overComplementMode: true,
				throughIfCollided: true
			})
		...
		reachPoint(realPoint, targetPoint, margin) {
			if (
				Math.abs(realPoint.x - targetPoint.x) <= margin &&
	...
	class CharacterMover {
		...
		checkCollisionCharacters(x, y, direction, targetCharacterClass) {
			return this.createCollisionChecker().checkCollisionCharacters(
				x,
				y,
				direction,
				targetCharacterClass
			)
		}
		...
		checkHitCharactersStepDir(x, y, direction, targetCharacterClass) {
			...
			const x2 = x + dis.x
			const y2 = y + dis.y
			return this.createCollisionChecker().checkHitCharacters(
				x2,
				y2,
				direction,
				targetCharacterClass
		...
		continuousMoveProcess() {
			...
			const moved = this.createDotMoveProcess().dotMoveByDeg(
				new Degree(this._moverData.moveDeg),
				dpf
			)
			if (moved) {
	...
	class MoverData {
		get targetFar() {
			return this._targetFar
		}
		set targetFar(_targetFar) {
			this._targetFar = _targetFar
		}
		get moveDeg() {
			return this._moveDeg
		}
		set moveDeg(_moveDeg) {
			this._moveDeg = _moveDeg
		}
		get stopping() {
			return this._stopping
		}
		set stopping(_stopping) {
			this._stopping = _stopping
		}
		constructor(...args) {
			this.initialize(...args)
		}
	...
	class CharacterDotMoveTempData {
		...
		get mapCharacterCacheUpdater() {
			return this._mapCharacterCacheUpdater
		}
		constructor(...args) {
			this.initialize(...args)
		}
	...
	class PlayerDotMoveTempData extends CharacterDotMoveTempData {
		get collideTriggerEventIds() {
			return this._collideTriggerEventIds
		}
		set collideTriggerEventIds(_collideTriggerEventIds) {
			this._collideTriggerEventIds = _collideTriggerEventIds
		}
	...
	class EventDotMoveTempData extends CharacterDotMoveTempData {
		...
		get eventTouchToPlayer() {
			return this._eventTouchToPlayer
		}
		set eventTouchToPlayer(_eventTouchToPlayer) {
			this._eventTouchToPlayer = _eventTouchToPlayer
		}
		constructor(character) {
			super(character)
		}
	...
	class FollowerDotMoveTempData extends CharacterDotMoveTempData {
		get sameDirectionTotalDpf() {
			return this._sameDirectionTotalDpf
		}
		set sameDirectionTotalDpf(_sameDirectionTotalDpf) {
			this._sameDirectionTotalDpf = _sameDirectionTotalDpf
		}
		constructor(character) {
			super(character)
		}
	...
	Game_CharacterBase.prototype.isCollidedWithCharacters = function (x, y, d = this.direction()) {
		...
		if (axis === 'x') {
			const collisionLength = Math.max(
				...collisionResults.map((result) => result.collisionLengthY())
			)
			if (collisionLength > this.slideLengthY()) canSlide = false
		} else {
			const collisionLength = Math.max(
				...collisionResults.map((result) => result.collisionLengthX())
			)
			if (collisionLength > this.slideLengthX()) canSlide = false
		}
		if (!canSlide) {
			const lens = collisionResults.map((result) =>
				result.getCollisionLengthByDirection(this.reverseDir(d))
			)
	...
	Game_CharacterBase.prototype.checkHitCharacters = function (targetCharacterClass) {
		return this.mover().checkHitCharacters(
			this._realX,
			this._realY,
			this._direction,
			targetCharacterClass
	...
	Game_Player.prototype.updateRemoveCollideTriggerEventIds = function () {
		...
		for (const eventId of tempData.collideTriggerEventIds) {
			...
			if (event) {
				...
				if (
					result &&
					result.collisionLengthX() >= event.widthArea() &&
					result.collisionLengthY() >= event.heightArea()
				) {
					continue
				}
			}
			tempData.collideTriggerEventIds = tempData.collideTriggerEventIds.filter(
				(id) => id !== eventId
			)
		}
	}
	...
	Game_Player.prototype.checkRideVehicles = function () {
		...
		if (
			airshipResult &&
			airshipResult.collisionLengthX() >= this.minTouchWidth() &&
			airshipResult.collisionLengthY() >= this.minTouchHeight()
		) {
			return 'airship'
		} else {
			...
			if (
				shipResult &&
				shipResult.collisionLengthX() >= this.minTouchWidth() &&
				shipResult.collisionLengthY() >= this.minTouchHeight()
			) {
				return 'ship'
			} else {
				...
				if (
					boatResult &&
					boatResult.collisionLengthX() >= this.minTouchWidth() &&
					boatResult.collisionLengthY() >= this.minTouchHeight()
				) {
					return 'boat'
				}
			}
	...
	Game_Player.prototype.startMapEventFront = function (x, y, d, triggers, normal, isTouch) {
		...
		for (const result of this.mover().checkHitCharactersStepDir(x, y, d, Game_Event)) {
			...
			if (
				result.getCollisionLength(otherAxis) >= area &&
				result.getCollisionLength(axis) >= otherAxisLen
			) {
				if (event.isTriggerIn(triggers) && event.isNormalPriority() === normal) {
	...
	Game_Event.prototype.checkEventTriggerTouchFront = function (d) {
		...
		if (this._trigger === 2) {
			...
			const otherAxis = axis === 'y' ? 'x' : 'y'
			const playerMinTouchWidthOrHeight =
				otherAxis === 'x' ? $gamePlayer.minTouchWidth() : $gamePlayer.minTouchHeight()
	...
	Game_Follower.prototype.isGathered = function () {
		...
		return (
			result.collisionLengthX() >= $gamePlayer.width() - margin &&
			result.collisionLengthY() >= $gamePlayer.height() - margin
		)
	}
