...
;(() => {
	...
	Scene_Battle.prototype.canChangeActor = function () {
		const numCanInputMembers = $gameParty
			.battleMembers()
			.filter((member) => member.canInput()).length
	...
	Scene_Battle.prototype.arePageButtonsEnabled = function () {
		...
		return (
			this.canChangeActor() && this._actorCommandWindow.active && this._actorCommandWindow.visible
		)
	}
