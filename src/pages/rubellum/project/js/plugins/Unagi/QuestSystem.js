...
*/
const QuestSystemPluginName = document.currentScript
...
;(function (QuestSystem) {
	...
	class QuestUtils {
		static startQuestScene(questCommands, fileName1 = '', fileName2 = '', xOfs = 240, yOfs = 300) {
			...
			SceneManager.prepareNextScene(questCommandsParam, {
				FileName1: fileName1,
				FileName2: fileName2,
				XOfs: xOfs,
				YOfs: yOfs
	...
	class QuestData {
		...
		get questOrderCommonEventId() {
			return this._questOrderCommonEventId
		}
		get questReportCommonEventId() {
			return this._questReportCommonEventId
		}
		get questCancelCommonEventId() {
			return this._questCancelCommonEventId
		}
		get priority() {
	...
	const COMMAND_TABLE = {
		...
		questOrder: new QuestCommand(
			['notOrdered'],
			Text.QuestOrderCommandText,
			CommandIcon.AllCommandIcon
		),
		orderingQuest: new QuestCommand(
			['ordering', 'reportable'],
			Text.OrderingQuestCommandText,
			CommandIcon.OrderingQuestCommandIcon
		),
		questCancel: new QuestCommand(
			['ordering'],
			Text.QuestCancelCommandText,
			CommandIcon.QuestCancelCommandIcon
		),
		questReport: new QuestCommand(
			['reportable'],
			Text.QuestReportCommandText,
			CommandIcon.QuestReportCommandIcon
		),
		reportedQuest: new QuestCommand(
			['reported'],
			Text.ReportedQuestCommandText,
			CommandIcon.ReportedQuestCommandIcon
		),
		failedQuest: new QuestCommand(
			['failed'],
			Text.FailedQuestCommandText,
			CommandIcon.FailedQuestCommandIcon
		),
		expiredQuest: new QuestCommand(
			['expired'],
			Text.ExpiredQuestCommandText,
			CommandIcon.ExpiredQuestCommandIcon
		),
		hiddenQuest: new QuestCommand(
			['hidden'],
			Text.HiddenQuestCommandText,
			CommandIcon.HiddenQuestCommandIcon
		)
	}
	// MV compatible
	...
	class Scene_QuestSystem extends superScene_Message {
		...
		questDetailWindowRect() {
			...
			const h =
				questCommandWindowRect.height +
				questListWindowRect.height +
				questOrderCountWindowRect.height
			return new Rectangle(x, y, w, h)
		}
	...
	class Window_QuestOrderCount extends Window_Selectable_MZMV {
		...
		drawAllItems() {
			...
			this.drawText(
				`${this._numOrderingQuests}/${MaxOrderingQuests}`,
				rect.x,
				rect.y,
				rect.width,
				'right'
	...
	class Window_QuestDetail extends Window_Selectable_MZMV {
		...
		onTouchOk() {
			...
			if (
				!(localPos.x >= 0 && localPos.y >= 0 && localPos.x < this.width && localPos.y < this.height)
			)
				return
	...
	DataManager.extractSaveContents = function (contents) {
		...
		$questSaveDatas = $dataQuests.map((questData) => {
			const savedQuestSaveData = contents.questSaveDatas.find(
				(saveData) => saveData.variableId() === questData.variableId
			)
