...
const SkillTreeClassAlias = (() => {
	...
	class SkillTreeConfigLoader {
		...
		loadSkillTreeNodes(type) {
			...
			if (!derivative)
				throw new SkillTreeConfigLoadError(`Missing skill type name ${type.skillTreeName()}`)
			...
			for (const data of derivative) {
				...
				if (data.length >= 2) {
					...
					for (const childTag of childsTag) {
						if (!nodes[childTag])
							throw new SkillTreeConfigLoadError(`Unknow derivative ${childTag}`)
		...
		loadSkillTreeInfo(actorId, allNodes) {
			...
			for (const node of Object.values(allNodes)) {
				if (!node.info())
					throw new SkillTreeConfigLoadError(`Node ${node.tag()} is missing node info`)
			}
		}
	}
	...
	class SkillTreeView {
		...
		createView() {
			...
			if (Utils.RPGMAKER_NAME === 'MZ') {
				...
				width =
					Math.ceil(maxPx / this._windowWidth) * this._windowWidth + IconWidth + IconSpaceWidth
				height =
					Math.ceil(maxPy / this._windowHeight) * this._windowHeight + IconHeight + IconSpaceHeight
			}
