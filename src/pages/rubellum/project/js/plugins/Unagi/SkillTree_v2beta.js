...
アクターIDを指定します。
*/
const SkillTreePluginName = document.currentScript
...
;(function (SkillTree) {
	...
	class SkillTreeNodeInfo {
		constructor(actorId, skillId, needCost, iconData, helpMessage) {
			...
			if (this._skillId !== 0) {
				...
				this._learnCommonEventId = skill.meta.LearnCommonEventId
					? parseInt(skill.meta.LearnCommonEventId)
					: 0
				this._forgetCommonEventId = skill.meta.ForgetCommonEventId
					? parseInt(skill.meta.ForgetCommonEventId)
					: 0
			} else {
	...
	class SkillTreeData {
		...
		enableTypes(actorId) {
			if (SkillTreeManagementByClasses) {
				...
				return this.types(actorId).filter(
					(type) => type.enabled() && type.classId().indexOf(classId) >= 0
				)
			} else {
	...
	class Window_TypeSelect extends Window_Command {
		...
		drawItem(index) {
			...
			if (iconIndex) {
				this.drawItemName(
					{ name: this.commandName(index), iconIndex: iconIndex },
					rect.x,
					rect.y,
					this.width
				)
	...
	class SkillTreeView {
		...
		viewXY() {
			...
			let viewX, viewY
			// ノードの中央を中心にビューを表示する。
			const selectNodeCenterPos = new Point(
				selectNodePos.x + IconWidth / 2,
				selectNodePos.y + IconHeight / 2
			)
		...
		createView() {
			...
			if (Utils.RPGMAKER_NAME === 'MZ') {
				...
				width =
					Math.ceil(maxPos.x / this._windowWidth) * this._windowWidth + IconWidth + IconSpaceWidth
				height =
					Math.ceil(maxPos.y / this._windowHeight) * this._windowHeight +
					IconHeight +
					IconSpaceHeight
			}
