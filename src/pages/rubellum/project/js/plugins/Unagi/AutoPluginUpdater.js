...
;(() => {
	...
	if (params['DisableUpdatePlugin'] !== '')
		DisableUpdatePlugin = JSON.parse(params['DisableUpdatePlugin'])

	const BackupPath = (() => {
		if (BackupDirName === '')
			throw new Error('バックアップファイルを保存するBackupDirNameが指定されていません。')
		const date = new Date()
	...
	class PluginUpdater {
		...
		createPluginFileDatas() {
			const enabledPlugins = $plugins.filter(
				(plugin) => plugin.status && !DisableUpdatePlugin.includes(plugin.name)
			)
