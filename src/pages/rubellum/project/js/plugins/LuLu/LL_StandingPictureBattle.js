		...
		static update(elm) {
			...
			if (isPhase == 'start') {
				if (!this._battleStart) {
					...
					// 先頭アクターIDを取得
					this._firstActorId =
						this._aliveBattleMembers.length > 0 ? this._aliveBattleMembers[0]._actorId : null
					// ランダムアクターID抽選
					this._randomActorId =
			...
			if (isPhase == 'turnEnd') {
				...
				this._randomActorId =
					this._aliveBattleMembers.length > 0
						? this._aliveBattleMembers[Math.floor(Math.random() * this._aliveBattleMembers.length)]
								._actorId
						: null
			}
			...
			if (isPhase == 'battleEnd') {
				if (isEscaped) {
					...
					if (!this._battleEnd) {
						...
						// 先頭アクターIDを取得
						this._firstActorId =
							aliveBattleMembers.length > 0 ? aliveBattleMembers[0]._actorId : null
						// ランダムアクターID抽選
						this._randomActorId =
							aliveBattleMembers.length > 0
								? aliveBattleMembers[Math.floor(Math.random() * aliveBattleMembers.length)]._actorId
								: null
					}
