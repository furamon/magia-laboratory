...
;(() => {
	...
	const ContentsBackGroundImg =
		(NUUN_Base_Ver >= 113
			? DataManager.nuun_structureData(parameters['ContentsBackGroundImg'])
			: null) || []
	const ContentsBackSettingsList =
		(NUUN_Base_Ver >= 113
			? DataManager.nuun_structureData(parameters['ContentsBackSettingsList'])
			: null) || []
	const ContentsList =
		(NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['ContentsList']) : null) || []
	...
	function getSaveContentsBuckgroundImg() {
		return !!$dataMap && $dataMap.meta && $dataMap.meta.SaveContentsBackImg
			? ContentsBackGroundImg[Number($dataMap.meta.SaveContentsBackImg) - 1]
			: $gameSystem.saveContentsBuckgroundImg
	}
	...
	DataManager.setPartyData = function (info, data) {
		...
		info.actorClass = ClassSpecifyActorOnry
			? [actor.currentClass().name]
			: $gameParty.actorClassForSavefile()
		info.actorNickName = NickNameSpecifyActorOnry
			? [actor.nickname()]
			: $gameParty.actorNickNameForSavefile()
	...
	Window_SavefileList.prototype.drawContentsBack = function (bitmap, index, buckgroundImg) {
		const data = ContentsBackSettingsList.find(
			(imgData) => buckgroundImg === imgData.ContentsBackGroundImg
		)
		let sx = 0
		let sy = 0
		...
		try {
			...
			const log = $gameSystem.isJapanese()
				? 'NUUN_BaseがVer.1.6.2以降ではありません。'
				: "'NUUN_Base' is not Ver.1.6.2 or later."
			throw ['LoadError', log]
		}
	}
	...
	Window_SavefileList.prototype.drawContentsNotData = function (
		info,
		x,
		y,
		width,
	...
	Window_SavefileList.prototype.drawPlaytime = function (info, x, y, width, data) {
		...
		if (info.playtime) {
			this.drawText(
				info.playtime,
				x + systemWidth + padding,
				y,
				width - (systemWidth + padding),
				data.Align
			)
		}
	}
	...
	Window_SavefileList.prototype.drawActorLeval = function (info, x, y, width, data, index) {
		...
		if (!!data.ShowEval) {
			...
			for (const level of levelActor) {
				...
				this.drawText(
					level,
					levelActorX + textWidth + padding / 2,
					y,
					width2 - (textWidth + padding),
					data.Align
				)
				levelActorX += width2
			}
	...
	Game_System.prototype.getSaveBuckGround = function () {
		this.saveBuckgroundImg =
			this.saveBuckgroundImg && !!this.saveBuckgroundImg[0]
				? this.saveBuckgroundImg
				: [BackGroundImg, 0]
	...
	Game_Party.prototype.charactersForSavefile = function () {
		if (PartyActorMode === 1) {
			...
			return this.members().map((actor) => [actor.characterName(), actor.characterIndex()])
		}
	}
	...
	Game_Party.prototype.facesForSavefile = function () {
		if (PartyActorMode === 1) {
			...
			return this.members().map((actor) => [actor.faceName(), actor.faceIndex()])
		}
	}
	...
	Game_Party.prototype.charactersFilterForSavefile = function (data) {
		return this.allMembers()
			.filter((actor) => eval(data.ShowEval))
