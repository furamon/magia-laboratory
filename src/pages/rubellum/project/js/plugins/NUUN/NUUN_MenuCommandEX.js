...
;(() => {
	...
	const MenuCommandSetting =
		(NUUN_Base_Ver >= 113
			? DataManager.nuun_structureData(parameters['MenuCommandSetting'])
			: null) || []
	...
	Window_MenuCommand.prototype.refreshCommandList = function () {
		...
		this._list.forEach((data, index) => {
			const find = MenuCommandSetting.findIndex(
				(command) => command.CommandName === data.name || command.CommandSymbol === data.symbol
			)
	...
	Window_MenuCommand.prototype.setCommandSprite = function () {
		this._list.forEach((data, index) => {
			...
			if (this._commandSprite[index]) {
				...
				this._commandSprite[index].setup(
					rect.x,
					rect.y,
					this.menuCommandExData(index),
					this.currentSymbol(),
					lastSymbol
				)
			}
	...
	Window_MenuCommand.prototype.drawContentsItemImg = function (bitmap, data, index) {
		...
		this.contents.blt(
			...
			rect.width,
			rect.height,
			rect.x + (data.CommandImgX || 0),
			rect.y + (data.CommandImgY || 0)
		)
	}
	...
	Window_MenuCommand.prototype.getBackgroundImg = function (index) {
		const data = this.menuCommandExData(index)
		if (data) {
			return this.isCurrentCommandEx(index) && data.SelectContentsBackGroundImg
				? data.SelectContentsBackGroundImg
				: data.ContentsBackGroundImg
		}
		return null
	}
