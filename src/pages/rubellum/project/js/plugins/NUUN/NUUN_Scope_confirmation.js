...
;(() => {
	...
	const UserSelectTasg =
		(NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['UserSelectTasg']) : null) ||
		[]
	...
	Scene_Battle.prototype.isTargetSelectForUser = function (action, item) {
		return (
			ForUserSelect &&
			this.enemyOnrySelect(action) &&
			!this.noTargetSelect(item) &&
			action.isForUser()
		)
	}
