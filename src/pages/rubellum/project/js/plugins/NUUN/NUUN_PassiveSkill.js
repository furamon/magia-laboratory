...
;(() => {
	...
	const PassiveSkillConditions =
		(NUUN_Base_Ver >= 113
			? DataManager.nuun_structureData(parameters['PassiveSkillConditions'])
			: null) || []
	...
	Game_Actor.prototype.getCondition = function (skill) {
		...
		return this.getTriggerConditions(
			skill,
			this,
			condTag,
			null,
	...
	Game_Actor.prototype.skillConditions = function (list) {
		...
		switch (list.ParamConditions) {
			case 'HP':
				...
				return (
					this.hp >= (this._pmhp * list.DwLimit) / 100 &&
					(list.UpLimit > 0 ? this.hp <= (this._pmhp * list.UpLimit) / 100 : true)
				)
			case 'MP':
				if (!this._pmmp) {
					const _mmp = this.mmp
				...
				return (
					this.mp >= (this._pmmp * list.DwLimit) / 100 &&
					(list.UpLimit > 0 ? this.mp <= (this._pmmp * list.UpLimit) / 100 : true)
				)
			case 'TP':
				return (
					this.tp >= (this.maxTp() * list.DwLimit) / 100 &&
					(list.UpLimit > 0 ? this.tp <= (this.maxTp() * list.UpLimit) / 100 : true)
				)
			case 'State':
			...
			case 'Equip':
				return list.EquipWeapon > 0
					? this.isEquippedWeaponType(list.EquipWeapon)
					: this.isEquippedArmorType(list.EquipWeapon)
			case 'Turn':
				return (
					this.turnCount() >= list.DwLimit &&
					(list.UpLimit > 0 ? this.turnCount() <= list.UpLimit : true)
				)
			case 'GVal':
				return (
					$gameVariables.value(list.VariableId) >= list.DwLimit &&
