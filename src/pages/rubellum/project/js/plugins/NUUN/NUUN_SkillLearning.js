...
;(() => {
	...
	const SkillLearningSe =
		NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['SkillLearningSe']) : null
	...
	Game_Action.prototype.skillLearning = function (target) {
		...
		if (skill.meta.SkillLearning) {
			...
			for (const battler of members) {
				if (
					(battler === target && battler.isSkillLearningAbility()) ||
					battler.isWatchingSkillLearningAbility()
				) {
					if (
						!this.isSkillLearningSkill(battler, skillId) &&
						Math.random() < battler.skillLearningRate(data[0])
					) {
	...
	Game_Action.prototype.setAttackSkillLearning = function (target, data) {
		...
		const skills = target
			.attackSkillLearningList()
			.filter(
				(skillId) =>
					!this.isSkillLearningSkill(subject, skillId) &&
					this.isOnAttackSkillLearning(skillId, subject, target, object)
			)
	...
	Game_Action.prototype.isSkillLearningSkill = function (battler, skillId) {
		return battler.isActor()
			? battler.isLearnedSkill(skillId)
			: battler.isEnemyLearningSkills(skillId)
	}
	...
	Game_BattlerBase.prototype.isAllSkillLearningAbility = function () {
		return this.traitObjects().some(
			(trait) => !!trait.meta.SkillLearningAbility || !!trait.meta.WatchingSkillLearningAbility
		)
	}
	...
	Game_BattlerBase.prototype.getSkillLearningCorrection = function () {
		return this.traitObjects().reduce((r, trait) => {
			return (
				r + (trait.meta.SkillLearningCorrection ? Number(trait.meta.SkillLearningCorrection) : 0)
			)
		}, 0)
	}
	...
	Window_BattleLog.prototype.displayActionResults = function (subject, target) {
		...
		if (result.used && result.isSkillLearning) {
			if (
				Imported.NUUN_popUp &&
				!!target.shouldPopupSkillLearning &&
				target.shouldPopupSkillLearning()
			) {
