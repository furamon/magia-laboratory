...
;(() => {
	...
	Game_Interpreter.prototype.setup = function (list, eventId) {
		...
		if (eventId > 0) {
			...
			$gamePlayer.setDistanceFrom(
				event.deltaXFrom($gamePlayer.x) * -1,
				event.deltaYFrom($gamePlayer.y) * -1
			)
		}
	}
	...
	Game_CharacterBase.prototype.moveStraight = function (d) {
		...
		if (this.isEventRangeEvent() && this.isMovementSucceeded() && !$gameMap.isEventRunning()) {
			...
			if (
				this._trigger === 2 &&
				this.range($gamePlayer.x, $gamePlayer.y) &&
				!$gamePlayer.pos(this.x, this.y)
			) {
	...
	Game_Event.prototype.isCollidedWithPlayerCharacters = function (x, y) {
		return (
			_Game_Event_isCollidedWithPlayerCharacters.call(this, x, y) ||
			(this.isNormalPriority() && this.isEventRangeCollidedWithPlayerCharacters(x, y))
		)
	}

	Game_Event.prototype.isEventRangeCollidedWithPlayerCharacters = function (x, y) {
		return (
			!!this.getEventRangeCollidedTag() &&
			($gamePlayer.range(x, y, this) || $gamePlayer.rangeFollower(x, y, this))
		)
	}
	...
	Game_CharacterBase.prototype.isCollidedWithEvents = function (x, y) {
		const events = $gameMap.eventsRangeXyNt(this, x, y)
		return (
			_Game_CharacterBase_isCollidedWithEvents.call(this, x, y) ||
			events.some((event) => event.isNormalPriority())
		)
	}
	...
	Game_Character.prototype.range = function (x, y, e) {
		...
		const recognition = event.event().meta.EventRecognition
			? Number(event.event().meta.EventRecognition)
			: EventRecognitionRange
	...
	Game_Character.prototype.rangeEX = function (x, y, x1, y1, x2, y2, x3, y3, x4, y4) {
		return (
			this.rangeCp(this.x + x1, this.y + y1, this.x + x2, this.y + y2, x, y) &&
