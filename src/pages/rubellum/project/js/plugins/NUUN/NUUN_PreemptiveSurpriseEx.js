...
;(() => {
	...
	const PreemptiveSurpriseSetting =
		(NUUN_Base_Ver >= 113
			? DataManager.nuun_structureData(parameters['PreemptiveSurpriseSetting'])
			: null) || []
	const DefaultPreemptive =
		NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['DefaultPreemptive']) : null
	const DefaultSurprise =
		NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['DefaultSurprise']) : null
	const PreemptiveSE =
		NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['PreemptiveSE']) : null
	const SurpriseSE =
		NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['SurpriseSE']) : null
	...
	const DefaultPresurSprict =
		(NUUN_Base_Ver >= 113
			? DataManager.nuun_structureData(parameters['DefaultPresurSprict'])
			: null) || []
	const PreemptivePresurSprict =
		(NUUN_Base_Ver >= 113
			? DataManager.nuun_structureData(parameters['PreemptivePresurSprict'])
			: null) || []
	const SurprisePresurSprict =
		(NUUN_Base_Ver >= 113
			? DataManager.nuun_structureData(parameters['SurprisePresurSprict'])
			: null) || []
	...
	BattleManager.customOnEncounter = function () {
		...
		if (tagData || _eventPresurMode > 0) {
			...
			if (tagData) {
				data = isNaN(tagData)
					? PreemptiveSurpriseSetting.find((a) => tagData === a.Name)
					: PreemptiveSurpriseSetting[Number(tagData) - 1]
			...
			if (preemptive || surprise) {
				...
				if ($gameTemp.onEventEncounter) {
					preemptive =
						data && data.EventPreemptiveSetting && data.EventPreemptiveSetting.EvalData
							? eval(data.EventPreemptiveSetting.EvalData)
							: false
					surprise =
						!preemptive && data && data.EventSurpriseSetting && data.EventSurpriseSetting.EvalData
							? eval(data.EventSurpriseSetting.EvalData)
							: false
				} else {
					preemptive =
						data && data.RandomPreemptiveSetting && data.RandomPreemptiveSetting.EvalData
							? eval(data.RandomPreemptiveSetting.EvalData)
							: DefaultPreemptive
								? eval(DefaultPreemptive)
								: false
					surprise =
						!preemptive && data && data.RandomSurpriseSetting && data.RandomSurpriseSetting.EvalData
							? eval(data.RandomSurpriseSetting.EvalData)
							: DefaultSurprise
								? eval(DefaultSurprise)
								: false
				}
			}
			...
			if (preemptive || surprise) {
				...
				if (!$gameTemp.onEventEncounter) {
					...
					surprise =
						!preemptive && (DefaultSurprise ? eval(DefaultSurprise) : this.onDefaultSurprise())
				}
			}
	...
	BattleManager.setPresurSe = function (preemptive, surprise, data) {
		if (data) {
			if (preemptive) {
				this.presurSE =
					data.PreemptiveSE && data.PreemptiveSE.name ? data.PreemptiveSE : PreemptiveSE
			} else if (surprise) {
	...
	Game_Interpreter.prototype.command301 = function (params) {
		...
		if (
			!$gameParty.inBattle() &&
			(EventPreSurSwitch === 0 || $gameSwitches.value(EventPreSurSwitch))
		) {
