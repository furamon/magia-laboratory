...
;(() => {
	...
	const _Window_FormationBattleMember_isCurrentItemEnabled =
		Window_FormationBattleMember.prototype.isCurrentItemEnabled
	...
	const _Window_FormationMember_isCurrentItemEnabled =
		Window_FormationMember.prototype.isCurrentItemEnabled
	...
	const _Window_FormationBattleMember_isChangeActorEnabled =
		Window_FormationBattleMember.prototype.isChangeActorEnabled
	Window_FormationBattleMember.prototype.isChangeActorEnabled = function (actor, pendingActor) {
		if (ActorFixedMovable && actor) {
			if (pendingActor) {
				return !pendingActor.isBattleMember()
					? _Window_FormationBattleMember_isChangeActorEnabled.call(this, actor)
					: true
			} else {
				return this.getPendingMode() === 'battle' || !this.getPendingMode()
					? true
					: _Window_FormationBattleMember_isChangeActorEnabled.call(this, actor, pendingActor)
			}
		} else if (actor) {
	...
	const _Window_FormationMember_isChangeActorEnabled =
		Window_FormationMember.prototype.isChangeActorEnabled
	...
	Window_FormationBattleMember.prototype.getPendingActor = function (pending) {
		const index =
			(pending === 'member' ? $gameParty.maxBattleMembers() : 0) + this.formationPendingIndex()
		return $gameParty.allMembers()[index]
	}
