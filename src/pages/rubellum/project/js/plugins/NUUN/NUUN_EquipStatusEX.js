...
;(() => {
	...
	const EquipPageList =
		(NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['EquipPageList']) : null) ||
		[]
	const EquipStatusCols = Number(parameters['EquipStatusCols'] || 1)
	const ElementResist =
		(NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['ElementResist']) : null) ||
		[]
	const ElementResistMode = eval(parameters['ElementResistMode'])
	const StateResist =
		(NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['StateResist']) : null) || []
	...
	const ActorsImgList =
		(NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['ActorsImgList']) : null) ||
		[]
	...
	const equipContents = {}
	equipContents.PageList1 =
		NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['EquipPageList1']) : []
	equipContents.PageList2 =
		NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['EquipPageList2']) : []
	equipContents.PageList3 =
		NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['EquipPageList3']) : []
	equipContents.PageList4 =
		NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['EquipPageList4']) : []
	equipContents.PageList5 =
		NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['EquipPageList5']) : []
	equipContents.PageList6 =
		NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['EquipPageList6']) : []
	equipContents.PageList7 =
		NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['EquipPageList7']) : []
	equipContents.PageList8 =
		NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['EquipPageList8']) : []
	equipContents.PageList9 =
		NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['EquipPageList9']) : []
	equipContents.PageList10 =
		NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['EquipPageList10']) : []
	...
	Scene_Equip.prototype.previousPage = function () {
		...
		this._statusWindow.refresh()
		if (this._itemWindow) SoundManager.playCursor()
		if (this._itemWindow.index() >= 0 && this._itemWindow.visible) {
			this._itemWindow.activate()
		} else if (this._slotWindow.visible) {
			this._slotWindow.activate()
	...
	Scene_Equip.prototype.nextActor = function () {
		if (
			(this._slotWindow.visible && this._slotWindow.active) ||
			(this._itemWindow.visible && this._itemWindow.active)
		) {
			this.nextPage()
		} else {
	...
	Window_EquipStatusEX.prototype.drawActorFace = function (actor, x, y, width, height) {
		...
		if (mode !== 'none') {
			if (mode === 'imgApng') {
				...
				bitmap =
					mode === 'face'
						? ImageManager.nuun_LoadPictures(data.FaceImg)
						: ImageManager.nuun_LoadPictures(data.ActorImg)
			}
		}
		if (bitmap) {
	...
	Window_EquipStatusEX.prototype.drawEquipActorFace = function (
		data,
		bitmap,
		x,
		y,
	...
	Window_EquipStatusEX.prototype.drawActorGraphic = function (
		data,
		bitmap,
		x,
		y,
	...
	Window_EquipStatusEX.prototype.drawAllParams = function () {
		if (!!this._equipBitmap) {
			this._equipBitmap.addLoadListener(
				function () {
	...
	Window_EquipStatusEX.prototype.drawParam = function (data, actor, x, y, width) {
		...
		const systemWidth = Math.min(
			data.SystemItemWidth || 160,
			width - (width - this.paramX(width)) - this.itemPadding()
		)
		...
		if (actor) {
			this.drawText(
				value + (data.paramUnit ? String(data.paramUnit) : ''),
				paramX + x,
				y,
				paramWidth,
				'right'
		...
		if (this._tempActor) {
			...
			this.changeTextColor(
				ColorManager.paramchangeTextColor(getBatStatus(data.BatStatus, value, newValue))
			)
			this.drawText(
				newValue + (data.paramUnit ? String(data.paramUnit) : ''),
				paramX + paramWidth + rightArrowWidth + x,
				y,
				paramWidth,
				'right'
			)
			if (EquipStatusDifference) {
				this.drawText(
					'(' + diffvalueText(diffvalue) + ')',
					paramX + paramWidth * 2 + rightArrowWidth + x,
					y,
					this.paramDifferenceWidth(),
					'left'
				)
			}
		}
	...
	Window_EquipStatusEX.prototype.drawXParam = function (data, actor, x, y, width) {
		...
		const value = NuunManager.numPercentage(
			actor.xparam(paramId - 30) * 100,
			data.Decimal || 0,
			DecimalMode
		)
		const paramX = this.paramX(width)
		...
		this.drawRightArrow(paramX + paramWidth + x, y)
		if (this._tempActor) {
			const newValue = NuunManager.numPercentage(
				this._tempActor.xparam(paramId - 30) * 100,
				data.Decimal || 0,
				DecimalMode
			)
			const diffvalue = newValue - value
	...
	Window_EquipStatusEX.prototype.drawSParam = function (data, actor, x, y, width) {
		...
		if (this._tempActor) {
			const newValue = NuunManager.numPercentage(
				this._tempActor.sparam(paramId - 40) * 100,
				data.Decimal - 2,
				DecimalMode
			)
			const diffvalue = badParams(paramId)
				? getBatStatus(data.BatStatus, newValue, value)
				: getBatStatus(data.BatStatus, value, newValue)
	...
	Window_EquipStatusEX.prototype.drawOriginalStatusParam = function (data, actor, x, y, width) {
		...
		if (actor) {
			this.drawText(
				text + (data.paramUnit ? String(data.paramUnit) : ''),
				paramX + x,
				y,
				paramWidth,
				'right'
		...
		if (this._tempActor) {
			...
			if (!isNaN(text)) {
				this.changeTextColor(
					ColorManager.paramchangeTextColor(getBatStatus(data.BatStatus, text, newValue))
				)
			} else {
				this.resetTextColor()
			}
	...
	Window_EquipStatusEX.prototype.drawElement = function (data, actor, x, y, width) {
		...
		ElementResist.forEach((element) => {
			...
			if (elementId > 0) {
				...
				this.resetTextColor()
				const rate = NuunManager.numPercentage(
					actor.elementRate(elementId) * 100,
					data.Decimal - 2,
					DecimalMode
				)
				if (actor) {
					this.drawText(
						rate + (data.paramUnit ? String(data.paramUnit) : '%'),
						paramX + x2 + x,
						y + y2,
						paramWidth,
						'right'
				...
				if (this._tempActor) {
					const newValue = NuunManager.numPercentage(
						this._tempActor.elementRate(elementId) * 100,
						data.Decimal - 2,
						DecimalMode
					)
					const diffvalue = rate - newValue
					this.changeTextColor(
						ColorManager.paramchangeTextColor(getBatStatus(element.BatStatus, newValue, rate))
					)
					this.drawText(
						newValue + (data.paramUnit ? String(data.paramUnit) : '%'),
						paramX + paramWidth + rightArrowWidth + x2 + x,
						y + y2,
						paramWidth,
						'right'
					)
					if (EquipStatusDifference) {
						this.drawText(
							'(' + diffvalueText(diffvalue * -1) + ')',
							paramX + paramWidth * 2 + rightArrowWidth + x2 + x,
							y + y2,
							this.paramDifferenceWidth(),
							'left'
						)
			...
			if (stateId > 0) {
				...
				if (this._tempActor) {
					let newValue =
						...
						100 *
						(this._tempActor.isStateResist(stateId) ? 0 : 1)
					newValue = NuunManager.numPercentage(newValue, data.Decimal - 2, DecimalMode)
					const diffvalue = rate - newValue
					this.changeTextColor(
						ColorManager.paramchangeTextColor(getBatStatus(state.BatStatus, newValue, rate))
					)
					this.drawText(
						newValue + (data.paramUnit ? String(data.paramUnit) : '%'),
						paramX + paramWidth + rightArrowWidth + x2 + x,
	...
	Window_EquipStatusEX.prototype.paramX = function (width) {
		//再定義
		...
		return (
			...
			paramWidth * 2 -
			rightArrowWidth -
			(EquipStatusDifference ? this.paramDifferenceWidth() : 0)
		)
	}
	...
	Window_EquipStatusEX.prototype.getSetBonusList = function (actor, data, width) {
		...
		list.forEach((setBonus) => {
			...
			if (setBonusSum >= 2) {
				textList.push({
					text: setData.SetBonusName,
					color: NuunManager.getColorCode(data.NameColor),
					row: 1,
					mode: 'headline',
					tag: tag
				})
			...
			setData.SetBonusNumberEquipment.forEach((numberEquip) => {
				if (
					setData.SetBonusEquip &&
					setData.SetBonusEquip.length > 1 &&
					numberEquip.SetNumberEquip <= setBonusSum
				) {
					this.drawSetBonusNumberEquipment(textList, tag, data, numberEquip, width)
				} else if (
					!(setData.SetBonusEquip && setData.SetBonusEquip.length > 1) &&
					numberEquip.SetNumberEquip <= setBonusSum
				) {
					this.drawSetBonusNumberEquipment(textList, tag, data, numberEquip, width)
				}
			...
			if (
				setData.SetBonusEquip &&
				setData.SetBonusEquip.length > 1 &&
				setData.SetBonusEquip.length === setBonusSum
			) {
	...
	Window_EquipStatusEX.prototype.drawSetBonusNumberEquipment = function (
		textList,
		tag,
		data,
		numberEquip,
		...
		const equip = getSetBonusEquip(
			numberEquip.SetNumberEquipWeaponData,
			numberEquip.SetNumberEquipArmorData
		)
		if (equip) {
			let text = ''
			let textWidth = 0
			if (numberEquip.SetBonusText) {
				textList.push({
					text: numberEquip.SetBonusText,
					color: NuunManager.getColorCode(data.NameColor),
					row: 0,
					mode: 'headline',
					tag: tag + numberEquip.SetBonusText
				})
	...
	Window_EquipStatusEX.prototype.drawSetBonusParam = function (
		textList,
		tag,
		data,
		numberEquip,
	...
	Window_EquipStatusEX.prototype.drawTextList = function (
		textList,
		tempList,
		x,
		y,
		...
		for (const text of textList) {
			if (this._tempActor) {
				if (tempList.some((a) => a.text === text.text)) {
					...
					text.color
						? this.changeTextColor(text.color)
