...
;(() => {
	...
	Sprite_Enemy.prototype.setMpGaugePosition = function () {
		if (this._battlerMp) {
			...
			const x =
				(enemy.meta.MPGaugeX ? Number(enemy.meta.MPGaugeX) : 0) +
				Gauge_X +
				this._enemy.getMpGaugePositionX()
			const y =
				(enemy.meta.MPGaugeY ? Number(enemy.meta.MPGaugeY) : 0) +
				Gauge_Y +
				this._enemy.getMpGaugePositionY()
			this._battlerMp.x = x
	...
	Sprite_BattlerMPGauge.prototype.setup = function (battler, type) {
		...
		this.opacity =
			this.gaugeVisibleResult() && (this.gaugeVisibleInDamage() || this.gaugeVisibleInSelect())
				? 255
				: 0
	}
