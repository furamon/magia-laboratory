...
;(() => {
	...
	const ActorLogWindowPosition =
		(NUUN_Base_Ver >= 113
			? DataManager.nuun_structureData(parameters['ActorLogWindowPosition'])
			: null) || {}
	const EnemyLogWindowPosition =
		(NUUN_Base_Ver >= 113
			? DataManager.nuun_structureData(parameters['EnemyLogWindowPosition'])
			: null) || {}
	...
	Window_BattleLog.prototype.drawLineText = function (index) {
		...
		if (LogMessageMode === 0) {
			Window_NUUN_BattleLog.prototype.drawItemName.call(
				this,
				this._battleLogItem,
				x,
				WindowY,
	...
	Window_BattleLog.prototype.numLines = function () {
		return LogMessageMode === 0
			? Math.min(_Window_BattleLog_numLines.call(this), 1)
			: this._battleLogMessage.length
	}
	...
	Window_BattleLog.prototype.startAction = function (subject, action, targets) {
		...
		if (LogWindowMode === 1) {
			this.push(
				'showBattleLogWindow',
				item,
				this._spriteset.battlerSprites().find((sprite) => sprite._battler === subject)
			)
		}
	...
	Window_BattleLog.prototype.displayDamage = function (target) {
		if (target.result().used) {
			this.push(
				'battleLogWindowPopup',
				target,
				this._spriteset.battlerSprites().find((sprite) => sprite._battler === target)
			)
		}
