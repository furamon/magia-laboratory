...
;(() => {
	...
	Spriteset_Battle.prototype.updateTimeLineEnemy = function () {
		...
		const leng =
			troop > this._tpbTimeLineEnemyMaxMembers ? troop.length : this._tpbTimeLineEnemyMaxMembers
		...
		if (state !== this._tpbState) {
			if (this._tpbState === 'acting' && state !== 'acting') {
				...
				this._targetX =
					...
					(this.isVertical()
						? 0
						: (this.getTimeLineDirection() > 0 ? TPBTimeLineLength : 0) + this.getChargedOffSet())
				this._targetY =
					this.getOffSetY() +
					(this.isVertical()
						? (this.getTimeLineDirection() > 0 ? TPBTimeLineLength : 0) + this.getChargedOffSet()
				...
				this._targetX =
					this.getOffSetX() +
					this.getActionOffSetX() +
					(!this.isVertical() && this.getTimeLineDirection() > 0
						? TPBTimeLineLength + this.getChargedOffSet()
						: 0)
				this._targetY =
					this.getOffSetY() +
					Math.floor(TPBTimeLineBattlerHeight / 2) +
					this.getActionOffSetY() +
					(this.isVertical() && this.getTimeLineDirection() > 0
						? TPBTimeLineLength + this.getChargedOffSet()
						: 0)
				this._return = false
	...
	Sprite_TimeLine.prototype.getReturnActionFlame = function () {
		return Math.max(
			Math.floor(
				ActionReturnDuration /
					(TPBTimeLineLength / Math.max(this.getActionOffSetX(), this.getActionOffSetY()))
			),
			1
		)
	}
	...
	Sprite_TimeLine.prototype.getTimeLineRatio = function () {
		return this.isCasting() || this.isTpbReady()
			? this.getCastTime() / this._battler.tpbRequiredCastTime()
			: this.getTpbTime()
	}
	...
	Sprite_TimeLine.prototype.getTimeLineDefaultHomeY = function () {
		switch (TimeLineDirection) {
			case 'up':
				return (
					TPBTimeLineLength + Math.floor(TPBTimeLineBattlerHeight / 2) + this.getChargedOffSet()
				)
			case 'down':
	...
	Sprite_TimeLine.prototype.updateCasting = function () {
		...
		const cast =
			requiredCastTime > 0
				? TPBTimeLineLength * (this.getCastTime() / requiredCastTime)
				: TPBTimeLineLength * this.getTpbTime()
		if (TimeLineDirection === 'up') {
			x += 0
	...
	Sprite_TimeLine.prototype.patternWidth = function () {
		return this._isBigCharacter
			? this._battlerSprite.bitmap.width / 3
			: this._battlerSprite.bitmap.width / 12
	}

	Sprite_TimeLine.prototype.patternHeight = function () {
		return this._isBigCharacter
			? this._battlerSprite.bitmap.height / 4
			: this._battlerSprite.bitmap.height / 8
	}
