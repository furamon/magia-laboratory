...
;(() => {
	...
	const ActorStatus =
		NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['ActorStatus']) : []
	const DecimalMode = eval(parameters['DecimalMode'] || 'true')
	const EquipIcons =
		NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['EquipIcons']) : []
	...
	const SupportActorBackColor =
		DataManager.nuun_structureData(parameters3['SupportActorBackColor']) || 5
	...
	function isbattleMembersDead(actor, withdrawalActor) {
		const members = $gameParty
			.battleMembers()
			.filter((member) => member !== withdrawalActor && member.isDead())
	...
	Game_Party.prototype.allStandByMembers = function () {
		return this.allMembers()
			.filter((member) => member.isAppeared())
			.slice(this.maxBattleMembers())
	}
	...
	Game_Party.prototype.changeEntryBattleMember = function () {
		const members =
			...
			(this.battleMembers().length === this._formationBattleMembers ? 1 : 0)
	...
	class Nuun_Formation {
		...
		battleMemberNameWindowRect() {
			const wx =
				param.BattleMemberName_X +
				(param.WindowCenter ? (Graphics.boxWidth - this.memberWindowWidth()) / 2 : 0)
		...
		battleMemberWindowRect() {
			const ww =
				param.BattleMemberWindowWidth > 0
					? param.BattleMemberWindowWidth
					: $gameSystem.windowPadding() * 2 + this.battleMemberWindowWidth()
			const wx =
				param.BattleMember_X +
				(param.WindowCenter ? (Graphics.boxWidth - this.memberWindowWidth()) / 2 : 0)
			const wy =
				param.BattleMember_Y + (param.WindowZero ? 0 : this._scene.calcWindowHeight(1, true))
			const wh =
				param.BattleMemberWindowHeight > 0
					? param.BattleMemberWindowHeight
					: 32 + param.BattleMember_Rows * param.MemberHeight
			return new Rectangle(wx, wy, ww, wh)
		}
		...
		memberWindowRect() {
			...
			const wy =
				param.Member_Y +
				(param.WindowZero ? 0 : this.memberY() + this._scene.calcWindowHeight(1, true))
			const wh =
				param.MemberWindowHeight > 0
					? param.MemberWindowHeight
					: 32 + param.Member_Rows * param.MemberHeight
			return new Rectangle(wx, wy, ww, wh)
		}
		...
		onChangeBattleMemberOk() {
			const index = Math.min(
				formationIndex % this._battleMemberWindow.maxCols(),
				$gameParty.formationMember().length - 1,
				param.Member_Cols
			)
		...
		onChangeMemberOk() {
			...
			const index = Math.min(
				value >= maxItem ? (rows > 1 ? value - cols : maxItem - 1) : value,
				maxItem - 1
			)
		...
		memberWindowWidth() {
			return (
				$gameSystem.windowPadding() * 2 + param.Member_Cols * (CharacterMode === 'face' ? 152 : 56)
			)
		}
		...
		battleMemberWindowWidth() {
			return (
				(param.BattleMember_Cols > 0
					? param.BattleMember_Cols
					: _Game_Party_maxBattleMembers.call($gameParty)) *
				(Imported.NUUN_SceneFormation_SupportActor && param.BattleMember_Rows === 1 ? 2 : 1) *
				(CharacterMode === 'face' ? 152 : 56)
			)
		}
	...
	Window_StatusBase.prototype.drawBackGroundActor = function (index) {
		...
		if (index !== this._pendingIndex) {
			...
			if (actor && DeadActorColor >= 0 && actor.isDead()) {
				...
				Imported.NUUN_SceneFormation_SupportActor &&
				SupportActorBackColor >= 0 &&
				actor &&
				actor.getSupportActor()
			) {
	...
	Window_FormationBattleMember.prototype.maxItems = function () {
		return Math.min(
			...
			_Game_Party_maxBattleMembers.call($gameParty) +
				(Imported.NUUN_SceneFormation_SupportActor
					? $gameParty.membersInSupportActorNum(this._members)
					: 0)
		)
	}

	Window_FormationBattleMember.prototype.maxCols = function () {
		return param.BattleMember_Cols > 0
			? param.BattleMember_Cols
			: _Game_Party_maxBattleMembers.call($gameParty)
	}
	...
	Window_FormationBattleMember.prototype.isCurrentItemEnabled = function () {
		...
		const pendingActor =
			pendingMode === 'battle'
				? this.actor(pendingIndex)
				: $gameParty.formationMember()[pendingIndex]
		if (pendingActor) {
			return (
				!this.isFormationMembersDead(actor, pendingActor) &&
				this.isChangeActorEnabled(actor, pendingActor) &&
				this.isFormationChangeActorEnabled(actor, pendingActor)
			)
		} else if (actor) {
			return (
				this.isChangeActorEnabled(actor) && this.isFormationChangeActorEnabled(actor, pendingActor)
			)
	...
	Window_FormationBattleMember.prototype.isFormationChangeActorEnabled = function (
		actor,
		pendingActor
	) {
		return true
	...
	const _Window_FormationBattleMember_processTouch =
		Window_FormationBattleMember.prototype.processTouch
			...
			if (CharacterMode === 'img') {
				...
				const imges =
					Imported.NUUN_ActorPicture && ActorPictureEXApp
						...
						: actor.getFormationActorImg(data)
	...
	const _Window_FormationBattleMember_setCursorRect =
	...
	const _Window_FormationBattleMember_drawBackgroundRect =
		Window_FormationBattleMember.prototype.drawBackgroundRect
	...
	Window_FormationMember.prototype.isCurrentItemEnabled = function () {
		...
		const pendingActor =
			pendingMode === 'battle'
				? $gameParty.formationBattleMember()[pendingIndex]
				: this.actor(pendingIndex)
		if (pendingActor) {
			return (
				!this.isFormationMembersDead(actor, pendingActor) &&
			...
			return (
				!this.isFormationMembersDead(actor, pendingActor) &&
				this.isChangeActorEnabled(actor) &&
				this.isFormationChangeActorEnabled(actor, pendingActor)
			)
		} else if (pendingMode && !actor && !pendingActor) {
			return false
	...
	Window_FormationMember.prototype.drawItem = function (index) {
		...
		if (!actor) {
			...
			if (CharacterMode === 'img') {
				data =
					Imported.NUUN_ActorPicture && ActorPictureEXApp
						? this.battlreFormationPicture(actor.actorId())
						: actor.getFormationActorImgData()
				const imges =
					Imported.NUUN_ActorPicture && ActorPictureEXApp
						? actor.getActorGraphicImg()
						: actor.getFormationActorImg(data)
	...
	const _Window_FormationMember_drawBackgroundRect =
		Window_FormationMember.prototype.drawBackgroundRect
	...
	Window_FormationStatus.prototype.expNextPercent = function (list) {
		if (this._actor.isMaxLevel()) {
			...
			return (
				NuunManager.numPercentage(this.expPercent(), list.Decimal || 0, DecimalMode) * 100 +
				String(list.paramUnit || '%')
			)
		}
	}
	...
	Window_FormationStatus.prototype.expPercent = function () {
		return (
			...
			(this._actor.nextLevelExp() - this._actor.currentLevelExp())
		)
	}
	...
	Sprite_FormationActor.prototype.battlreActorPicture = function (id) {
		//立ち絵表示EX用
		const actors = ActorPictureData
		const find = actors.find((actor) => actor.actorId === id)
		if (!find) {
