...
;(() => {
	...
	const GaugeValueSetting =
		(NUUN_Base_Ver >= 113
			? DataManager.nuun_structureData(parameters['GaugeValueSetting'])
			: null) || []
	...
	Sprite_Gauge.prototype.getFindGaugeData = function (statusType) {
		return GaugeValueSetting.find(
			(data) =>
				(data.Type === statusType || data.Type === 'all') && this.filteringGaugeDataClass(data)
		)
	}

	Sprite_Gauge.prototype.valueWidth = function () {
		return (
			(this._gaugeData.ValueWidth && this._gaugeData.ValueWidth > 0
				? Math.min(this._gaugeData.ValueWidth, this.bitmapWidth() - this.gaugeX())
				: this.bitmapWidth() - this.gaugeX()) - this.valueMargin()
		)
	}
	...
	Sprite_Gauge.prototype.gaugeX = function () {
		return this._gaugeData && this._gaugeData.GaugeX >= 0
			? this._gaugeData.GaugeX
			: _Sprite_Gauge_gaugeX.call(this)
	}
	...
	Sprite_Gauge.prototype.valueAlign = function () {
		return this._gaugeData && this._gaugeData.ValueAlign && this._gaugeData.ValueAlign !== 'default'
			? this._gaugeData.ValueAlign
			: ValueAlign
	}
	...
	Sprite_Gauge.prototype.valueFontSize = function () {
		return this._gaugeData
			? $gameSystem.mainFontSize() + (this._gaugeData.ValueFontSize || 0)
			: _Sprite_Gauge_valueFontSize.call(this)
	}

	Sprite_Gauge.prototype.maxValueFontSize = function () {
		return (
			$gameSystem.mainFontSize() + (this._gaugeData ? this._gaugeData.MaxValueFontSize || 0 : -6)
		)
	}

	Sprite_Gauge.prototype.separationFontSize = function () {
		return (
			$gameSystem.mainFontSize() + (this._gaugeData ? this._gaugeData.SeparationFontSize || 0 : -6)
		)
	}
	...
	Sprite_Gauge.prototype.labelFontSize = function () {
		return this._gaugeData
			? $gameSystem.mainFontSize() + (this._gaugeData.LabelFontSize || 0)
			: _Sprite_Gauge_labelFontSize.call(this)
	}

	const _Sprite_Gauge_labelColor = Sprite_Gauge.prototype.labelColor
	Sprite_Gauge.prototype.labelColor = function () {
		return this._gaugeData
			? NuunManager.getColorCode(this._gaugeData.LabelColor || 16)
			: _Sprite_Gauge_labelColor.call(this)
	}

	Sprite_Gauge.prototype.maxValueColor = function () {
		return this._gaugeData
			? this._gaugeData.MaxValueColor === -1
				? this.valueColor()
				: NuunManager.getColorCode(this._gaugeData.MaxValueColor || 0)
			: ColorManager.textColor(0)
	}

	Sprite_Gauge.prototype.separationColor = function () {
		return this._gaugeData
			? this._gaugeData.SeparationColor === -1
				? this.valueColor()
				: NuunManager.getColorCode(this._gaugeData.SeparationColor || 0)
			: ColorManager.textColor(0)
	}
	...
	Sprite_Gauge.prototype.changeGaugeColor1 = function () {
		if (this._gaugeData.GaugeColorMaxApply && this.currentMaxValue() === this.currentValue()) {
			...
			this._gaugeData.GaugeColorRatioApply &&
			this._statusType === 'hp' &&
			this.getGaugeHpRatio()
		) {
	...
	Sprite_Gauge.prototype.valueXPosition = function (
		x,
		width,
		valueWidth,
		maxValueWidth,
	...
	Sprite_Gauge.prototype.maxValueXPosition = function (
		x,
		width,
		valueWidth,
		maxValueWidth,
	...
	Sprite_Gauge.prototype.separationXPosition = function (
		x,
		width,
		valueWidth,
		maxValueWidth,
	...
	Sprite_Gauge.prototype.drawGaugeGaugeValueEX = function () {
		if (this.drawGaugeVisible()) {
			...
			const gaugeY =
				(this.gaugeAbsoluteCoordinates()
					? 0
					: (this.textHeight ? this.textHeight() : 24) - this.gaugeHeight()) + this.gaugeY()
	...
	Sprite_Gauge.prototype.drawValue2 = function () {
		...
		const x =
			(this.valueAbsoluteCoordinates() ? 0 : this.gaugeX()) + this.valueX() + this.valueMargin()
		const y = this.valueY()
		this.setupValueFont()
	...
	Sprite_Gauge.prototype.drawValueMaxValue = function () {
		...
		this.setupValueFont()
		const valueWidth = valueDigits
			? Math.min(this.getValueTextWidth(), width)
			: Math.min(this.bitmap.measureTextWidth(currentValue), width)
		this.bitmap.fontSize = this.maxValueFontSize()
		const maxValueWidth = valueDigits
			? Math.min(this.getValueTextWidth(), width)
			: Math.min(this.bitmap.measureTextWidth(currentMaxValue), width)
		this.setupValueFont()
		this.bitmap.drawText(
			currentValue,
			this.valueXPosition(x, bitmapWidth, valueWidth, maxValueWidth, ValueMarginX) + this.valueX(),
			this.valueY(),
			valueWidth,
			height,
			this.valueAlign()
		...
		this.bitmap.drawText(
			'/',
			this.separationX() +
				this.separationXPosition(x, bitmapWidth, valueWidth, maxValueWidth, ValueMarginX),
			this.separationY(),
			12,
			height,
			'center'
		...
		this.bitmap.drawText(
			currentMaxValue,
			this.maxValueXPosition(x, bitmapWidth, valueWidth, maxValueWidth, ValueMarginX) +
				this.maxValueX(),
			this.maxValueY(),
			maxValueWidth,
			height,
	...
	Sprite_Gauge.prototype.drawLabel2 = function () {
		if (this._LabelIconSprite && this._gaugeData.LabelIcon > 0) {
			...
			sprite.x =
				(this.labelAbsoluteCoordinates() ? 0 : this.labelOutlineWidth() / 2) + this.labelX()
	...
	Sprite_Gauge.prototype.isGaugeImageLabel = function () {
		return (
			Imported.NUUN_GaugeImage &&
			this._gaugeImgData &&
			this._gaugeImgData.LabelGaugeImg &&
			this._gaugeImgData.LabelGaugeImg.Gaugeimage
		)
	}
	...
	Sprite_Gauge.prototype.valueFontFace = function () {
		if (this._gaugeData) {
			return this._gaugeData.UserValueFontFace
				? this._gaugeData.UserValueFontFace
				: this._gaugeData.ValueFontFace
					? $gameSystem.mainFontFace()
					: _Sprite_Gauge_valueFontFace.call(this)
		} else {
	...
	Sprite_Gauge.prototype.labelFontFace = function () {
		if (this._gaugeData) {
			return this._gaugeData.UserLabelFontFace
				? this._gaugeData.UserLabelFontFace
				: _Sprite_Gauge_labelFontFace.call(this)
		} else {
