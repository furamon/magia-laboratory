...
;(() => {
	...
	BattleManager.getNotVisibleIcons = function () {
		return params.NotVisibleStateIcons.filter((stateId) => $dataStates[stateId])
			.map((stateId) => $dataStates[stateId].iconIndex)
			.filter((id) => id > 0)
	}
	...
	Game_BattlerBase.prototype.stateIcons = function () {
		if (
			...
			BattleManager.bsVisibleStates.length > 0
		) {
	...
	Game_BattlerBase.prototype.statesFilter = function () {
		return this.states().filter(
			(state) =>
				BattleManager.bsVisibleStates.indexOf(state.id) >= 0 &&
				this.notBsIconFilter(state.iconIndex)
		)
	}
	...
	Game_BattlerBase.prototype.buffsFilter = function (id) {
		return (
			BattleManager.bsVisibleBuffs.indexOf(id) >= 0 &&
			this.notBsIconFilter(this.buffIconIndex(this._buffs[id], id))
		)
	}

	Game_BattlerBase.prototype.notBsIconFilter = function (id) {
		return BattleManager.notIconList && BattleManager.notIconList.length > 0
			? !BattleManager.notIconList.indexOf(id) >= 0
			: true
	}
	...
	Game_Battler.prototype.isBSActorGraphicDead = function (data) {
		return (
			(data && data.ImgStateAll && this.getStateData(data.ImgStateAll)) ||
			data.ChangeGraphicScenes === 'death' ||
			(data.ChangeGraphicScenes === 'state' && data.stateId && this.getStateData(data.stateId))
		)
	}
	...
	Game_Actor.prototype.isSpriteVisible = function () {
		return params.ActorEffectShow && !$gameSystem.isSideView()
			? $gameParty.inBattle()
				? true
	...
	Scene_Battle.prototype.getActorCommandX = function () {
		if (
			...
			return (
				params.ActorCommand_X + (params.ActorommandWindowCenter ? this.actorCommandCenter() : 0)
			)
	...
	Scene_Battle.prototype.createItemWindow = function () {
		...
		if (params.ItemWindowBackgroundImg) {
			const x =
				params.ItemBackground_X + this._itemWindow.x + (Graphics.width - Graphics.boxWidth) / 2
			const y =
				params.ItemBackground_Y + this._itemWindow.y + (Graphics.height - Graphics.boxHeight) / 2
	...
	Scene_Battle.prototype.createSkillWindow = function () {
		...
		if (params.SkillWindowBackgroundImg) {
			const x =
				params.SkillBackground_X + this._skillWindow.x + (Graphics.width - Graphics.boxWidth) / 2
			const y =
				params.SkillBackground_Y + this._skillWindow.y + (Graphics.height - Graphics.boxHeight) / 2
	...
	Scene_Battle.prototype.createHelpWindow = function () {
		...
		if (params.HelpWindowBackgroundImg) {
			const x =
				params.HelpBackground_X + this._helpWindow.x + (Graphics.width - Graphics.boxWidth) / 2
			const y =
				params.HelpBackground_Y + this._helpWindow.y + (Graphics.height - Graphics.boxHeight) / 2
	...
	Scene_Battle.prototype.itemWindowRect = function () {
		const wx =
			params.ItemWindow_X + (params.ItemWindowMode ? 0 : (Graphics.boxWidth - Graphics.width) / 2)
		const ww =
			params.ItemWindow_Width > 0
				? Math.min(params.ItemWindow_Width, Graphics.width)
				: Graphics.boxWidth
		const wh = this.calcWindowHeight(params.ItemMaxRow, true)
		const wy =
			params.ItemWindow_Y +
			(params.ItemWindowMode ? Graphics.boxHeight - wh : (Graphics.boxHeight - Graphics.height) / 2)
		return new Rectangle(wx, wy, ww, wh)
	}

	Scene_Battle.prototype.skillWindowRect = function () {
		const wx =
			params.SkillWindow_X + (params.SkillWindowMode ? 0 : (Graphics.boxWidth - Graphics.width) / 2)
		const ww =
			params.SkillWindow_Width > 0
				? Math.min(params.SkillWindow_Width, Graphics.width)
				: Graphics.boxWidth
		const wh = this.calcWindowHeight(params.SkillMaxRow, true)
		const wy =
			params.SkillWindow_Y +
			(params.SkillWindowMode
	...
	Scene_Battle.prototype.enemyWindowRect = function () {
		//再定義
		const wx =
			params.EnemyWindow_X + (params.EnemyWindowMode ? 0 : (Graphics.boxWidth - Graphics.width) / 2)
	...
	Scene_Battle.prototype.statusWindowRect = function () {
		...
		let wx =
			...
			(this.getDefaultPartyCommandPositionMode()
				? this.isRightInputMode()
					? 0
					: getActorWindowOrgWidth() - ww
				: 0)
	...
	Scene_Battle.prototype.bsMessageBackground = function () {
		if (BattleManager.getDisplayMessageType() === 'Appear' && params.AppearBackgroundImg) {
			...
			this._messageBackground.x =
				this._messageWindow.x + params.AppearBackground_X + (Graphics.width - Graphics.boxWidth) / 2
			...
			this._messageBackground.x =
				this._messageWindow.x + params.LoseBackground_X + (Graphics.width - Graphics.boxWidth) / 2
			this._messageBackground.y =
				this._messageWindow.y + params.LoseBackground_Y + (Graphics.height - Graphics.boxHeight) / 2
			...
			this._messageBackground.x =
				this._messageWindow.x + params.EscapeBackground_X + (Graphics.width - Graphics.boxWidth) / 2
			this._messageBackground.y =
				this._messageWindow.y +
	...
	Scene_Battle.prototype.statusWindowX = function () {
		...
		if (this.isAnyInputWindowActive()) {
			wx += this.getDefaultPartyCommandPositionMode()
				? this.isRightInputMode()
					? 0
					: getActorWindowOrgWidth() - this.getActorWindowWidth()
				: 0
		} else {
	...
	Scene_Battle.prototype.getActorWindowWidth = function () {
		return (
			(params.ActorStatusWindow_Width > 0 ? params.ActorStatusWindow_Width : Graphics.boxWidth) -
	...
	Window_PartyCommand.prototype.itemRect = function (index) {
		...
		if (params.PartyCommandMode) {
			rect.x +=
				(this.itemWidth() / 2) * (this.maxCols() - Math.min(this.maxItems(), this.maxCols()))
		}
		return rect
	}
		...
		if ((statusData && this._actor) || actorIndex >= 0) {
			...
			if (params.ActorCommandPosition === 'actor') {
				if (Imported.NUUN_SupportActor && this._actor.getSupportActor()) {
					...
					this.x =
						rect.x +
						(statusData.x - (Graphics.width - Graphics.boxWidth) / 2) +
	...
	Window_BattleStatus.prototype.itemRect = function (index) {
		let rect =
			params.ActorStatusMode === 'triangle'
				? this.triangleRect(index)
				: Window_Selectable.prototype.itemRect.call(this, index)
	...
	Window_Selectable.prototype.triangleRect = function (index) {
		...
		const row = Math.floor(
			((topCol > index || topCol === 0 ? 0 : maxCols - topCol) + index + shiftIndex) / maxCols
		)
	...
	Window_BattleStatus.prototype.faceRect = function (index) {
		...
		if (params.bsMode === 'Standard') {
			...
			height =
				params.FaceHeight > 0
					? Math.min(params.FaceHeight, ImageManager.faceHeight)
					: this.basicGaugesY(rect) - this.gaugeLineHeight() + this.gaugeLineHeight() / 2 - rect.y
			rect.height = params.FaceHeightOnWindow ? Math.min(rect.height, height) : height
		}
	...
	Window_BattleStatus.prototype.drawActorIcons = function (actor, x, y, width, data) {
		...
		sprite.move(x, y)
		sprite.setupVisibleIcons(
			this.getVisibleIcons(data.DetaEval1),
			this.getVisibleIcons(data.DetaEval2)
		)
		sprite.show()
	}
	...
	Window_BattleStatus.prototype.bs_PlaceActorName = function (actor, x, y) {
		...
		const sprite = $gameTemp.userStatusParam
			? this.createInnerSprite(key, Sprite_BSName)
			: this.createInnerSprite(key, Sprite_Name)
		sprite.setup(actor)
		sprite.move(x, y)
		sprite.show()
	}
	...
	Window_BattleStatus.prototype.placeStateIcon = function (actor, x, y, data) {
		const key = 'actor%1-stateIcon%2'.format(
			actor.actorId(),
			data && data.UserParamID ? data.UserParamID : '_bsStateIcon'
		)
	...
	Window_BattleStatus.prototype.placeGauge = function (actor, type, x, y) {
		...
		const sprite =
			$gameTemp.userStatusParam || $gameTemp.actorData
				? this.createInnerSprite(key, Sprite_BSGauge)
				: this.createInnerSprite(key, Sprite_Gauge)
	...
	Window_BattleActorImges.prototype.drawItemStatus = function (index) {
		//再定義
		const actor = this.actor(index)
	...
	Window_BattleActorImges.prototype.drawItemBattler = function (index, actor) {
		...
		if (bitmap) {
			...
			const x =
				rect.x +
				(actorImgData.ActorImgHPosition === 'center' ? Math.floor(this.itemWidth() / 2) + 4 : 8) +
			...
			if (params.Img_SW > 0 || params.Img_SH > 0) {
				...
				const sx =
					...
					(actorImgData.ActorImgHPosition === 'center' ? (bitmap.width - sw) / 2 : 0)
	...
	Window_BattleActorImges.prototype.drawItemFace = function (index, actor) {
		...
		const y =
			rect.y +
			(actorImgData.ActorImgVPosition === 'under' ? sh : 1) +
			(positionData.ImgChangePosition ? positionData.ActorImg_Y : 0) +
			(actorImgData ? actorImgData.Actor_Y : 0)
		sprite.setHome(x, y)
	...
	Window_BattleActorImges.prototype.placeStateIcon = function (actor, x, y, data) {
		const key = 'actor%1-stateIcon%2'.format(
			actor.actorId(),
			data ? data.UserParamID || 'dparam' : 'dparam'
		)
	...
	Window_BattleActorStatus.prototype.initialize = function (rect) {
		...
		this._opening = true
		this.visible = true
		BattleManager.gaugeMaxWidth = params.ActorStatusVariable
			? this.width
			: this.itemRectWithPadding(0).width
	...
	Sprite_BSFrontActor.prototype.damageOffsetX = function () {
		return (
			(this.viewFrontActor ? 0 : Sprite_Actor.prototype.damageOffsetX.call(this)) +
			params.ActorDamage_X
		)
	}

	Sprite_BSFrontActor.prototype.damageOffsetY = function () {
		return (
			(this.viewFrontActor ? 0 : Sprite_Actor.prototype.damageOffsetY.call(this)) +
	...
	Sprite_Enemy.prototype.startBossCollapse = function () {
		...
		if (
			Imported.NUUN_GamePadVibration &&
			params.BossCollapseVibration &&
			params.BossCollapseVibrationSetting
		) {
	...
	Sprite_Animation.prototype.setupFrontTargets = function (targets) {
		if (!$gameSystem.isSideView() && params.ActorEffectShow) {
			this._frontTargets = targets.map((sprite) =>
	...
	Sprite_ActorImges.prototype.refreshStateOverlay = function () {
		if (this._stateSprite && getStateAnimationShow()) {
			...
			if (this._loadedBitmap && this._loadedBitmap.isReady()) {
				...
				this._stateSprite.y =
					((this.anchor.y === 0.5 ? (this._bsBitmapHeight / 2) * scale : 0) + y) * scale
			}
		}
	}
		...
		if (this._loadedBitmap && this._loadedBitmap.isReady()) {
			if (this._zoomDuration > 0) {
				...
				if (this.x === this._homeX) {
					this.x =
						this._homeX +
						(this._data.ActorImgHPosition === 'left' ? (this._bsBitmapWidth / 2) * scale : 0)
				}
				if (this.y === this._homeY) {
					this.y =
						this._homeY -
						(this._bsBitmapHeight / 2) * scale * (this._data.ActorImgVPosition === 'top' ? -1 : 1)
				}
			} else {
	...
	Sprite_ActorImges.prototype.isCounter = function () {
		return (
			this._imgScenes === 'counter' ||
			this._imgScenes === 'reflection' ||
			this._imgScenes === 'counterEX'
		)
	}
	...
	Sprite_ActorImges.prototype.setActorGraphic = function (actor, bitmap) {
		const pass =
			Imported.NUUN_ActorPicture && params.ActorPictureEXApp
				? actor.getActorGraphicImg()
				: actor.getBSImgName()
		...
		if (name && this.addApngChild && this.loadApngSprite(name)) {
			...
			if (this.isDead()) {
				this._actorImgesOpacity = this.isActorGraphicDead()
					? this.opacity - actor.getBattleStyleOpacity()
					: this.opacity - 0
	...
	Sprite_ActorImges.prototype.imgFrameRefresh = function () {
		...
		if (params.Img_SW > 0 || params.Img_SH > 0) {
			...
			const sx =
				...
				(this._data.ActorImgHPosition === 'center' ? (this.bitmap.width - sw) / 2 : 0)
	...
	Sprite_BSGauge.prototype.bitmapBaseWidth = function () {
		return params.ActorStatusVariable
			? BattleManager.gaugeMaxWidth
			: Math.min(BattleManager.gaugeMaxWidth, BattleManager.rectMaxWidth)
	}
	...
	Sprite_BSGauge.prototype.currentMaxValue = function () {
		if (this._battler && this.userStatusParam) {
			switch (this._statusType) {
				...
				default:
					return this.userStatusParam.DetaEval2
						? eval(this.userStatusParam.DetaEval2)
						: Sprite_Gauge.prototype.currentMaxValue.call(this)
			}
		} else {
			return Sprite_Gauge.prototype.currentMaxValue.call(this)
		}
	...
	Sprite_BSStateIcon.prototype.updateFrame = function () {
		this._index =
			this._battler && this._battler.isActor() && params.NoneStateIcon > 0
				? params.NoneStateIcon
				: this._index
