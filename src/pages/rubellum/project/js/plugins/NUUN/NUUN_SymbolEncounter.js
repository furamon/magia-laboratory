...
;(() => {
	...
	const SymbolEncountSetting =
		NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['SymbolEncountSetting']) : []
	...
	Game_Event.prototype.setSymbolEnc = function (eventId) {
		if ($dataMap) {
			this._symbolEnc = $dataMap.events[eventId]
				? Number($dataMap.events[eventId].meta.SymbolEncEnemy)
				: 0
		} else {
			this._symbolEnc = 0
		}
	...
	Game_Character.prototype.searchLimit = function () {
		return this.isSymbolEnc && this.isSymbolEnc()
			? PlayerSearchLimit
			: _Game_Character_searchLimit.call(this)
	}
			...
			try {
				...
				const log =
					($gameSystem.isJapanese()
						? '無効なシンボルエンカウントIDが設定されています。'
						: 'An invalid symbol encounter ID has been set.') +
					'(ID:' +
					this._symbolEnc +
					')'
				throw [log]
	...
	Game_Event.prototype.updateSymbolEncount = function () {
		if (this.isSymbolEnc() && !this.symbolEncTriggerOn) {
			if (
				(this.isMoving() || $gamePlayer.isMoving()) &&
	...
	Game_Event.prototype.isTriggerIn = function (triggers) {
		return (
			_Game_Event_isTriggerIn.call(this, triggers) &&
			(this.isSymbolEnc() ? !$gamePlayer.isSymbolEncInvincible() : true)
		)
	}
	...
	Game_Event.prototype.getSymbolEncFind = function () {
		...
		if (data.FindCommonEvent > 0) {
			page.list = [
				{ code: 117, indent: 0, parameters: [data.FindCommonEvent] },
	...
	Game_Event.prototype.getSymbolEncBattle = function () {
		...
		if (data.BattleCommonEvent > 0) {
			page.list = [
				{ code: 117, indent: 0, parameters: [data.BattleCommonEvent] },
				{ code: 0, indent: 0, parameters: null }
			]
		}
		page.trigger = 3
	...
	Game_Event.prototype.getSymbolEncLose = function () {
		...
		if (data.LoseCommonEvent > 0) {
			page.list = [
				{ code: 117, indent: 0, parameters: [data.LoseCommonEvent] },
				{ code: 0, indent: 0, parameters: null }
			]
		} else {
	...
	Game_Event.prototype.getSymbolVictory = function () {
		...
		if (data.DefeatCommonEvent > 0) {
			page.list = [
				{ code: 117, indent: 0, parameters: [data.DefeatCommonEvent] },
				{ code: 0, indent: 0, parameters: null }
			]
		} else {
	...
	Game_Event.prototype.getSymbolPlayerEscape = function () {
		...
		if (data.PlayerEscapeCommonEvent > 0) {
			page.list = [
				{ code: 117, indent: 0, parameters: [data.PlayerEscapeCommonEvent] },
				{ code: 0, indent: 0, parameters: null }
			]
		} else {
	...
	Game_Character.prototype.getSymbolEncSight = function () {
		return $gameParty.hasEncounterNone()
			? 0.0
			: $gamePlayer.encounterProgressValue() *
					($gamePlayer.isOnBush() ? this._symbolEncSightBush / 2 : 1.0)
	}
	...
	Game_Character.prototype.symbolEncountFindPlayer = function (x, y, e) {
		...
		if (data && this.isSymbolEncSightMode() && !$gamePlayer.isSymbolEncInvincible()) {
			...
			if (rate > 0) {
				if (mode === 'range') {
					result =
						this.rangeX(x, Math.floor(Number(arr[1]) * rate)) &&
						this.rangeY(y, Math.floor(Number(arr[2]) * rate))
				} else if (mode === 'besideRange') {
					result = this.besideRange(
						this.deltaXFrom(x),
						y,
						Math.floor(Number(arr[1]) * rate),
						Math.floor(Number(arr[2]) * rate)
					)
				} else if (mode === 'verticalRange') {
					result = this.verticalRange(
						x,
						this.deltaYFrom(y),
						Math.floor(Number(arr[1]) * rate),
						Math.floor(Number(arr[2]) * rate)
					...
					result = this.rangeEX(
						...
						Math.floor(Number(arr[1]) * rate),
						Math.floor(Number(arr[2]) * rate),
						Math.floor(Number(arr[3]) * rate),
						Math.floor(Number(arr[4]) * rate),
						Math.floor(Number(arr[5]) * rate),
						Math.floor(Number(arr[6]) * rate),
						Math.floor(Number(arr[7]) * rate),
						Math.floor(Number(arr[8]) * rate)
					)
				} else if (mode === 'circle') {
	...
	function getVictoryCommand(data) {
		...
		list.push({
			code: 205,
			indent: 0,
			parameters: [0, { list: getDefeatCommand(), repeat: false, skippable: false, wait: true }]
		})
	...
	function getDefeatCommand() {
		return [
			{ code: 42, parameters: [192], indent: 0 },
	...
	function getPlayerEscapeCommand(data) {
		...
		if (EventFreezesFrame > 0) {
			list.push({
				...
				parameters: [
					0,
					{ list: getEscapeCommand(data), repeat: false, skippable: false, wait: true }
				]
			})
		}
		list.push({ code: 0, indent: 0, parameters: null })
