...
;(() => {
	...
	const UserKey =
		(NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['UserKey']) : null) || []
		...
		if (this.isOpenAndActive()) {
			for (const data of UserKey) {
				if ((data.UserKey && data.UserKey.KeyCode >= 0) || data.UserKey.GamePadCode >= 0) {
					...
					if (
						!this.isDefaultProcessHandling(keyName) &&
						this.isHandled(keyName) &&
						isRepeated(data.UserKey.Repeated, keyName)
					) {
	...
	Scene_Base.prototype.updateUserKey = function () {
		...
		for (const data of UserKey) {
			if (
				data.UserKey &&
				this.isValidCond(data.UserKey, _className) &&
				(!!data.UserKey.KeySprict || data.UserKey.KeyCommonEvent > 0) &&
				(data.UserKey.KeyCode >= 0 || data.UserKey.GamePadCode >= 0)
			) {
				const keyName = data.UserKey.KeyName
