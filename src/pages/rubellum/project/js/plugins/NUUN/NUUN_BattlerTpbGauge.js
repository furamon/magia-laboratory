...
;(() => {
	...
	Sprite_Enemy.prototype.setTpbGaugePosition = function () {
		if (this._battlerTpb) {
			...
			const x =
				(enemy.meta.TPBGaugeX ? Number(enemy.meta.TPBGaugeX) : 0) +
				Gauge_X +
				this._enemy.getTpbGaugePositionX()
			const y =
				(enemy.meta.TPBGaugeY ? Number(enemy.meta.TPBGaugeY) : 0) +
				Gauge_Y +
				this._enemy.getTpbGaugePositionY()
