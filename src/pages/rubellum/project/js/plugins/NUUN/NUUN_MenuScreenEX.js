...
;(() => {
	...
	const BackGroundImges =
		(NUUN_Base_Ver >= 113 ? DataManager.nuun_structureData(parameters['BackGroundImges']) : null) ||
		[]
	...
	Scene_MenuBase.prototype.nuun_statusWindowX = function () {
		switch (this._menuLayout.StatusWindowBesidePosition) {
			...
			case 'left':
				return (
					this._menuLayout.MenuStatusX +
					(this.isWindowUiIgnore() ? (Graphics.boxWidth - Graphics.width) / 2 : 0)
				)
			case 'right':
				return (
					(this.isWindowUiIgnore()
						? Graphics.width + (Graphics.boxWidth - Graphics.width) / 2
						: Graphics.boxWidth) -
					this.nuun_infoWindowWidth(data) +
					this._menuLayout.MenuStatusX
				)
			case 'command':
				return (
					this._menuLayout.MenuStatusX +
					this.nuun_mainCommandX() +
					(this.isRightInputMode() ? -this.nuun_statusWindowWidth() : this.nuun_mainCommandWidth())
				)
		}
	}
	...
	Scene_MenuBase.prototype.nuun_statusWindowY = function () {
		switch (this._menuLayout.StatusWindowVerticalPosition) {
			...
			case 'top':
				return (
					this._menuLayout.MenuStatusY +
					(this.isWindowUiIgnore() ? 0 : this.nuun_menuHelpAreaHeight())
				)
			case 'under':
				return (
					(this.isWindowUiIgnore()
						? Graphics.height + (Graphics.boxHeight - Graphics.height) / 2
						: Graphics.boxHeight) -
					this.nuun_infoWindowHeight(data) +
					this._menuLayout.MenuStatusY
				)
			case 'commandtop':
				return (
					this._menuLayout.MenuStatusY + this.nuun_mainCommandY() - this.nuun_statusWindowHeight()
				)
			case 'commandunder':
				return (
					this._menuLayout.MenuStatusY + this.nuun_mainCommandY() + this.nuun_mainCommandHeight()
				)
		}
	}

	Scene_MenuBase.prototype.nuun_statusWindowWidth = function () {
		return this.isWindowUiIgnore()
			? this._menuLayout.MenuStatusWidth > 0
				? this._menuLayout.MenuStatusWidth
				: Graphics.width
			: (this._menuLayout.MenuStatusWidth > 0
					? this._menuLayout.MenuStatusWidth
					: Graphics.boxWidth) +
					(this.nuun_statusWindowIsCommandBesideDifference() ? -this.nuun_mainCommandWidth() : 0)
	}

	Scene_MenuBase.prototype.nuun_statusWindowHeight = function () {
		return (
			(this._menuLayout.MenuStatusHeight > 0
				? this._menuLayout.MenuStatusHeight
				: this.isWindowUiIgnore()
					? Graphics.height
					: Graphics.boxHeight - this.nuun_menuHelpAreaHeight()) +
			(this.nuun_statusWindowIsCommandVerticalDifference() ? -this.nuun_mainCommandHeight() : 0)
		)
	}
	...
	Scene_Menu.prototype.nuun_infoWindowX = function (data) {
		switch (data.WindowBesidePosition) {
			...
			case 'left':
				return (
					data.X_Position + (this.isWindowUiIgnore() ? (Graphics.boxWidth - Graphics.width) / 2 : 0)
				)
			case 'right':
				return (
					(this.isWindowUiIgnore()
			...
			case 'right':
				return (
					(this.isWindowUiIgnore()
						...
						: this.nuun_menuHelpAreaHeight()) + this._menuLayout.MenuCommandY
				)
			case 'under':
				return (
					(this.isWindowUiIgnore()
	...
	Scene_MenuBase.prototype.nuun_mainCommandHeight = function () {
		const m = this._menuLayout
		return m.CommandHeightMode
			? m.MenuCommandRows > 0
				? this.nuun_mainCommandAreaHeight(m.MenuCommandRows)
				: this.mainAreaHeight()
			: m.MenuCommandHeight > 0
				? m.MenuCommandHeight
				: this.mainAreaHeight()
	}
	...
	Scene_Menu.prototype.isBesideMenuCommand = function () {
		return (
			this._menuLayout.MenuCommandPosition === 'under' ||
			this._menuLayout.MenuCommandPosition === 'top'
		)
	}
		...
		if (this._menuLayout && this._menuLayout.BackGroundImg) {
			...
			if (sprite.bitmap && !sprite.bitmap.isReady()) {
				sprite.bitmap.addLoadListener(
					this.setBackGround.bind(this, sprite, this._menuLayout.BackUiWidth2)
				)
			} else {
	...
	Window_StatusBase.prototype.nuunMenu_contentsDrawActorFace = function (
		actor,
		data,
		x,
		y,
	...
	Window_MenuStatus.prototype.drawActorBack = function (bitmap, index) {
		const rect = this.itemRect(index)
		this.contentsBack.nuun_contentsBackBlt(
			bitmap,
			0,
			0,
			rect.width,
	...
	Window_MenuStatus.prototype.nuunMenu_drawItemContentsStatus = function (index) {
		...
		for (const data of list) {
			...
			const contentsX =
				rect.x + (itemWidth + colSpacing) * (position - 1) + data.X_Coordinate + colSpacing
			const contentsY =
				rect.y + lineHeight * (data.Y_Position - 1) + data.Y_Coordinate + this.itemPadding()
			const width =
				data.ItemWidth && data.ItemWidth > 0 ? Math.min(data.ItemWidth, itemWidth) : itemWidth
			data._width =
				data.ItemWidth && data.ItemWidth > 0
					? Math.min(data.ItemWidth, width)
					: Math.min(width, 128)
			...
			try {
				...
				const log =
					($gameSystem.isJapanese()
						? '無効なIDが設定されています。'
						: 'An invalid ID has been configured.') + data.DateSelect
				throw ['DataError', log]
			}
		}
	}
	...
	Window_StatusBase.prototype.nuun_DrawMenuStatusContentsNickname = function (
		data,
		x,
		y,
		width,
	...
	Window_StatusBase.prototype.nuun_DrawMenuStatusContentsClass = function (
		data,
		x,
		y,
		width,
	...
	Window_StatusBase.prototype.nuun_DrawMenuStatusContentsLevel = function (
		data,
		x,
		y,
		width,
	...
	Window_StatusBase.prototype.nuun_DrawMenuStatusContentsState = function (
		data,
		x,
		y,
		width,
	...
	Window_StatusBase.prototype.nuun_DrawMenuStatusContentsState2 = function (
		data,
		x,
		y,
		width,
	...
	Window_StatusBase.prototype.nuun_DrawMenuStatusContentsOrgParam = function (
		data,
		x,
		y,
		width,
		...
		const textWidth =
			data.Align === 'left' && data.SystemItemWidth === 0
				...
				: this.nuunMenu_systemWidth(data.SystemItemWidth, width)
	...
	Window_MenuStatus.prototype.nuun_DrawMenuStatusContentsHpGauge = function (
		data,
		x,
		y,
		width,
	...
	Window_MenuStatus.prototype.nuun_DrawMenuStatusContentsMpGauge = function (
		data,
		x,
		y,
		width,
	...
	Window_MenuStatus.prototype.nuun_DrawMenuStatusContentsTpGauge = function (
		data,
		x,
		y,
		width,
	...
	Window_MenuStatus.prototype.nuun_DrawMenuStatusContentsExpGauge = function (
		data,
		x,
		y,
		width,
	...
	Window_MenuStatus.prototype.nuun_DrawMenuStatusContentsCharchip = function (
		data,
		x,
		y,
		width,
	...
	Window_MenuStatus.prototype.nuun_DrawMenuStatusContentsSvActor = function (
		data,
		x,
		y,
		width,
	...
	Window_StatusBase.prototype.nuun_DrawMenuStatusContentsImges = function (
		data,
		x,
		y,
		width,
	...
	Window_StatusBase.prototype.nuun_DrawMenuStatusContentsCritcalEvade = function (
		data,
		x,
		y,
		width,
	...
	Window_StatusBase.prototype.nuun_DrawMenuStatusContentsMagicEvade = function (
		data,
		x,
		y,
		width,
	...
	Window_StatusBase.prototype.nuun_DrawMenuStatusContentsMagicrEflect = function (
		data,
		x,
		y,
		width,
	...
	Window_StatusBase.prototype.nuun_DrawMenuStatusContentsCounter = function (
		data,
		x,
		y,
		width,
	...
	Window_StatusBase.prototype.nuun_DrawMenuStatusContentsHpRegen = function (
		data,
		x,
		y,
		width,
	...
	Window_StatusBase.prototype.nuun_DrawMenuStatusContentsMpRegen = function (
		data,
		x,
		y,
		width,
	...
	Window_StatusBase.prototype.nuun_DrawMenuStatusContentsTpRegen = function (
		data,
		x,
		y,
		width,
	...
	Window_StatusBase.prototype.nuun_DrawMenuStatusContentsAggro = function (
		data,
		x,
		y,
		width,
	...
	Window_StatusBase.prototype.nuun_DrawMenuStatusContentsGuard = function (
		data,
		x,
		y,
		width,
	...
	Window_StatusBase.prototype.nuun_DrawMenuStatusContentsRecovery = function (
		data,
		x,
		y,
		width,
	...
	Window_StatusBase.prototype.nuun_DrawMenuStatusContentsItemEffect = function (
		data,
		x,
		y,
		width,
	...
	Window_StatusBase.prototype.nuun_DrawMenuStatusContentsMpCost = function (
		data,
		x,
		y,
		width,
	...
	Window_StatusBase.prototype.nuun_DrawMenuStatusContentsTpCharge = function (
		data,
		x,
		y,
		width,
	...
	Window_StatusBase.prototype.nuun_DrawMenuStatusContentsPhysicalDamage = function (
		data,
		x,
		y,
		width,
	...
	Window_StatusBase.prototype.nuun_DrawMenuStatusContentsMagicalDamage = function (
		data,
		x,
		y,
		width,
	...
	Window_StatusBase.prototype.nuun_DrawMenuStatusContentsFloorDamage = function (
		data,
		x,
		y,
		width,
	...
	Window_StatusBase.prototype.nuun_DrawMenuStatusContentsGainExpRate = function (
		data,
		x,
		y,
		width,
	...
	Window_StatusBase.prototype.nuunMenu_drawParams = function (data, param, x, y, width, actor) {
		...
		this.nuun_setContentsValueFontFace(data)
		const textParam =
			(data.DetaEval ? eval(data.DetaEval) : actor.param(param)) +
	...
	Window_StatusBase.prototype.nuunMenu_contentsDrawActorGraphic = function (
		actor,
		data,
		bitmap,
		x,
	...
	Window_InfoMenu.prototype.nuun_DrawMenuInfoContentsGold = function (data, x, y, width) {
		...
		this.contents.fontSize =
			$gameSystem.mainFontSize() + this._infoFontSize + (data.ContentsFontSize || 0)
		...
		this.drawCurrencyValue(
			this.value(),
			this.currencyUnit(),
			x + systemWidth + 8 + iconWidth,
			y,
			width - (systemWidth + 8 + iconWidth)
		)
		this.resetFontSettings()
	}
	...
	Window_InfoMenu.prototype.nuun_DrawMenuInfoContentsPlaytime = function (data, x, y, width) {
		...
		this.drawText(
			...
			x + systemWidth + iconWidth + this.itemPadding(),
			y,
			width - (systemWidth + 8 + iconWidth),
			data.Align
		)
	...
	Window_InfoMenu.prototype.nuun_DrawMenuInfoContentsCurrentLocation = function (
		data,
		x,
		y,
		width
	...
	Window_InfoMenu.prototype.nuun_DrawMenuInfoContentsCommandDescription = function (
		data,
		x,
		y,
		width
	...
	Sprite_MenuGauge.prototype.gaugeColor1 = function () {
		if (this._battler && this.menuParam) {
			switch (this._statusType) {
				case 'menuexp':
					return this.menuParam.Color1 >= 0
						? NuunManager.getColorCode(this.menuParam.Color1)
						: NuunManager.getColorCode(17)
				default:
					return this.menuParam.Color1 >= 0
						? NuunManager.getColorCode(this.menuParam.Color1)
						: Sprite_Gauge.prototype.gaugeColor1.call(this)
			}
		} else {
	...
	Sprite_MenuGauge.prototype.gaugeColor2 = function () {
		if (this._battler && this.menuParam) {
			switch (this._statusType) {
				case 'menuexp':
					return this.menuParam.Color2 >= 0
						? NuunManager.getColorCode(this.menuParam.Color2)
						: NuunManager.getColorCode(6)
				default:
					return this.menuParam.Color2 >= 0
						? NuunManager.getColorCode(this.menuParam.Color2)
						: Sprite_Gauge.prototype.gaugeColor2.call(this)
			}
		} else {
	...
	Sprite_MenuGauge.prototype.displyaExp = function () {
		...
		if (mode === 1) {
			...
			return NuunManager.numPercentage(
				...
				this.expDecimalParam(),
				this.decimalModeParam()
			)
		}
			...
			switch (this._statusType) {
				...
				case 'time':
					return this.menuParam.ParamName
						? this.menuParam.ParamName
						: Sprite_Gauge.prototype.label.call(this)
				case 'menuexp':
					return this.labelShowParam()
						? this.menuParam.ParamName
							? this.menuParam.ParamName
							: TextManager.expA
						: ''
	...
	Sprite_MenuScreenCharacter.prototype.changePaintOpacity = function () {
		const battleMemberOpacity =
			this._data.BattleMemberOpacity !== undefined ? this._data.BattleMemberOpacity : true
		if (battleMemberOpacity) {
			this.opacity = this._actor.isBattleMember()
				? 255
				: Window_Base.prototype.translucentOpacity.call(this)
		}
	}
	...
	Sprite_MenuActorImg.prototype.setup = function (battler, data) {
		...
		this._data = data
		const name =
			Imported.NUUN_ActorPicture && _menuLayout.ActorPictureEXApp
