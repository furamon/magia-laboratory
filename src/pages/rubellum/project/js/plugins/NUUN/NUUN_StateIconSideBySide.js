...
;(() => {
	...
	Sprite_StateIcon.prototype.createStateIcons = function (icons, turns) {
		...
		this.displayIconsLength =
			!this._isEnemyMode && NoStateIcon > 0 ? ActorStateIconShowVal : displayIcons.length
	}
	...
	Game_BattlerBase.prototype.nuun_stateTurns = function () {
		return this.states().reduce((r, state) => {
			if (state.iconIndex > 0) {
				const turn = [
					{
						turn: this.nuun_isNonRemoval(state)
							? 0
							: this.nuun_getStateTurn(state.id) + (state.autoRemovalTiming === 2 ? -1 : 0),
						bad: !!state.meta.BatState
					}
				]
