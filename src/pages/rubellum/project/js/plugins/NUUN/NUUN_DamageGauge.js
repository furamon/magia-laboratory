...
;(() => {
	...
	const DamageGaugeSetting =
		(NUUN_Base_Ver >= 113
			? DataManager.nuun_structureData(parameters['DamageGaugeSetting'])
			: null) || []
	...
	Sprite_Gauge.prototype.getFindGaugeDamageData = function (statusType) {
		return DamageGaugeSetting.find(
			(data) =>
				(data.Type === statusType || data.Type === 'all') &&
				this.filteringGaugeDamageDataClass(data)
		)
	}
	...
	Sprite_Gauge.prototype.drawGaugeRect = function (x, y, width, height) {
		if (this.gaugeDamageVisualization() && !(Imported.NUUN_GaugeImage && this._gaugeImgData)) {
			...
			if (LLparameters && LLSolidGradation) {
				//if内はルルの協会様のコードをお借りしています。
