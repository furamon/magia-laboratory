...
;(function () {
	...
	HzTimingBar.prototype.initialize = function (x, y, hitArea, criticalArea, requiredAreas, varNo) {
		...
		for (var i = 0; i < this._requiredAreas.length; i++) {
			...
			var reqWidth =
				(HzTimingBar.WIDTH * (requiredArea[1] - requiredArea[0])) / HzTimingBar.maxFrame
		...
		// ヒットエリア
		var hitWidth =
			(HzTimingBar.WIDTH * (this._hitArea[1] - this._hitArea[0])) / HzTimingBar.maxFrame
		...
		// クリティカルエリア
		var criticalWidth =
			(HzTimingBar.WIDTH * (this._criticalArea[1] - this._criticalArea[0])) / HzTimingBar.maxFrame
