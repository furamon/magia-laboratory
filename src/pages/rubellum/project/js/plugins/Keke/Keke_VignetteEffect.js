 ...
 *
 * @arg ピクチャ使用
 * @desc usePicture ピクチャを使用する。プラグインパラメータでピクチャを登録→その名前を書く。\v[番号]で変数を使用可能
 *
 * @arg 画像サイズ
 ...
 *
 * @arg 呼び出すコモン
 * @desc callCommon 呼び出すコモン。プラグインパラメータ→ビネットコモン で登録し、そのコモン名を書く
 *
 * @arg …取得ポイント
 ...
 *
 * @arg 呼び出すコモン
 * @desc callCommon 呼び出すコモンの名前。プラグインパラメータ→ビネットコモン で登録し、そのコモン名を書く
 *
 * @arg …開始ポイント
 ...
 *
 * @param ピクチャ使用
 * @desc usePicture ピクチャを使用する。プラグインパラメータでピクチャを登録→その名前を書く
 * 
 * @arg 画像サイズ
 * @desc imageSizeRate ビネットとして表示する画像のサイズ。3 なら画面サイズの 3倍。空欄だと 3
 * 
...
;(() => {
	...
	function toBoolean(str) {
		...
		if (str2 == 'true' || str2 == 'on' || Number(str2)) {
			return true
		}
	...
	//- ピクチャ登録
	let keke_pictureList = parameters['ピクチャ登録']
		? JSON.parse(parameters['ピクチャ登録']).map((d) => JSON.parse(d))
		: []

	for (let i = 1; i <= 9; i++) {
		const pictures = parameters['ピクチャ_' + i]
			? JSON.parse(parameters['ピクチャ_' + i]).map((d) => JSON.parse(d))
			: []
		keke_pictureList = [...keke_pictureList, ...pictures]
	}

	//- コモン登録
	const keke_commonList = parameters['コモン登録']
		? JSON.parse(parameters['コモン登録']).map((d) => JSON.parse(d))
		: []
	...
	PluginManager.registerCommand(pluginName, 'ビネット表示', (args) => {
		...
		if (time <= 1 || toBoolean(args2['…フェード中ウェイト'])) {
			preter.wait(time)
		}
		args2.preter = preter
		// ビネットの開始
	...
	PluginManager.registerCommand(pluginName, 'アイリスエフェクト', (args) => {
		...
		if (time <= 1 || toBoolean(args2['…動作中ウェイト'])) {
			setIrisWait(preter)
		}
		args2.preter = preter
		// アイリスエフェクトの開始
	...
	function setIrisWait(preter) {
		...
		if (!preter._irisEffectWaitKe) {
			preter._irisEffectWaitKe = 0
		}
	...
	function setControlWait(preter) {
		...
		if (!preter._vignetteControlWaitKe) {
			preter._vignetteControlWaitKe = 0
		}
	...
	Game_Interpreter.prototype.updateWaitMode = function () {
		...
		if (this._irisEffectWaitKe || this._vignetteControlWaitKe) {
			result = true
		}
		return result
	}
	...
	function clearVignette(gm, fadeTime, isAuto, sceneType) {
		...
		vignettes.forEach((vnt, i) => {
			...
			if (isAuto && (vnt.takeOver || vnt.sceneName != sceneName)) {
				return
			}
			vnt.delFlag = true
			// フェードのセット
	...
	function destroySprite(sprite) {
		if (!sprite || sprite == 'nothing') {
			return
		}
		sprite.children.forEach((s) => destroySprite(s))
		if (sprite.constructor.name == 'Sprite_Character') {
			return
		}
	...
	function initVignetteSprites(i) {
		const gt = $gameTemp
		if (!gt._vignetteSpritesKe) {
			gt._vignetteSpritesKe = []
		}
	}
	...
	function tempChildVignetteSprite(sprite, force) {
		...
		const gt = $gameTemp
		if (!gt._vignetteTempLayerKe) {
			gt._vignetteTempLayerKe = new PIXI.Container()
		}
	...
	//- ビネットレイヤーの作成
	function createVignetteLayer(spriteset) {
		...
		for (let i = 0; i < 3; i++) {
			...
			const field =
				scene.constructor.name == 'Scene_Map'
					? spriteset._tilemap
					: scene.constructor.name == 'Scene_Battle'
	...
	function childVignette(sprite) {
		...
		const scene = SceneManager._scene
		if (!scene._vignetteLayersKeA) {
			createVignetteLayer(SceneManager._scene._spriteset)
		}
	...
	Game_Interpreter.prototype.loadImages = function () {
		...
		if (!this._regKe) {
			// リストからのピクチャロード
			loadPictureFromList(this._list)
		}
	}

	//- リストからのピクチャロード
	function loadPictureFromList(list) {
    ...
    Scene_Boot.prototype.isReady = function() {
        ...
        // ピクチャイメージの読み込み
        setTimeout(loadPictureImage, 100);
    ...
    function loadPictureImage() {
        ...
        list.forEach(data => {
            ...
            const url = getPictureUrl(name, data);
            if (!url) { return; }
            ImageManager.loadBitmapFromUrl(url);
        });
    };*/
	...
	//- ピクチャURLの取得
	function getPictureUrl(name, data) {
		if (!data) {
			if (!name) {
				return
			}
			data = keke_pictureList.find((d) => d['ピクチャ名'] == name)
			if (!data) {
				return
			}
		}
	...
	function startCommonLoad(args) {
		let callNames = null
		// 読み込むコモンを取得
		callNames = args['読み込むコモン']
			? args['読み込むコモン'].replace(/\s*/g, '').split(',')
			: null
		if (!callNames) {
			return
		}
	...
	function startVignette(args) {
		...
		// カラートーン
		if (
			(args.commonName || args.commonSub || p.picture) &&
			(args['カラー赤'] || args['カラー緑'] || args['カラー青'])
		) {
	...
	function convertVignetteParamBasic(p, args) {
		...
		p.picture = args['ピクチャ使用'] ? args['ピクチャ使用'] : p.picture
	...
	function setVignetteAnime(vignette, args) {
		...
		params.forEach((param, i) => {
			...
			anime.timeMax = news[word + 'アニメ-時間']
				? makeTime(news[word + 'アニメ-時間'])
				: anime.timeMax
			anime.dura = 0
	...
	function setVignetteChase(vignette, args) {
		...
		const charas = [
			...getCharasByName(params['対象キャラ-名前']),
			...getCharasById(params['対象キャラ-ID'])
		]
		if (charas[0]) {
			chase.targetId = charaToId(charas[0])
		}
		chase.spinByDire = params['向きによる回転']
			? toBoolean(params['向きによる回転'])
			: chase.spinByDire
	...
	function createVignetteSprite(spriteset, sceneType, vignette, i) {
		...
		if (!SceneManager._scene._vignetteLayersKeA) {
			return
		}
		if (vignette.sceneName != currentScene()) {
			return
		}
		// ピクチャが読み込み済みか
		if (!isLoadedPicture(vignette)) {
			return
		}
		const gw = Graphics.width
		const gh = Graphics.height
		// ビネットビットマップの形成
		const r = vignette.commonName
			? getVignetteCash(vignette.commonName, vignette.commonPoint)
			: createVignetteBitmap(vignette)
	...
	function createVignetteBitmap(vignette) {
		...
		if (form == 'スクエア') {
			designInSquare(
				...
				squareW,
				squareH,
				vignette.squareRoundLine,
				vignette.squareRoundEdge
			)
		} else if (form == '真円') {
			designInCircle(
				bitmap,
				...
				aWidth,
				aHeight,
				Math.max(lengthX, 0) / 2,
				Math.max(lengthY, 0) / 2,
				circleStart,
				circleEnd,
				'arc'
			)
			...
			const freePoints = formCfg['フリー作図']
				? JSON.parse(formCfg['フリー作図']).map((pos) => JSON.parse(pos))
				: []
			designInFree(bitmap, 0, -blurH, aWidth, aHeight, freePoints)
			toLate = true
		}
		context.fill()
		return { bitmap: bitmap, toLate: toLate }
	}

	//- 扇型か
	function isFan(vignette) {
		if (!vignette.form.includes('円') || !vignette.circleToFan || !vignette.circleToFan.length) {
			return false
		}
		toFan = vignette.circleToFan
		if (toFan[0] == toFan[1] || Math.abs(toFan[1] - toFan[0]) % 360 == 0) {
			return false
		}
		return true
	}
	...
	function rechildVignette(scene) {
		...
		vignetteSprites().forEach((sprite) => {
			...
			if (vnt.sceneName != scene.constructor.name && !vnt.takeOver) {
				return
			}
	...
	function updateVignette(spriteset, sceneType) {
		...
		updateIrisAppo(gm)
		const vignettes = gm._vignettesKe
		if (!gm._vignettesKe || !gm._vignettesKe.length) {
			return
		}
		// ビネットごとに処理
		let del = false
	...
	function spinVignetteByDire(vignette, init) {
		if (!vignette.chase || !vignette.chase.targetId || !vignette.chase.spinByDire) {
			return
		}
	...
	function getCharaDire(chara, vnt) {
		...
		if (vnt.charaPreX != null) {
			...
			// 斜め解除判定
			if (vnt.charaDiagoCount) {
				vnt.charaDiagoCount--
			}
			if (
				!vnt.charaDiagoCount &&
				vnt.charaDiagoDire &&
				(inputMove() || inputStraight(vnt, goX, goY))
			) {
				vnt.charaDiagoDire = null
			}
			// 斜め判定
			let diagoOk = false
			if ((Input.isPressed('left') && Input.isPressed('up')) || (goX < 0 && goY < 0)) {
				vnt.charaDiagoDire = 7
				diagoOk = true
			} else if ((Input.isPressed('left') && Input.isPressed('down')) || (goX < 0 && goY > 0)) {
				vnt.charaDiagoDire = 1
				diagoOk = true
			} else if ((Input.isPressed('right') && Input.isPressed('up')) || (goX > 0 && goY < 0)) {
				vnt.charaDiagoDire = 9
				diagoOk = true
			} else if ((Input.isPressed('right') && Input.isPressed('down')) || (goX > 0 && goY > 0)) {
				vnt.charaDiagoDire = 3
				diagoOk = true
			}
			if (diagoOk) {
				vnt.charaDiagoCount = 10
			}
			// 斜め中なら斜め
	...
	function inputStraight(vnt, goX, goY) {
		...
		if ((goX && !goY) || (!goX && goY)) {
			straight = true
		} else if (Input.isPressed('left') && !Input.isPressed('up') && !Input.isPressed('down')) {
			straight = true
		} else if (Input.isPressed('right') && !Input.isPressed('up') && !Input.isPressed('down')) {
			straight = true
		} else if (Input.isPressed('up') && !Input.isPressed('left') && !Input.isPressed('right')) {
			straight = true
		} else if (Input.isPressed('down') && !Input.isPressed('left') && !Input.isPressed('right')) {
			straight = true
		}
		if (straight) {
			if (!vnt.straightCount) {
				vnt.straightCount = 0
			}
			vnt.straightCount++
	...
	function spinVignetteSmooth(vnt) {
		...
		vnt.spinAngle =
			vnt.spinAngleTage +
			(Math.sin(Math.PI * (vnt.spinDuration / vnt.spinTimeMax + 1.5)) * volAngles[0] +
				volAngles[0]) /
				2
		...
		vnt.offsetXSpin =
			vnt.offsetXSpinTage +
			(Math.sin(Math.PI * (vnt.spinDuration / vnt.spinTimeMax + 1.5)) * volOffsetX + volOffsetX) / 2
		const volOffsetY = vnt.offsetYSpinPre - vnt.offsetYSpinTage
		vnt.offsetYSpin =
			vnt.offsetYSpinTage +
			(Math.sin(Math.PI * (vnt.spinDuration / vnt.spinTimeMax + 1.5)) * volOffsetY + volOffsetY) / 2
	}

	//- ビネットのフェード
	function fadeVignette(vignette) {
		if (!vignette.fadeTime) {
			return
		}
		const v = vignette
		// 不透明度を変更
	...
	function updateVignetteAnimeEach(vignette, param, word) {
		...
		const anime = vignette[param]
		if (!anime.moving || anime.moving == `${word}無し` || !anime.timeMax || !anime.pow) {
			return
		}
		// パラメータ変動
	...
	function limitIrisEffectOne(args, handler, vignettes = [null]) {
		...
		if (
			((inIrisEffect() || !SceneManager._scene._spriteset) && !(vignette && vignette.irisEffect)) ||
			!isLoadedPicture(vignette)
		) {
			// アイリス予約のセット
	...
	function updateIrisAppo(gm) {
		if (!gm._irisApposKe || !gm._irisApposKe.length) {
			return
		}
		...
		if (!isLoadedPicture(appo.vignette)) {
			return
		}
	...
	function startIrisEffect2(args, vignette) {
		...
		if (!wait) {
			startIrisEffect2_a(args, vignette)
		}
	}
	...
	function startIrisEffect3(args, vignette) {
		...
		params.forEach((param, i) => {
			...
			if (param.includes('tone') && !(args.commonName || vignette.picture)) {
				return
			}
			if (!args[word]) {
				return
			}
			iris[param + 'Drift'] = makeDrift(
				args[word],
				vignette[param],
				iris.timeMax,
				iris.easing,
			...
			if (param.includes('tone')) {
				iris.toneChange = true
			}
		})
		// ずらしを適用
		...
		if (!iris.timeMax) {
			const spriteset = SceneManager._scene._spriteset
			if (spriteset) {
				updateIrisEffect(spriteset, null, vignette, i)
			}
		}
		args.preter = null
	}
	...
	function startVignetteControl(args) {
		...
		targets.forEach((vnt) => {
			...
			if (!ctrl.timeMax) {
				const spriteset = SceneManager._scene._spriteset
				if (spriteset) {
					updateVignetteControl(spriteset, null, vnt, i)
				}
			}
		})
		args.preter = null
	...
	function getTargetVignette(args) {
		...
		vignettes.forEach((vnt) => {
			// 対象ビネットか判定
			let ok = false
			if (names && names.some((n) => (vnt.name ? vnt.name.includes(n) : 0))) {
				ok = true
			}
			if (ids && ids.includes(i)) {
				ok = true
			}
			if (!ok) {
				return
	...
	function updateIrisEffect(spriteset, sceneType, vignette, i) {
		...
		// アイリスエフェクトの更新-キャッシュ
		if (iris.cashName) {
			updateIrisEffectCash(sceneType, vignette, i, iris, sprites)
		}
		// 各パラムの更新
		const params = [
			'toneRed',
		...
		params.forEach((param, i) => {
			const word = words[i]
			// トーン変更がない時はカラートーンしない
			if (param.includes('tone') && !iris.toneChange) {
				return
			}
			// 変動を適用
			if (iris[param + 'Drift']) {
				...
				if (param.includes('opacity')) {
					vignette.opacityOri += plus
				}
				// 回転角の追加処理
				if (param.includes('angle')) {
					vignette.anglePi = angleToRadian(vignette.angle)
				}
			}
		})
		// カラートーンの追加処理
	...
	function updateVignetteControl(spriteset, sceneType, vignette, i) {
		if (!vignette.vntControls || !vignette.vntControls.length) {
			return
		}
	...
	function adjustIrisVignette(vignette, iris) {
		...
		params.forEach((param, i) => {
			if (iris[param + 'Drift']) {
				vignette[param] = iris[param + 'Drift'].end
			}
		})
	}
	...
	function makeIrisCash(args, data, callName, vignette, handler, speed) {
		...
		if (!im._irisCashsKe[callName]) {
			im._irisCashsKe[callName] = { list: [], comp: false, sprites: [], noDestroy: false }
		} else {
			return false
		}
		...
		if (args.inSceneOpen) {
			...
			// マップ条件
			if (args.inSceneOpen == 'map' && preScene && preScene._spriteset) {
				ok = true
			}
			// バトル条件
			if (args.inSceneOpen == 'battle' && SceneManager._scene.constructor.name == 'Scene_Map') {
				ok = true
			}
			if (ok) {
				im._irisCashsKe[callName].noDestroy = true
			}
		}
		// 読み込みパックにセット
		speed = Math.max((speed || keke_irisLoadSpeed) - 1, 0)
		if (!im._irisLoadPacksEachKe) {
			im._irisLoadPacksEachKe = []
		}
		im._irisLoadPacksEachKe.push({
			route: route,
			callName: callName,
			loading: false,
			index: -1,
	...
	function calcIrisRoute(iris, vignette) {
		...
		while (iris.dura < timeMax) {
			...
			params.forEach((param, j) => {
				const word = words[j]
				if (iris[param + 'Drift']) {
					r[param] = route[i - 1][param] + updateDrift(iris[param + 'Drift'], word)
				} else {
					r[param] = vignette[param]
				}
			})
	...
	function setIrisImageLoad(packs, args, vignette, handler) {
		const im = ImageManager
		if (!im._irisImageLoadesKe) {
			im._irisImageLoadesKe = []
		}
	...
	function getIrisCash(callName) {
		...
		if (!im._irisCashsKe[callName].comp) {
			return
		}
	...
	function eraseVignette(args) {
		...
		// すべて削除
		if (eval(args['すべて削除'])) {
			clearVignette(gm, fadeTime)
		}
		// IDで削除
		if (args['IDで削除']) {
			delVignette(gm, args['IDで削除'], fadeTime)
		}
		// 名前で削除
		if (args['名前で削除']) {
			delVignette(gm, null, args['名前で削除'], fadeTime)
		}
	}
	...
	function delVignette(gm, ids, names, fadeTime) {
		...
		vignettes.forEach((vnt, i) => {
			if (ids && !ids.includes(i)) {
				return
			} else if (names && !names.some((n) => vnt.name.includes(n))) {
				return
			}
			vnt.delFlag = true
			// フェードのセット
			...
			if (ctrls && ctrls.length) {
				ctrls.forEach((ctrl) => remControlWait(ctrl.preter))
			}
			del = true
		})
		return del
	...
	function charaToId(subject) {
		...
		if (subject._followers) {
			id = -1
		} else if (subject._memberIndex) {
			id = -1 - subject._memberIndex
		} else if (subject._type) {
			id = subject._vehicleIdKe
		}
	...
	Game_InterpreterStartingKeVnef.prototype.setupChild = function (list, eventId) {
		if (!this._childInterpreter) {
			this._childInterpreter = new Game_InterpreterStartingKeVnef(this._depth + 1)
		}
	...
	function designInCircle(
		...
		context.moveTo(x, y3)
		context.lineTo(x2, y3)
		if (toFan && upStart != null && upStart != Math.PI) {
		...
		context.moveTo(x5, y3)
		context.lineTo(x4, y3)
		if (toFan && downStart != null && downStart != 0) {
			context.lineTo(x3, y3)
		}
		...
		if (toFan && downStart != null && downEnd != Math.PI) {
			context.lineTo(x3, y3)
		}
		context.lineTo(x, y3)
		context.lineTo(x, y5)
	...
	function designInGiza(bitmap, x, y, w, h, inW, inH, cfg = {}, smooth, outX, outY) {
		...
		if (smooth) {
			...
			for (let i = 0; Math.ceil(i < yNum * 0.5); i++) {
				...
				context.quadraticCurveTo(
					isOdd ? nowX + xOdd / 2 : nowX - rand * 0.5,
					isOdd ? nowY : nowY - yDiv * 0.5,
					nowX + xOdd,
					nowY - yDiv * yOdd
				)
				if (isOdd) {
			...
			for (let i = 0; i < Math.ceil(yNum * 0.5); i++) {
				...
				if (isEven) {
					context.quadraticCurveTo(nowX + rand * 0.5, nowY + yDiv * 0.5, nowX, nowY + yDiv)
				}
				if (isOdd) {
					nowX += rand
				}
			...
			for (let i = 0; i < Math.ceil(yNum * 0.5); i++) {
				...
				context.quadraticCurveTo(
					isOdd ? nowX - xOdd * 0.5 : nowX + rand * 0.5,
					isOdd ? nowY : nowY + yDiv * 0.5,
					nowX - xOdd,
					nowY + yDiv * yOdd
				)
