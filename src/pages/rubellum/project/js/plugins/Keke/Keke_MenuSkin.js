 ...
 * 
 * @param 文字色
 * @desc テキストの文字色。赤, 緑, 青, 濃度。色0〜255、濃度0〜1。空欄なら元々の色を適用
 * @default 255, 255, 255, 1
 *
 * @param 縁取り幅
 * @desc テキストの縁取り幅。5 なら 5ピクセル。空欄なら元々の縁取り幅を適用
 * @default 6
 *
 * @param 縁取り色
 * @desc テキストの縁取り色。赤, 緑, 青, 濃度。色0〜255、濃度0〜1。。空欄なら元々の色を適用
 * @default 0, 0, 0, 1
 */
 ...
 * @param 対象ウインドウC
 * @parent 外部プラグイン用
 * @desc カスタムメニュー系の他プラグイン用。対象とするウインドウの識別子, で区切って複数指定可能
 * @type multiline_string
 * @default 
 * 
 * @param 対象シーンC
 * @parent 外部プラグイン用
 * @desc カスタムメニュー系の他プラグイン用。対象とするシーンの識別子, で区切って複数指定可能
 * @type multiline_string
 * @default 
 * 
 */
...
;(() => {
	...
	PluginManager.registerCommand(pluginName, '背景変更', (args) => {
		...
		if (!$gameSystem._backChangesKeMnsk) {
			$gameSystem._backChangesKeMnsk = {}
		}
	...
	PluginManager.registerCommand(pluginName, 'ピクチャ変更', (args) => {
		...
		if (!$gameSystem._pictChangesKeMnsk) {
			$gameSystem._pictChangesKeMnsk = {}
		}
	...
	PluginManager.registerCommand(pluginName, 'BGM変更', (args) => {
		...
		if (!$gameSystem._bgmChangesKeMnsk) {
			$gameSystem._bgmChangesKeMnsk = {}
		}
	...
	function getStyleByWindow(windo, design) {
		...
		const finds = design.filter((d) => {
			...
			// 対象ウインドウの判定(カスタムメニュー)
			if (judgeTageWindowCustomMenu(d, windo)) {
				return true
			}
			// 対象ウインドウの判定
			if (judgeTageWindow(d, windo, windowName)) {
				return true
			}
			return false
		})
	...
	function judgeTageWindow(d, windo, windowName) {
		...
		for (let tage of tages) {
			...
			if (d['継承先にも適用']) {
				if (windo instanceof window[tageWindow]) {
					return true
				}
			} else {
				if (windowName == tageWindow) {
					return true
				}
			}
		}
	...
	function getStyleByScene(currentScene, design) {
		...
		if (!currentScene) {
			currentScene = SceneManager._scene
		}
		...
		const finds = design.filter((d) => {
			...
			// 対象シーンの判定
			if (judgeTageScene(d, currentScene, sceneName)) {
				return true
			}
			// 対象シーンの判定(カスタムメニュー)
			if (judgeTageSceneCustomMenu(d)) {
				return true
			}
		})
		if (!finds.length) {
	...
	function judgeTageScene(d, currentScene, sceneName) {
		...
		for (let tage of tages) {
			...
			if (d['継承先にも適用']) {
				if (currentScene instanceof window[tageScene]) {
					return true
				}
			} else {
				if (sceneName == tageScene) {
					return true
				}
			}
		}
	...
	function judgeTageSceneCustomMenu(d) {
		if (!PluginManager._scripts.some((n) => n == 'SceneCustomMenu') || !d['対象シーンC']) {
			return false
		}
		let tages = d['対象シーンC']
		tages = Array.isArray(tages) ? tages : [tages]
		for (let tage of tages) {
			if (SceneManager.isCustomScene(tage)) {
				return true
			}
		}
		return false
	...
	//- デザインの取得
	function getDesign() {
		if (!$gameSystem._designKeMnsk) {
			$gameSystem._designKeMnsk = keke_startPack
		}
	...
	function isValidDesign() {
		if (!isValidScene(SceneManager._scene)) {
			return false
		}
		if (!$gameSystem) {
			return false
	...
	function getBackChange() {
		...
		if (!(changes['全部'] || changes[designName])) {
			return null
		}
	...
	function isValidScene(scene) {
		if (scene.constructor.name.match(/Scene_Map|Scene_Battle/)) {
			return false
		}
	...
	function createRectSkin(windo, rect) {
		...
		const skinSprite = createSkin(
			...
			openAnime,
			'',
			{ anchorX: 0.5, anchorY: 0.5, animeDelay: animeDelay }
		)
		if (!skinSprite) {
			return
		}
		...
		// チルド
		const cliendArea = windo._clientAreaMnskKe
		cliendArea.addChild(skinSprite)
		// 変数セット
		if (!windo._rectSkinsKe) {
			windo._rectSkinsKe = []
		}
	...
	function delRectSkin(windo, rect) {
		if (!windo._rectSkinsKe || !windo._rectSkinsKe.length) {
			return
		}
		let del = false
	...
	function setWindowAnime(scene) {
		if (!setsWindowAnimes || !setsWindowAnimes.length) {
			return
		}
	...
	function makeBackSkin(windo) {
		if (!isValidDesign() || windo._drawedBackSkinKe) {
			return
		}
		// 自動スタイル取得
		const style = getStyleAuto(windo)
	...
	function createBackSkin(windo, style) {
		...
		const padding = $gameSystem.windowPadding()
		// スキンの形成
		const skinSprite = createSkin(-padding, -padding, w, h, backSkin, windo, style, '', '', {
			anchorX: 0.5,
			anchorY: 0.5
		})
		if (!skinSprite) {
	...
	function createCursorSprite(windo) {
		...
		const onCursorAnime =
			skinData && skinData['画像'] && (style['カーソルアニメ-開始'] || style['カーソルアニメ'])
		if (cursorSkin || onCursorAnime) {
			windo._cursorSprite._isNoneKe = true
		}
		// 表示
		if (cursorSkin) {
			cursorSprite = createSkin(0, 0, 0, 0, cursorSkin, windo, style, '', '', {
				anchorX: 0.5,
				anchorY: 0.5
			})
		}
	...
	Window_Base.prototype._refreshCursor = function () {
		// カーソルのリフレッシュ
		if (refreshCursor(this, this._cursorSpriteKe)) {
			return
		}
	...
	function updateCursor(windo) {
		...
		if (defoCursor._isNoneKe) {
			const del =
				windo.maxItems() ||
				(windo.constructor.name == 'Window_ShopNumber' && windo._makedRectSkinKe)
			if (del) {
				defoCursor.visible = false
			} else {
				defoCursor.visible = true
			}
		}
		// 独自カーソル
		if (!windo._cursorSpriteKe) {
			return false
		}
	...
	function startCursorAnime(windo, skinSprite) {
		...
		if (!windo._inCursorAnimeSpritesKe) {
		...
		if (!isValidAnime(style, cursorAnime) && !isValidAnime(style, cursorAnimeStart)) {
			return
		}
		// 現在のレクトを取得
		...
		// すでにカーソルアニメ中ならリターン
		if (windo._inCursorAnimeSpritesKe.some((s) => s == skinSprite)) {
			return
		}
		// セットアップ未完了時
		if (!skinSprite._isSetupedKe) {
			if (skinSprite._breakSetupKe) {
				return
			}
		...
		tageSkins.forEach((sprite) => {
			...
			callAnimeByStyle(
				...
				style,
				cursorAnimeStart,
				getCursorAnimeType(windo, style, true)
			)
			// アニメの呼び出し-表示中
	...
	function stopCursorAnimeAll(windo) {
		if (!windo._inCursorAnimeSpritesKe || !windo._inCursorAnimeSpritesKe.length) {
			return
		}
	...
	function updateCursorAnime(sprite, windo, drift) {
		...
		if (!animeType.includes('cursor') || !(windo instanceof Window_Base)) {
			return false
		}
		// ウインドウが非アクティブ時の処理
		if (!windo.active) {
	...
	function updateHelpVisible(windo) {
		if (!isValidDesign() || !windo._windowSkinKe) {
			return
		}
		if (windo._text) {
			windo.visible = true
	...
	function createBackEach(
		...
		const name = `${i}:${j}`
		// 現在位置変数を取得
		if (!$gameTemp._backPossKeMnsk[name]) {
			$gameTemp._backPossKeMnsk[name] = { x: 0, y: 0, scaleX: 0, scaleY: 0 }
		}
		...
		const backSkin = createSkin(
			...
			openAnime,
			'',
			{ anchorX: 0.5, anchorY: 0.5, scaleX: scaleX, scaleY: scaleY, animeType: 'scaleMirror' }
		)
		if (!backSkin) {
			return
		}
	...
	function updateBack(scene) {
		if (!scene._backSkinsKe || !scene._backSkinsKe.length || !$gameTemp._backPossKeMnsk) {
			return
		}
		const gw = Graphics.width
		const gh = Graphics.height
	...
	function reverseScaleAnime(sprite, xy) {
		if (!sprite._animationsKe || !sprite._animationsKe.length) {
			return
		}
	...
	function createPictureEach(style, pict, scene) {
		...
		// スキンの形成
		const pictSprite = createSkin(x, y, w, h, skin, scene, pict, openAnime, ingAnime, {
			anchorX: anchorX,
			anchorY: anchorY,
			animeDelay: 0
		})
		...
		if (layer == '下層') {
			...
			childPict(scene, pictSprite)
			setTimeout(childPict, 0, scene, pictSprite)
		}
		// 変数セット
		if (!scene._pictsKeMnsk) {
			scene._pictsKeMnsk = []
		}
		...
		if (pict['有効シーン']) {
			...
			const tageScenes = Array.isArray(okScene)
				? okScene.map((s) => 'Scene_' + s)
				: 'Scene' + okScene
	...
	function updatePict(scene) {
		if (!scene._pictsKeMnsk || !scene._pictsKeMnsk.length) {
			return
		}
		// ピクチャを全て処理
	...
	function updatePictVisible(pictSprite) {
		if (!pictSprite._noVisiblesKe || !pictSprite._noVisiblesKe.length) {
			...
			if (pictSprite._textSpriteKe) {
				pictSprite._textSpriteKe.visible = true
			}
		} else {
			pictSprite.visible = false
			if (pictSprite._textSpriteKe) {
				pictSprite._textSpriteKe.visible = false
			}
		}
	}
	...
	function pictToNoVisible(pictSprite, word) {
		if (!pictSprite._noVisiblesKe) {
			pictSprite._noVisiblesKe = []
		}
		if (pictSprite._noVisiblesKe.some((w) => w == word)) {
			return
		}
	...
	function pictSynchroWindow(windo, flag) {
		if (!isValidDesign() || !windo || !windo.parent) {
			return
		}
		const scene = SceneManager._scene
		...
		picts.forEach((pictSprite) => {
			// 連動するなら可視状態を変更
			if (!pictSprite._synchroWindowNameKe) {
				return
			}
	...
	function judgePictShow(pictSprite) {
		if (!pictSprite._showConditionKe) {
			return
		}
	...
	function applyFontCfg(windo) {
		...
		// 文字色設定済みフラグ
		if (font['文字色']) {
			windo._textColorSetedKeMnsk = true
		}
		if (font['縁取り色']) {
			windo._outColorSetedKeMnsk = true
		}
		// フォント設定を記憶
	...
	Window_Base.prototype.changeTextColor = function (color) {
		if (this._textColorSetedKeMnsk && !colorChangeForce) {
			return
		}
	...
	Window_Base.prototype.changeOutlineColor = function (color) {
		if (this._outColorSetedKeMnsk && !colorChangeForce) {
			return
		}
	...
	function startMenuBGM(scene) {
		...
		// BGM変更なしならリターン
		if (menuBgmData && bgmName == menuBgmData.curBgmName) {
			return
		}
	...
	function endMenu(scene) {
		...
		if (!nextScene.match(/Map|Battle|Title|Load/)) {
			return
		}
		// 背景の初期化
		initBack()
	...
	function setMenuEndAnime(scene) {
		...
		if (sceneName.match(/Scene_Load|Scene_GameEnd/)) {
			return
		}
		// シーンからのスタイル取得
		const style = getStyleByScene(scene)
		...
		if (!isValidAnime(style, endAnime)) {
			return
		}
		// 画面を撮影
	...
	function processMenuEndAnime(scene) {
		if (!$gameTemp._menuEndAnimeAppoKe) {
			return
		}
		// メニュー終了スプライトを形成
	...
	function getAnimeData(animeName, animeType, delay = 0) {
		...
		if (animeName.constructor.name == 'Array') {
			animeName.forEach((name) => {
				...
				if (args) {
					animeDatas.push({ args: args, animeType: animeType, delay: delay })
				}
			})
		} else {
	...
	function setAnimeData(sprite, animeDatas, body) {
		if (!animeDatas || !animeDatas.length) {
			return
		}
		// シングルアニメの処理
		...
		if (!sprite._animeDatasKe) {
			sprite._animeDatasKe = []
		}
		sprite._animeDatasKe = [...sprite._animeDatasKe, ...animeDatas]
		// アニメ開始スプライテスとしてセット
		if (!body._animeStartSpritesKeMnsk) {
			body._animeStartSpritesKeMnsk = []
		}
	...
	function updateAnimeStart(sprite, body, force) {
		if (!sprite._animeDatasKe || !sprite._animeDatasKe.length) {
			return
		}
		// データをひとつ取得
		...
		if (!force) {
			if (
				sprite._animationsKe &&
				sprite._animationsKe.some((drift) => drift.animeType.includes('start') && !drift.noWait)
			) {
				return
			}
			if (
				sprite._animationsKe &&
				sprite._animationsKe.some((drift) => drift.animeType.includes('end') && !drift.noWait)
			) {
				return
			}
			if (animeType.includes('start') && sprite._waitStartAnimeKe) {
				return
			}
			if (animeType.includes('ing') && sprite._waitIngAnimeKe) {
				return
			}
		}
		// データをひとつ消去
	...
	function setAnime(sprite, data, body) {
		...
		// 回転角の初期化
		if (sprite._angleKe == null) {
			sprite._angleKe = angleToDegree(sprite.rotation)
		}
		...
		for (let i = 0; i <= 3; i++) {
			if (ct[i] != null) {
				const colorTones = calcMulti(
					sprite._colorTone[i],
					ct[i],
					sprite._colorToneOriKe ? sprite._colorToneOriKe[i] : 0
				)
				// カラートーンの不透明度補正
		...
		// アニメ中スプライテスとしてセット
		if (!body._inAnimeSpritesKeMnsk) {
			body._inAnimeSpritesKeMnsk = []
		}
	...
	function setOpenVal(sprite, args, data) {
		if (args['位置X'] != null) {
			sprite.x = calcMulti(sprite.x, args['位置X'], sprite._posXOriKe)[0].val
		}
		if (args['位置Y'] != null) {
			sprite.y = calcMulti(sprite.y, args['位置Y'], sprite._posYOriKe)[0].val
		}
		if (args['スケールX'] != null) {
		...
		if (args['不透明度'] != null) {
			sprite.opacity = calcMulti(sprite.opacity, args['不透明度'], sprite._opacityOriKe)[0].val
		}
		if (
			args['カラー赤'] != null ||
			args['カラー緑'] != null ||
			...
			for (let i = 0; i <= 3; i++) {
				if (ct[i] != null) {
					sprite._colorTone[i] = calcMulti(
						sprite._colorTone[i],
						ct[i],
						sprite._colorToneOriKe[i]
					)[0].val
					// カラートーンの不透明度補正
					sprite._colorTone[i] = colorToneOpaRevise(
						sprite._colorTone[i],
						sprite.opacity,
						data.animeType
					)
	...
	function updataAnimeStartSprite(body) {
		if (!body._animeStartSpritesKeMnsk || !body._animeStartSpritesKeMnsk.length) {
			return
		}
		let del = false
		...
		if (del) {
			body._animeStartSpritesKeMnsk = body._animeStartSpritesKeMnsk.filter((s) => s)
		}
	}

	//- アニメ中スプライトの更新
	function updataInAnimeSprites(body) {
		if (!body._inAnimeSpritesKeMnsk || !body._inAnimeSpritesKeMnsk.length) {
			return
		}
		let del = false
	...
	function updateAnime(sprite, body) {
		...
		sprite._animationsKe.forEach((drift, i) => {
			...
			if (!sprite.transform || !drift || drift.terminate) {
				...
				// シングル判定
				if (drift && drift.animeType == 'single') {
					single = true
				}
				// 破棄判定
				if (drift && drift.animeType == 'destroy') {
					destroy = true
				}
			}
		})
		...
		if (end) {
			...
			if (!sprite._animationsKe.length) {
				del = true
			}
		}
		return del
	...
	function updateAnimeEach(sprite, drift, i, body) {
		if (!sprite.transform || !drift) {
			return drift
		}
		// カーソルアニメの更新
		if (updateCursorAnime(sprite, body, drift)) {
			return drift
		}
		// ループアニメの停止
	...
	function remNoVisible(sprite) {
		if (!sprite || !sprite.parent || !sprite.transform || sprite._noVisibleKe) {
			return
		}
		if (sprite.x > -5000) {
			return
	...
	function restoreSpriteOriVar(sprite) {
		if (!(sprite instanceof Sprite) || !sprite.transform) {
			return
		}
		if (sprite._scaleXOriKe != null) {
			sprite.scale.x = Number(sprite._scaleXOriKe.replace('*', ''))
		}
		if (sprite._scaleYOriKe != null) {
			sprite.scale.y = Number(sprite._scaleYOriKe.replace('*', ''))
		}
		if (sprite._angleOriKe != null) {
			sprite._angleKe = sprite._angleOriKe
		}
		if (sprite._opacityOriKe) {
			sprite.opacity = 255 * Number(sprite._opacityOriKe.replace('*', ''))
		}
		if (sprite._colorToneOriKe != null) {
			sprite.setColorTone(sprite._colorToneOriKe)
		}
	}
	...
	function stopAnime(sprite, animeType, instant) {
		// アニメデータ
		if (sprite._animeDatasKe) {
			sprite._animeDatasKe = sprite._animeDatasKe.filter(
				(data) => animeType && data.aniemType != animeType
			)
		}
		// アニメ中
		if (sprite._animationsKe) {
	...
	function isAnimeRunning(sprite) {
		return (
			(sprite._animeDatasKe && sprite._animeDatasKe.length) ||
			(sprite._animationsKe && sprite._animationsKe.length)
		)
	}
	...
	function inAnime(body) {
		return (
			(body._animeStartSpritesKeMnsk && body._animeStartSpritesKeMnsk.length) ||
			(body._inAnimeSpritesKeMnsk && body._inAnimeSpritesKeMnsk.length)
		)
	}
	...
	function scanControlChar(text, fontSize, fontSizeOri, bitmap = null) {
		...
		if (match) {
			if (bitmap) {
				bitmap.fontBold = !match[1] ? false : Number(match[1]) ? true : false
			}
			text = text.replace(match[0], '')
			matched = true
		}
		...
		if (match) {
			if (bitmap) {
				bitmap.fontItalic = !match[1] ? false : Number(match[1]) ? true : false
			}
			text = text.replace(match[0], '')
			matched = true
		}
		...
		if (match) {
			if (bitmap) {
				bitmap.textColor = match[1].includes(',')
					? strToColor(match[1])
					: ColorManager.textColor(Number(match[1]))
			}
			text = text.replace(match[0], '')
			matched = true
		}
		...
		if (match) {
			if (bitmap) {
				bitmap.outlineColor = strToColor(match[1])
			}
			text = text.replace(match[0], '')
			matched = true
		}
		...
		if (match) {
			if (bitmap) {
				bitmap.outlineWidth = !match[1] ? 3 : Number(match[1])
			}
			text = text.replace(match[0], '')
			matched = true
		}
		...
		while (line) {
			...
			if (match) {
				...
				if (match[1].toUpperCase() == 'IC') {
					lineW +=
						(Number(match[2].replace(/\s/g, '').split(',')[1]) ||
							fontSize ||
							ImageManager.iconWidth) + 1
				}
	...
	function getTextHeight(text, fontSize, fontSizeOri, lineSpace) {
		...
		lines.forEach((line) => {
			...
			if (!d.cOk && d.cNum && strBytes(d.str) - d.cNum <= 1) {
				return
			}
	...
	function fontSettingByStyle(bitmap, style, word) {
		if (style[word + 'フォント']) {
			bitmap.fontFace = style[word + 'フォント']
		}
		if (style[word + '文字サイズ']) {
			bitmap.fontSize = getFontSize(style[word + '文字サイズ'])
		}
		if (style[word + '文字色']) {
			bitmap.textColor = style[word + '文字色'] || 'rgba(255, 255, 255, 1)'
		}
		if (style[word + '縁取り幅']) {
			bitmap.outlineWidth = style[word + '縁取り幅'] || 4
		}
		if (style[word + '縁取り色']) {
			bitmap.outlineColor = style[word + '縁取り色'] || 'rgba(0, 0, 0, 1)'
		}
	}
	...
	function calcMulti(cur, tage, ori, cmds = []) {
		...
		tageLine.forEach((tages, i) => {
			const match = tages.match(
				/^(r*m*s*\/|)([\+\*\/\%\=←→↑↓]*)(\-*\d+\.*\d*)~*(\-*\d*\.*\d*)([\+\*\/\%\=←→↑↓]*)(tn|cg|fk|cf|rd|bk|ei|eo|en|e|)(\(?\-*\d*\.*\d*\)?)(&*b*j*d*c*t*o*)(\_*\-?\d*\.*\d*)$/i
			)
			if (!match) {
				sols.push({
					val: tages,
	...
	function loopAngle(angle) {
		...
		if (angle >= 360) {
			...
			newAngle = angle < -1 ? 360 + (angle % 360) : 0
		}
		return newAngle
	}
