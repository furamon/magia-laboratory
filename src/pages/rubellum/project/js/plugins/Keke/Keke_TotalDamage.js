...
;(() => {
	...
	function measureTotalDamage(battler) {
		...
		if (!battler._totalDamagesKe) {
			battler._totalDamagesKe = { hp: 0, mp: 0, order: false, start: true, hit: 0, showText: null }
		}
		if (!BattleManager._totalDamagesAllKe) {
			BattleManager._totalDamagesAllKe = {
				hp: 0,
				mp: 0,
				order: false,
				start: true,
		...
		if (result.missed || result.evaded) {
			...
			const changeHp = result.hpDamage - (result.hpRegeneKe || 0)
			if (showEach) {
				total.hp += changeHp
				total.hit++
				total.order = true
			}
			if (showAll && result.hpDamage > 0) {
				totalAll.hp += changeHp
				totalAll.hit++
				totalAll.order = true
			}
		} else if (battler.isAlive() && result.mpDamage !== 0) {
			const changeMp = result.mpDamage - (result.mpRegeneKe || 0)
			if (showEach) {
				total.mp += changeMp
				total.hit++
				total.order = true
			}
			if (showAll && result.mpDamage > 0) {
	...
	function startTotalDamageSprite(bodySprite, allCfg) {
		const total = isAll(allCfg)
			? BattleManager._totalDamagesAllKe
			: bodySprite._battler
				? bodySprite._battler._totalDamagesKe
				: null
		if (!total || !total.order) {
			return
		}
		if (total.hp) {
			if (isAll(allCfg) && total.hp < 0) {
				return
			}
			// 合計ダメージスプライトの形成
			createTotalDamageSprite(bodySprite, total.hp, total.hit, 'hp', total.start, allCfg)
		} else if (total.mp) {
			if (isAll(allCfg) && total.mp < 0) {
				return
			}
			// 合計ダメージスプライトの形成
			createTotalDamageSprite(bodySprite, total.mp, total.hit, 'mp', total.start, allCfg)
	...
	//- 合計ダメージスプライトの形成
	function createTotalDamageSprite(bodySprite, value, hitNum, type, isStart, allCfg) {
		// フォント設定を取得
		...
		const cfg = allCfg[type2 + 'ダメージ'] || keke_eachCfg[word]
		if (cfg['無効']) {
			return
		}
		// サイズを取得
	...
	function createSprite3(sprite, d, isStart, bodySprite, allCfg) {
		...
		// 出現アニメの開始
		if (isStart) {
			startAppearAnime(sprite, allCfg)
		}
		// 位置更新フラグ
	...
	function updateTotalDamagePos(sprite, battlerSprite) {
		...
		if (asi) {
			...
			sprite.x =
				faceBaseSprite.x +
				offsetX +
				(sprite._damageCenterXKe || 0) -
				(faceBaseSprite._tempMovedXKeFast || 0)
			sprite.y =
				faceBaseSprite.y -
				asi.faceH * pos +
				offsetY +
				(sprite._frontActorYKe || 0) -
				(faceBaseSprite._tempMovedYKeFast || 0)
			// 通常時の位置
		} else {
			// 位置
			...
			sprite.y =
				battlerSprite.y -
				battlerSprite._frame.height * battlerSprite.scale.y * pos +
				offsetY +
				(sprite._frontActorYKe || 0) +
				underOffsetY
		}
	...
	function updateTotalDamagePosAll(sprite, spriteset, allCfg) {
		...
		sprite.y = y + posY + (sprite._movedKYe || 0)
	}
