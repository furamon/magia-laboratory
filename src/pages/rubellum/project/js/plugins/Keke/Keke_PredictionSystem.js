 ...
 * @param 行動-文字サイズ
 * @parent 行動ビュー
 * @desc act-fontSize 行動ビューの文字サイズ。基本 20
 * @default 20
 *
 * @param 行動-文字色
 * @parent 行動ビュー
...
;(() => {
	...
	function getFullAnimeStatusAsi(battler) {
		...
		// 顔アイコンがなければバトグラに表示
		if (!asi || !asi.faceBaseSprite || !asi.faceBaseSprite.visible) {
			return null
		}
		return asi
	}
	...
	function getValidByMemo(subject) {
		...
		// メモ欄に無効フラグがあったら無効
		if (members.some((battler) => getPredictionCfg(battler, '無効'))) {
			return false
		}
		// メモ欄に有効フラグがあったら有効
		if (members.some((battler) => getPredictionCfg(battler, '有効'))) {
			return true
		}
		return null
	}
	...
	function startPrediction(subject, force) {
		// 対象は敵キャラ & 自動キャラのみ
		if (subject._actorId && !subject.isAutoBattle()) {
			return
		}
		// 行動の先行作成
	...
	function isTargetLineShow(subject) {
		if (getPredictionCfg(subject, 'ラインあり')) {
			return true
		}
		if (getPredictionCfg(subject, 'ラインなし')) {
			return false
		}
	...
	function isValidPrediction(subject) {
		// 無効スイッチがオンなら無効
		if ($gameSwitches.value(keke_predictionOffSwitch)) {
			return false
		}
		//メモ欄に無効フラグがあったら無効
		...
		return (
			keke_predictionAlways || memoValid == true || $gameSwitches.value(keke_predictionOnSwitch)
		)
	}
	...
	function misfirePopup(battler) {
		if (!PluginManager._scripts.some((n) => n == 'Keke_StatePopup')) {
			return
		}
		// 不発ポップの表示
	...
	Window_ActViewKe.prototype.drawBackKe = function (width, height, color) {
		...
		if (backSprite.bitmap) {
			backSprite.bitmap.destroy()
		}
	...
	function updateActView(battlerSprite, battler) {
		...
		if (BattleManager._phase == 'start' && !$gameTroop.isEventRunning()) {
			return
		}
		// 行動ビューリフレッシュの更新
	...
	function updateActViewRefresh(subject) {
		...
		if (faceSprite) {
			...
			if (!subjectSprite || !subjectSprite._frame.width) {
				return
			}
			// 行動ビューのリフレッシュ
	...
	function refreshActView(battler, battlerSprite) {
		...
		// 背景の描画
		actWindow.drawBackKe(
			widthMax + keke_actOutW,
			textState.outputHeight + keke_actOutW,
			keke_actBackColor
		)
		if (widthMax != null) {
			actWindow._widthKe = widthMax
		}
		// 行動ビューの位置更新
	...
	function createActView(battlerSprite, battler, actionNum, text) {
		...
		if (preSprite && preSprite._actionNum == actionNum) {
			return
		}
		// 前のがあったら破棄
		if (preSprite) {
		...
		// 仮ウインドウを作成
		const tempWindow = new Window_ActViewKe({
			x: 0,
			y: 0,
	...
	function updateViewPosAct(sprite, battlerSprite, battler) {
		...
		// 画面外に出さない
		noOutScreen(sprite, sprite._widthKe, sprite._widthKe, sprite._padding, steelH)
	}
	...
	function isVisibleAct(battler) {
		...
		if (!isActShow() || !isValidPrediction(battler)) {
			return false
		}
		if (isActing(battler) || battler.isRestricted()) {
			return false
		}
		if (keke_actOnlyInput && !BattleManager.isInputting()) {
			return false
		}
		if (battler._actorId) {
			let valid = false
			if (keke_actShowCastActor && isCasting(battler)) {
				valid = true
			}
			if (keke_actShowAutoActor && isAutoBattle(battler)) {
				valid = true
			}
			if (!valid) {
				return false
	...
	function isCasting(battler, isStart) {
		...
		const isActive =
			battler.isAlive() && !battler.isRestricted() && BattleManager._phase != 'battleEnd'
	...
	function makeTargetFast(subject, redo) {
		...
		actions.forEach((action, i) => {
			...
			if (t.listForLine) {
				targetsForLine[i] = t.listForLine
			}
		})
		// ターゲットを保存
		if (targets.length) {
	...
	function applyTargetFast(subject) {
		if (!subject._targetsFastKe || !subject._targetsFastKe.lists.length) {
			return
		}
		// ターゲットラインの消去
	...
	function restoreEachTarget(delTarget) {
		...
		$gameTroop.aliveMembers().forEach((subject) => {
			...
			if (!targets || !targets.listsForLine.length) {
				return
			}
			// 全ての行動に対して判定
			targets.listsForLine.forEach((tages, i) => {
				...
				if (!tages.some((target) => idToBattler(target) == delTarget)) {
					return
				}
				// アクションを取得
				...
				// ターゲットを修正
				if (t.list) {
					targets.lists[i] = t.list
				}
				if (t.listForLine) {
					targets.listsForLine[i] = t.listForLine
				}
				// ターゲットラインのリフレッシュ
				refreshTargetLine(subject)
			})
	...
	function makeAllWord(subject, i) {
		...
		cfg.fontColor =
			allCfg['文字色'] || keke_lineColorList[subject.index() % 8] || 'rgba(255, 255, 255)'
	...
	function createAllWord(subjectSprite, subject, i) {
		...
		sprite.anchor.y = 0.5
		// 変数セット
		if (!subjectSprite._targetLinesKe) {
			subjectSprite._targetLinesKe = { list: [] }
		}
	...
	function createWire(subject, target, i) {
		...
		if (!isFrontViewActor(target) && !targetSprite) {
			return
		}
		// ライン始点の取得
	...
	function getPointFromActView(battler, battlerSprite, i, allData) {
		...
		if (allData) {
			...
			point.x +=
				dire == '左'
					? -frame.iconW - allData.width / 2 - outWs
					: frame.width + allData.width / 2 + outWs
			// 画面外に出さない-全体ワード
			...
			point.x +=
				dire == '左'
					? -frame.iconW - startSize
					: dire == '右'
						? frame.width + startSize
						: frame.width / 2
			point.y +=
				dire == '上'
					? -frame.height / 2 - startSize
					: dire == '下'
						? frame.height / 2 + startSize
						: 0
		}
	...
	function noOutScreenPoint(point) {
		...
		// 限界位置を超えた分だけずらす
		if (point.x < leftMax) {
			point.x += leftMax - point.x
		} else if (point.x > rightMax) {
			point.x += rightMax - point.x
		}
		if (point.y < upMax) {
			point.y += upMax - point.y
		} else if (point.y > downMax) {
			point.y += downMax - point.y
		}
	}
	...
	function drawWire(subject, target, bitmap, ax, ay, bx, by, receptIndex, isArrow) {
		...
		if (isArrow) {
			strokeArrow(context, ax, ay, bx, by, dire, offsetX, offsetY, lineW, ow, true)
		} else {
		...
		if (isArrow) {
			strokeArrow(context, ax, ay, bx, by, dire, offsetX, offsetY, lineW, ow)
		} else {
	...
	function saveReceptLine(target, subject) {
		if (!target._receptLinesKe) {
			target._receptLinesKe = []
		}
	...
	function delReceptLine(target, subject) {
		if (!target || !target._receptLinesKe) {
			return
		}
		const recepts = target._receptLinesKe
		// 行動者のスロットがあったら消去
	...
	function updateTargetLineVisible(subject, subjectSprite) {
		if (!subjectSprite._targetLinesKe || !subjectSprite._targetLinesKe.list.length) {
			return
		}
		let valid = true
		while (true) {
			...
			if (isActing(subject) || subject.isRestricted()) {
				valid = false
				break
			}
			if (keke_lineOnlyInput && !BattleManager.isInputting()) {
				valid = false
				break
			}
			if (!isTargetLineShow(subject) || !isValidPrediction(subject)) {
				valid = false
				break
			}
			break
	...
	function updateTargetLineCycle() {
		const gt = $gameTemp
		if (gt._tlFlickerDurationKe == null) {
	...
	function totalAllMetaArray(battler, words, action, noDelSpace, ratioIndex) {
		...
		if (battler) {
			...
			if (data) {
				metaAll(data.note, words, ratioIndex).forEach((e) => array.push(e))
			}
			if (battler._actorId) {
				// 職業値
			...
			if (matches) {
				matches.forEach(function (line) {
					...
					if (
						ratioIndex &&
						vals[ratioIndexEx] &&
						Math.randomInt(100) >= Number(vals[ratioIndexEx])
					) {
						return
					}
					result.push(match[1])
	...
	function showScreenFlash(d) {
		if (!d['画面フラッシュ'] || !d['…フラッシュ色' || !d['…フラッシュ時間']]) {
			return
		}
	...
	function startScreenStop(str) {
		if (!PluginManager._scripts.some((n) => n == 'Keke_HitStop')) {
			return
		}
		// 画面ストップ
