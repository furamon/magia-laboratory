...
;(function () {
	...
	Game_Enemy.prototype.multiplyHate = function (index, rate) {
		...
		if (HateDebugMode) {
			console.log(
				this.name() + 'の' + $gameActors.actor(index).name() + 'へのヘイトが' + hate + 'になった'
			)
		}
	}
	...
	Game_Action.prototype.confusionTarget = function () {
		switch (this.subject().confusionLevel()) {
			case 1:
				if (this._subjectActorId > 0) return this.opponentsUnit().randomTarget()
	...
	DataManager.processHateNotetags = function (group) {
		var note1 =
			/<HATE_control:[ ]*(user|target|whoHateUser|whoHateTarget|all|exceptUser|exceptTarget)[ ]*,[ ]*(user|target|exceptUser|targetsTarget)[ ]*,[ ]*(.+)[ ]*>/i
	...
	Scene_Battle.prototype.createHateWindows = function () {
		...
		this._ABEnemyListWindow = new Window_ABEnemyList(
			EnemyListX,
			EnemyListY,
			EnemyListWidth,
			Window_Base.prototype.fittingHeight(9)
		)
	...
	var _Scene_Battle_prototype_startActorCommandSelection =
