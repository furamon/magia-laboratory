...
;(function () {
	...
	Scene_Map.prototype.srpgAIPosition = function (user, event, action) {
		...
		if (targetEvent && targetEvent.isTargetValid()) {
			var distRouteXY = $gameTemp.distRouteXY(
				targetEvent.posX(),
				targetEvent.posY(),
				event.posX(),
				event.posX(),
			...
			bestXY = $gameTemp.bestXYinMoveList(
				validMoveList,
				distRouteXY[2][0],
				distRouteXY[2][1],
				distRouteXY[1]
			...
			battleMode === 'random' ||
			0.1 * _confusionMoveRate * user.confusionLevel() > Math.random()
		) {
			...
			bestXY = $gameTemp.nearestTargetXY(
				validMoveList,
				[event.isType()],
				event.posX(),
				event.posY()
		...
		} else if (battleMode.match(/\s*nearestFlag(.+)/i)) {
			//<mode:nearestFlag['xxxx']>
		...
		} else if (battleMode.match(/\s*mostFlags(.+)/i)) {
			//<mode:mostFlags['xxxx']>
			typeArray = eval(battleMode.match(/\s*mostFlags(.+)/i)[1])
			bestXY = $gameTemp.mostPositionXY(validMoveList, typeArray, event.posX(), event.posY())
		} else if (battleMode.match(/\s*avoidOpponents/i)) {
			//<mode:avoidOpponents> <mode:absAvoidOpponents>
			typeArray = [event.isType() === 'actor' ? 'enemy' : 'actor']
			bestXY = $gameTemp.avoidTargetXY(validMoveList, typeArray, false)
		} else if (battleMode.match(/\s*avoidFlags(.+)/i)) {
			//<mode:avoidFlags['xxxx']>
			...
			bestXY = $gameTemp.bestRegionXY(validMoveList, -1)
		} else if (battleMode.match(/\s*region\s+(\d+)/i)) {
			//<mode:region x>
		...
		} else if (battleMode.match(/\s*position\s+(\d+)\s+(\d+)/i)) {
			//<mode:position x y>, <mode:absPosition x y>
	...
	Game_Map.prototype.isOccupied = function (x, y) {
		return $gameMap.events().some(function (otherEvent) {
			return (
				otherEvent.pos(x, y) &&
				otherEvent.eventId() !== $gameTemp.activeEvent().eventId() &&
				!otherEvent.isThrough() &&
				!otherEvent.isErased() &&
				['enemy', 'actor', 'playerEvent'].contains(otherEvent.isType())
			)
		})
	}
	...
	Game_Temp.prototype.validMoveList = function (avoidDamageFloor) {
		...
		if (validMoveList.length === 0)
			validMoveList.push([$gameTemp.activeEvent().posX(), $gameTemp.activeEvent().posY()])
	...
	Game_Event.prototype.aiMove = function () {
		...
		if (this._aiMove !== undefined) {
			...
			this.eventId() === $gameTemp.targetEvent().eventId() &&
			action.item() &&
			action.item().meta.aiMove
		) {
			//self targeting skill (does it really work?)
	...
	Game_Event.prototype.aiMoveExtension = function (meta, a) {
		if (meta === 'timid') {
		...
		} else if (meta.match(/\s*patrolRegion\s+(\d+)\s+(\d+)/i)) {
			//<aiMove: patrolRegion x y>
			...
			else if (a.battleMode() !== 'region ' + region1 && a.battleMode() !== 'region ' + region2)
				return 'region ' + region1
			else return a.battleMode()
		} else if (meta.match(/keepDist\s+(\d+)/i)) {
			//<aiMove:'keepDist x'>
			var atkDist = meta.match(/keepDist\s+(\d+)/i)[1]
			var ctrlDist = false
			if (meta.match(/keepDist\s+(\d+)\s+(\d+)/i))
				ctrlDist = meta.match(/keepDist\s+(\d+)\s+(\d+)/i)[2]
	...
	Game_Temp.prototype.battleModeExtension = function (
		battleMode,
		validMoveList,
		event,
		user,
	...
	Game_Temp.prototype.nearestTargetXY = function (validMoveList, typeArray, eventX, eventY) {
		...
		$gameMap.events().forEach(function (otherEvent) {
			if (
				otherEvent.eventId() === activeEventId ||
				otherEvent.isErased() ||
				!otherEvent.isTargetValid()
			)
				return
	...
	Game_Temp.prototype.mostPositionXY = function (validMoveList, typeArray, eventX, eventY) {
		...
		for (var i = 0; i < maxClusters.length; i++) {
			...
			var distRouteXY = this.distRouteXY(
				Math.round(sumX / maxClusters[i].length),
				Math.round(sumY / maxClusters[i].length),
				eventX,
				eventY,
				true
			)
	...
	Game_Temp.prototype.certainRegionXY = function (validMoveList, regionId, oriX, oriY) {
		...
		if (minDist >= _maxMove + width + height) {
			//no valid pos, use fallback
	...
	Game_Temp.prototype.distRouteXY = function (x, y, oriX, oriY, fallBackMove) {
		...
		if (this.MoveTable(x, y)[0] >= 0 && !$gameMap.isOccupied(x, y)) {
			//if stepable
		...
		for (var d = 2; d < 10; d += 2) {
			...
			if ($gameMap.isValid(dx, dy)) {
				if (this.MoveTable(dx, dy)[0] >= 0 && _maxMove - this.MoveTable(dx, dy)[0] + 1 < minDist) {
					// +1 is not accurate but it should be fine
		...
		if (minDist < _maxMove) {
			//distance = 1 positions stepable
			var route = this.MoveTable(minX, minY)[1]
			return [minDist, route, [minX, minY]]
		} else if (_fallBack && fallBackMove && (fallBackMove === true || fallBackMove > _maxMove)) {
			// blocked! use fall back if set true or: this _maxMove < minDist sent from fallBackMove
	...
	Game_Temp.prototype.routeDistance = function (endX, endY, x, y, route) {
		...
		var dx = endX
		var dy = endY
		if (event && event.posX() === x && event.posY() === y)
			return $gameMap.distTo(x, y, endX, endY) + _maxMove //original pos not considered as in route
		if (!route) return false
		for (var i = route.length; i > 1; i--) {
			//route[0] = 0
			var d = route[i] || 0
