...
;(function ($) {
	...
	Game_Map.prototype.updateScrollAtDrag = function () {
		...
		if (this._oldMouseX && this._oldMouseX !== TouchInput._mouseX) {
			var difX =
				((this._oldMouseX - TouchInput._mouseX) * Math.pow(2, $.Parameters.dragSpeed)) / 256
			difX > 0 ? this.scrollRight(difX) : this.scrollLeft(-difX)
		}
		if (this._oldMouseY && this._oldMouseY !== TouchInput._mouseY) {
			var difY =
				((this._oldMouseY - TouchInput._mouseY) * Math.pow(2, $.Parameters.dragSpeed)) / 256
...
*/
	...
	Game_Map.prototype.updateScrollAtBorder = function () {
		...
		if (
			...
			!this._flagSRPGNormalPhaseCenterOnce &&
			$.Parameters.useCenteringFeature
		)
			return
	...
	Game_Map.prototype.updateSRPGCursorCenter = function () {
		...
		if ($gameSystem.isSubBattlePhase() === 'battle_window' && $.Parameters.useCenteringFeature)
			this.camCenterTo($gamePlayer)
		...
		if ($gameSystem.isSubBattlePhase() === 'normal') {
			...
			if (
				this.isSRPGCamCenterStopMoving(
					this._oldSRPGScreenCheckX,
					this._oldSRPGScreenCheckY,
					$gamePlayer
				)
			) {
