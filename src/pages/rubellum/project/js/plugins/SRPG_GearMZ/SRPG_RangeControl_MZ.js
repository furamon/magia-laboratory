...
;(function () {
	...
	Game_CharacterBase.prototype.LoSEvents = function (skill) {
		var blockingTypes = []
		if (
			(!_throughObject && skill.meta.throughObject != 'true') ||
			skill.meta.throughObject == 'false'
		) {
			blockingTypes.push('object')
		}
		if (
			(!_throughFriend && skill.meta.throughFriend != 'true') ||
			skill.meta.throughFriend == 'false'
		) {
			blockingTypes.push(this.isType() != 'enemy' ? 'actor' : 'enemy')
		}
		if (
			(!_throughOpponent && skill.meta.throughOpponent != 'true') ||
			skill.meta.throughOpponent == 'false'
		) {
			blockingTypes.push(this.isType() != 'enemy' ? 'enemy' : 'actor')
		}
		if (
			(!_throughEvent && skill.meta.throughEvent != 'true') ||
			skill.meta.throughEvent == 'false'
		) {
