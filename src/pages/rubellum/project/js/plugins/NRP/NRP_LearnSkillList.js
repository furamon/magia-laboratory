...
;(function () {
	...
	Window_LearnSkillList.prototype.drawItemName = function (learnSkillData, x, y, width) {
		...
		if (learnSkillData.DisplayName) {
			// %1=スキル名, %2=アイコン
			const displayName = learnSkillData.DisplayName.format(
				skillName,
				'\\i[' + dataSkill.iconIndex + ']'
			)
	...
	Window_LearnSkillActor.prototype.refresh = function () {
		...
		if (this._actor) {
			...
			this.drawActorFace(
				this._actor,
				nameRect.x,
				this.itemPadding() + nameRect.height + this.itemPadding()
			)
			this.drawSkillPoint()
		}
	}
	...
	Game_Actor.prototype.resetLearnSkillList = function () {
		...
		// 対象アクターのスキルセットのみを取得
		const skillSetList = pSkillSetList.filter(
			(skillSet) => !skillSet.Actor || skillSet.Actor == this.actorId()
		)
		// さらにスキルセットの中で条件を満たすスキルのみを追加
		for (const skillSet of skillSetList) {
			const skillList = parseStruct1(skillSet.SkillList)
