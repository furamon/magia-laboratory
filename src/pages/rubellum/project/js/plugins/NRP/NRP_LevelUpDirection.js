...
;(function () {
	...
	Game_Actor.prototype.displayLevelUp = function (newSkills) {
		...
		// 本来のレベルアップメッセージを取得
		const levelUpMessage = TextManager.levelUp.format(this._name, TextManager.level, this._level)
		...
		for (let i = $gameMessage._texts.length - 1; i >= 0; i--) {
			...
			if ($gameMessage._texts[i] == levelUpMessage) {
				...
				// ※\LUC[アクターＩＤ]
				$gameMessage._texts[i] =
					'\\' + LEVEL_UP_CODE + '[' + this.actorId() + ']' + $gameMessage._texts[i]
	...
	if (typeof AdditionalClass !== 'undefined') {
		...
		AdditionalClass.prototype.displayLevelUp = function (newSkills) {
			...
			const levelUpMessage = pLvUpMessage.format(actor.name(), this.name, this._level)
			...
			for (let i = $gameMessage._texts.length - 1; i >= 0; i--) {
				...
				if ($gameMessage._texts[i] == levelUpMessage) {
					...
					// ※\ACLUC[アクターＩＤ]
					$gameMessage._texts[i] =
						'\\' + AC_LEVEL_UP_CODE + '[' + actor.actorId() + ']' + $gameMessage._texts[i]
					return
				}
			}
		}
