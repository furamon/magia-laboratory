...
;(function () {
		...
		for (let i = 0; i < pSpecialTypeList.length; i++) {
			...
			if (isGaugeConditionOK(i, a)) {
				...
				this.placeGauge(
					...
					STATUS_TYPE + i,
					x + pMenuStartGaugeX,
					y + pMenuStartGaugeY + i * pGaugeInterval
				)
			}
		}
	}
	...
	if (pShowMenu) {
		...
		if (
			...
			Window_StatusBase.prototype.drawActorSimpleStatus
		) {
	...
	if (pShowSkillStatus) {
		 ...
		 */
		if (
			Window_SkillStatus.prototype.drawActorSimpleStatus ==
			Window_StatusBase.prototype.drawActorSimpleStatus
		) {
		 ...
		 * ●アクターのスキル画面のステータス描画
		 */
		const _Window_SkillStatus_drawActorSimpleStatus =
			Window_SkillStatus.prototype.drawActorSimpleStatus
		Window_SkillStatus.prototype.drawActorSimpleStatus = function (actor, x, y) {
			...
			for (let i = 0; i < pSpecialTypeList.length; i++) {
				...
				if (isGaugeConditionOK(i, a)) {
					...
					this.placeGauge(
						a,
						STATUS_TYPE + i,
						x + pStatusStartGaugeX,
						y + pStatusStartGaugeY + i * pGaugeInterval
					)
				}
			}
		}
			...
			for (let i = 0; i < pSpecialTypeList.length; i++) {
				...
				if (isGaugeConditionOK(i, a)) {
					...
					this.placeGauge(
						a,
						STATUS_TYPE + i,
						x + pBattleStartGaugeX,
						y + pBattleStartGaugeY + i * pGaugeInterval
					)
				}
			}
		}
