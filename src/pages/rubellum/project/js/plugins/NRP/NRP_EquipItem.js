...
;(function () {
	...
	Game_BattlerBase.prototype.canEquipItem = function (item) {
		return isEquipItem(item) && !this.isEquipTypeSealed(item.etypeId)
	}
	...
	Window_EquipItem.prototype.includes = function (item) {
		...
		if (isEquipItem(item)) {
			...
			return this._actor && this._actor.canEquip(item) && equipSlots.includes(this.etypeId())
		}
	...
	if (pUseOriginalCommand) {
		...
		Window_ActorCommand.prototype.makeCommandList = function () {
			...
			// 装備アイテムコマンドの追加
			if (this._actor) {
				this._list.splice(pCommandPosition, 0, {
					name: pCommandName,
					symbol: 'equipItem',
					enabled: true,
					ext: null
	...
	if (pUseOriginalCommand) {
		...
		Scene_Battle.prototype.isAnyInputWindowActive = function () {
			return (
				_Scene_Battle_isAnyInputWindowActive.apply(this, arguments) || this._equipItemWindow.active
			)
		}
