...
;(function () {
	...
	Game_BattlerBase.prototype.isCounterRestricted = function () {
		if (this.isAppeared()) {
			...
			// 行動制約の判定から除外する。
			const restrictions = this.states().map(
				(state) => state.restriction && !state.meta.CounterIgnoreRestrict
			)
	...
	Game_Action.prototype.applyCounter = function (object, target, addCount) {
		...
		// 仲間のスキルかどうか？
		if (
			(target.isActor() && this.subject().isActor()) ||
			(target.isEnemy() && this.subject().isEnemy())
		) {
			// 仲間のスキルに反撃しない場合は次へ
	...
	function resistCounter(target, subject, skillId, object, addCount) {
		// 既に登録済の場合は次へ
		const isResisted = mCounterList.some(
			(data) => data.subject == target && data.target == subject && data.skillId == skillId
		)
		if (isResisted) {
			return
		}
