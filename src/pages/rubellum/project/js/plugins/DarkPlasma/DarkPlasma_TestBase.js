...
;(() => {
	...
	PluginManager.setupTestPlugins = function (plugins) {
		if (Utils.isNwjs()) {
			const fs = require('fs')
			const loadedPluginNames = plugins
			...
			plugins
				...
				.filter(
					(plugin) =>
						!loadedPluginNames.includes(plugin.name) && fs.existsSync(this.makeUrl(plugin.name))
				)
				.forEach((plugin) => {
	...
	class TestTargetPlugins {
		 ...
		 */
		find(name, version) {
			return this._targetPlugins.find(
				(plugin) => plugin.name === name && plugin.version === version
			)
		}
	...
	class Scene_TestCase extends Scene_Base {
		...
		testCaseWindowRect() {
			...
			return new Rectangle(
				...
				y,
				Graphics.boxWidth,
				Graphics.boxHeight - y - this.reasonWindowHeight()
			)
		}
	...
	class Window_TestCase extends Window_Selectable {
		...
		drawItem(index) {
			...
			this.drawText(
				...
				rect.x,
				rect.y,
				this.width - this.textWidth('FAILED'),
				'right'
			)
		}
