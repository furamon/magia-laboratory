...
;(() => {
	...
	class StorageCategories {
		...
		count() {
			return [this.item, this.weapon, this.armor, this.keyItem].filter((category) => category)
				.length
		}
	}
	/**
	...
	class Window_StorageItemCategory extends Window_ItemCategory {
		...
		needsCommand(name) {
			return (
				super.needsCommand(name) && !!$gameTemp.storageCategories()?.categories().includes(name)
			)
		}
	}
	class Window_StorageItems extends Window_ItemList {
		...
		makeItemList() {
			const allItems = this.isPartyItem()
				? $gameParty.allItems()
				: $gameParty.storageItems().allItems()
	...
	class Window_StorageNumber extends Window_MenuNumber {
		...
		drawNumberChange() {
			...
			this.drawText(
				`${$gameParty.numItems(this._item) + changePartyItemNumber}`,
				0,
				0,
				this.innerWidth,
				'right'
