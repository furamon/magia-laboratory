...
;(() => {
	...
	const old_stateOnBattleStartTraitId = uniqueTraitIdCache.allocate(
		pluginName,
		localTraitId,
		'戦闘開始時ステート'
	)
	...
	const stateOnBattleStartTraitId = uniqueTraitIdCache.allocate(
		pluginName,
		localTraitId + 4,
		'戦闘開始時ステート'
	)
	const buffOnBattleStartTraitId = uniqueTraitIdCache.allocate(
		pluginName,
		localTraitId + 5,
		'戦闘開始時強化・弱体'
	)
	...
	class StateBuffOnBattleStartManager {
		constructor() {
			this._states = settings.stateOnBattleStart.map((object) =>
				StateOnBattleStart.fromObject(object)
			)
			...
			if (hasTraits(data)) {
				...
				if (data.meta.buffOnBattleStart) {
					String(data.meta.buffOnBattleStart)
						...
						.filter((line) => line.contains('oneOf:'))
						.forEach((line) => {
							const tokens = /oneOf: ?(((mhp|mmp|atk|def|mat|mdf|agi|luk)[\+\-][1-9]+,?)+)/.exec(
								line
							)
							if (tokens) {
	...
	function Game_Battler_StateBuffOnBattleStartMixIn(gameBattler) {
		...
		gameBattler.statesOnBattleStart = function () {
			...
			const old_randomIds = this.traitObjects()
				.filter((object) =>
					object.traits.some((trait) => trait.code === old_stateOnBattleStartRandomTraitId.id)
				)
				.map((object) => {
					const traits = object.traits.filter(
						(trait) => trait.code === old_stateOnBattleStartRandomTraitId.id
					)
		...
		gameBattler.buffsOnBattleStart = function () {
			...
			const old_randomIds = this.traitObjects()
				.filter((object) =>
					object.traits.some((trait) => trait.code === old_buffOnBattleStartRandomTraitId.id)
				)
				.map((object) => {
					const traits = object.traits.filter(
						(trait) => trait.code === old_buffOnBattleStartRandomTraitId.id
					)
