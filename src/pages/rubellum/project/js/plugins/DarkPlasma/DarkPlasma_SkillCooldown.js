...
;(() => {
	...
	class Game_SkillCooldown {
		...
		static setup(triggerSkillId) {
			...
			if ($dataSkills[triggerSkillId].meta.cooldownTurn) {
				result.push(
					new Game_SkillCooldown(
						triggerSkillId,
						Number($dataSkills[triggerSkillId].meta.cooldownTurn) + 1
					)
				)
			}
			/**
		...
		plusCooldownTurns(targetBattlers, plus, skills) {
			const targetCooldowns = targetBattlers
				.map((battler) => {
					...
					return skills
						? cooldowns.filter((_, skillId) => skills.some((skill) => skill.id === skillId))
						: cooldowns
				})
				.flat()
			targetCooldowns
	...
	PluginManager.registerCommand(pluginName, command_plusCooldownTurns, function (args) {
		...
		const targetSkills =
			parsedArgs.skills.length === 0
				? undefined
				: parsedArgs.skills.map((skillId) => $dataSkills[skillId])
		...
		battleManager.endTurn = function () {
			...
			if (!this.isTpb() && settings.decreaseBenchwarmersCooldown) {
				$gameParty
					...
					.forEach((actor) =>
						skillCooldownManager.decreaseCooldownTurns(actor.skillCooldownId(), true)
					)
			}
		}
	}
	...
	function Game_BattlerBase_SkillCooldownMixIn(gameBattlerBase) {
		...
		gameBattlerBase.setupCooldownTurnByTargetSkill = function (targetSkill, turn) {
			skillCooldownManager.setupCooldownTurnByTargetSkill(
		...
		gameBattlerBase.isDuringCooldown = function (skill) {
			return (
				$gameParty.inBattle() &&
				skillCooldownManager.isDuringCooldown(this.skillCooldownId(), skill, this.isActor())
			)
		}
		/**
		 * 指定したスキルの残りクールタイムを返す
