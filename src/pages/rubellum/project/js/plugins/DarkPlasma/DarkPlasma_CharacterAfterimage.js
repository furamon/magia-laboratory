...
;(() => {
	...
	function Sprite_Character_AfterimageMixIn(spriteCharacter) {
		...
		spriteCharacter.updateAfterimage = function () {
			 ...
			 */
			this._clearReservedAfterimageSprites = this._afterimageSprites.filter(
				(afterimage) => afterimage.opacity <= 0
			)
			this._afterimageSprites = this._afterimageSprites.filter(
				(afterimage) => afterimage.opacity > 0
			)
