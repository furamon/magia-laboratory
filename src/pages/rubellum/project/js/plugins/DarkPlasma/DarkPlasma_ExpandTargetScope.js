...
;(() => {
	...
	const settings = {
		...
		skipTargetSelectionForAll:
			String(pluginParameters.skipTargetSelectionForAll || false) === 'true'
	}
	...
	function Game_Temp_ExpandTargetScopeMixIn(gameTemp) {
		...
		gameTemp.processSyncSelectionEffect = function (battler) {
			this._syncSelectionEffectRequestedBattlers =
				this._syncSelectionEffectRequestedBattlers.filter((b) => b !== battler)
		}
	...
	function Scene_Battle_ExpandScopeTargetMixIn(sceneBattle) {
		...
		sceneBattle.createActorWindow = function () {
			...
			this._actorWindow.setHandler(
				settings.switchScopeButton,
				this.toggleTargetScopeActor.bind(this)
			)
		}
		...
		sceneBattle.createEnemyWindow = function () {
			...
			this._enemyWindow.setHandler(
				settings.switchScopeButton,
				this.toggleTargetScopeEnemy.bind(this)
			)
		}
	...
	function Window_BattleTarget_ExpandTargetScopeMixIn(windowClass) {
		...
		windowClass.canToggleScope = function () {
			return (
				$gameParty.inBattle() &&
				!!BattleManager.inputtingAction()?.canExpandScope() &&
				!this.cursorFixed()
			)
		}
