...
;(() => {
	...
	const settings = {
		...
		exportHelpText: String(
			pluginParameters.exportHelpText || `表示されているテキストを保存してください。`
		),
		importHelpText: String(
			pluginParameters.importHelpText || `セーブデータのテキストを貼り付けてください。`
		),
	...
	function Graphics_ImportExportSaveFileMixIn(graphics) {
		...
		graphics._stretchWidth = function () {
			return this._importExportMode && Utils.isMobileDevice()
				? _stretchWidth.call(this) / 2
				: _stretchWidth.call(this)
		}
