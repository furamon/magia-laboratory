	...
	function toFusionItemGood(data, item) {
		return new FusionItemGoods(
			...
			item.base.materialItems
				...
				.concat(
					item.base.materialArmors.map(
						(material) => new FusionItemMaterial($dataArmors[material.id], material.count)
					)
				),
			item.base.gold,
		...
		const goods = parsedArgs.presetIds
			.map((presetId) => {
				...
				return preset.items
					...
					.concat(
						preset.weapons.map((weapon) => toFusionItemGood($dataWeapons[weapon.result], weapon))
					)
	...
	class Scene_FusionItem extends Scene_ShopLikeMixIn(Scene_MenuBase) {
		...
		maxBuy() {
			...
			return this._materials.reduce((prev, current) => {
				return Math.min(
					prev,
					Math.floor($gameParty.numUsableItemsForFusion(current.data) / current.count)
				)
			}, max)
		}
		buyingPrice() {
	...
	class Window_FusionShopBuy extends Window_ShopBuy {
		...
		isEnabled(index) {
			...
			return (
				...
				materials.every(
					(material) => $gameParty.numUsableItemsForFusion(material.data) >= material.count
				)
			)
		}
		makeItemList() {
