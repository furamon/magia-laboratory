...
;(() => {
	...
	const settings = {
		...
		rewardMessages: JSON.parse(
			...
			return ((parameter) => {
				...
				return {
					message: String(
						parsed.message || `メダルを${medalCount}個集めた。\n「${itemName}」を手に入れた！\n`
					),
	...
	const rewardItems = settings.rewardItems
		...
		.concat(
			settings.rewardWeapons.map((rewardWeapon) =>
				RewardItem.fromObject(rewardWeapon, ITEM_KIND.WEAPON)
			)
		)
		.concat(
			settings.rewardArmors.map((rewardArmor) =>
				RewardItem.fromObject(rewardArmor, ITEM_KIND.ARMOR)
			)
		)
	/**
	 * @type {RewardMessageStatic[]}
	 */
	const rewardMessages = settings.rewardMessages.map((rewardMessage) =>
		RewardMessageStatic.fromObject(rewardMessage)
	)
