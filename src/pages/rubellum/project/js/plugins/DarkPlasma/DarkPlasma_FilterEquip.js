...
;(() => {
			...
			this._filters = this.actor()
				...
				.map((etypeId) => {
					const equips = $gameParty
						...
						.filter(
							(item) =>
								!DataManager.isItem(item) && this.actor().canEquip(item) && item.etypeId === etypeId
						)
	...
	class EquipFilterBuilder {
		 ...
		 */
		equipToTraits(equip) {
			return this._equipToTraitsRules
				.map((func) => func(equip))
				.reduce((result, traits) => result.concat(traits), [])
		}
		/**
			...
			if (effects_.length > 0) {
				return new EquipFilter_Trait(
					...
					effects_
						...
						.map(
							(effect) =>
								new EquipFilter_Effect(
									effect.dataId,
									this.traitToEffectName(traitId, effect.dataId)
								)
						)
				)
			}
		...
		isIncludedItem(equip) {
			...
			return this._traits.some(
				(trait) =>
					...
					(trait.effects.every((effect) => !effect.isIncluded()) ||
	...
	class EquipFilter_Trait {
		...
		hasIncludedEffectItem(itemTraits) {
			...
			const traits = itemTraits.filter(
				(trait) => trait.code === this._id && !hasNoEffectTrait(trait)
			)
			return this._effects.some(
				(effect) => effect.isIncluded() && traits.some((trait) => trait.dataId === effect.id)
			)
		}
	}
	...
	class Window_EquipFilter extends Window_Selectable {
		...
		processOk() {
			if (this.index() < this.filterNameList().length) {
				...
				if (
					this.isHandled('ok') &&
					this.isFilterOn(this.index()) &&
					this.needsOkHandler(this.index())
				) {
					this.callOkHandler()
				}
				this.refresh()
	...
	class Window_EquipFilterEffect extends Window_EquipFilter {
		...
		filterNameList() {
			return this._filter && this._filterTraitWindow
				? this._filter.effectNames(this._filterTraitWindow.index())
				: []
		}
		toggleFilter() {
			super.toggleFilter()
	...
	Window_CustomKeyHandlerMixIn(
		settings.key,
		Window_EquipFilterEffect.prototype,
		FILTER_HANDLER_NAME
	)
