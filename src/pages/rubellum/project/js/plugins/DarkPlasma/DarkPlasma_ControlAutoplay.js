...
;(() => {
	...
	function Game_Map_ControlAutoPlayMixIn(gameMap) {
		...
		gameMap.isAutoplayBgmEnabled = function () {
			return (
				!!$dataMap?.autoplayBgm &&
				(!settings.disableAutoplayBgmSwitch ||
					!$gameSwitches.value(settings.disableAutoplayBgmSwitch))
			)
		}
		gameMap.isAutoplayBgsEnabled = function () {
			return (
				!!$dataMap?.autoplayBgs &&
				(!settings.disableAutoplayBgsSwitch ||
					!$gameSwitches.value(settings.disableAutoplayBgsSwitch))
			)
		}
	}
