	...
	class Data_ActorCommandTrait {
		...
		static fromMeta(meta) {
			...
			const priority = Number(
				lines.find((line) => line.startsWith('priority:'))?.split(':')[1] || 0
			)
			const commands = lines
				...
				.map((line) => {
					...
					return Data_ActorCommand.fromSymbolAndDataId(
						symbol,
						splitted[1] ? Number(splitted[1]) : undefined
					)
				})
	...
	function Window_ActorCommand_ActorCommandTraitMixIn(windowClass) {
		...
		windowClass.makeCommandList = function () {
			if (this._actor) {
				...
				const trait =
					traits.length > 0 ? traits.reduce((a, b) => (a.priority > b.priority ? a : b)) : undefined
				if (trait) {
