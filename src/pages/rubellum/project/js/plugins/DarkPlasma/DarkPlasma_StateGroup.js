...
;(() => {
	...
	class StateGroup {
		...
		setPriorityOf(stateId, priority) {
			...
			if (!targetState) {
				throw Error(
					`グループ ${this.name} にステート ${$dataStates[stateId].name} が存在しません。`
				)
			}
	...
	function Game_Battler_StateGroupMixIn(gameBattler) {
		...
		gameBattler.addState = function (stateId) {
			...
			if ($dataStates[stateId].meta.OverwriteStateGroup) {
				const group = StateGroupManager.groupByName(
					String($dataStates[stateId].meta.OverwriteStateGroup)
				)
				if (group) {
					this.states()
		...
		gameBattler.isStateAddable = function (stateId) {
			...
			return (
				_isStateAddable.call(this, stateId) && !this.isHigherOrEqualPriorityStateAffected(stateId)
			)
		}
		/**
			...
			return statesOnBattleStart.filter((stateOnBattleStart) => {
				...
				if (groupA) {
					...
					return statesOnBattleStart
						...
						.map((s) => s.stateId)
						.every((stateB) => {
							const group = StateGroupManager.groupForComparePriority(
								stateOnBattleStart.stateId,
								stateB
							)
							return (
								!group ||
								group.higherPriorityStateIdIn(stateOnBattleStart.stateId, stateB) ===
									stateOnBattleStart.stateId
							)
						})
				}
