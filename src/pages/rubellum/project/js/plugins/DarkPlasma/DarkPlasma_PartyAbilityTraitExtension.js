...
;(() => {
	...
	Game_Actor.prototype.paramBasePlus = function (paramId) {
		return Math.max(
			0,
			_Game_Actor_paramBasePlus.call(this, paramId) + this.paramPlusByPartyAbility(paramId)
		)
	}
	...
	Game_Actor.prototype.partyAbilityTraitsSum = function (key) {
		return this.traitObjects().reduce(
			(result, object) => result + partyAbilityTraitAdd(object, key),
			0
		)
	}
	...
	Game_Actor.prototype.partyAbilityTraitsPi = function (key) {
		return this.traitObjects().reduce(
			(result, object) => result * partyAbilityTraitMulti(object, key),
			1
		)
	}
	...
	Game_Party.prototype.paramPlusByPartyAbility = function (paramId) {
		return this.allMembers().reduce(
			(result, actor) => result + actor.partyAbilityTraitsSum(paramKey(paramId)),
			0
		)
	}
	...
	Game_Party.prototype.sparamRateByPartyAbility = function (paramId) {
		return this.allMembers().reduce(
			(result, actor) => result * actor.partyAbilityTraitsPi(sparamKey(paramId)),
			1
		)
	}
		...
		if (this._tempActor !== tempActor && tempActor) {
			...
			party.allMembers = function () {
				return this._actors.map((id) =>
					id === tempActor.actorId() ? tempActor : $gameActors.actor(id)
				)
			}
		...
		Scene_Equip.prototype.equipFilterBuilder = function (equips) {
			...
			return builder
				.withEquipToTraitsRule((equip) => {
					const validSParamKeys = SPARAM_KEYS.filter(
						(key) => equip && partyAbilityTraitMulti(equip, key) !== 1
					)
