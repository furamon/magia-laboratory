...
;(() => {
	...
	function Window_EnemyBookIndex_InBattleMixIn(windowClass) {
		...
		windowClass.makeItemList = function () {
			...
			if (this._isInBattle && settings.battlerEnemyToTop) {
				this._list = this._list
					...
					.concat(
						this._list.filter((enemy) =>
							$gameTroop.members().every((gameEnemy) => gameEnemy.enemy() !== enemy)
						)
					)
			}
		}
		...
		windowClass.selectNextBattlerEnemy = function () {
			const nextIndex =
				this._battlerEnemyIndexes.find((index) => index > this.index()) ||
				this._battlerEnemyIndexes[0]
		...
		windowClass.selectPreviousBattlerEnemy = function () {
			...
			const prevIndex =
				candidates.length > 0 ? candidates.slice(-1)[0] : this._battlerEnemyIndexes.slice(-1)[0]
