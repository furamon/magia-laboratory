...
;(() => {
	...
	const settings = {
		...
		weakElementAndStateLabel: String(
			pluginParameters.weakElementAndStateLabel || `弱点属性/ステート/弱体`
		),
		resistElementAndStateLabel: String(
			pluginParameters.resistElementAndStateLabel || `耐性属性/ステート/弱体`
		),
		devideResistAndNoEffect: String(pluginParameters.devideResistAndNoEffect || false) === 'true',
		noEffectElementAndStateLabel: String(
			pluginParameters.noEffectElementAndStateLabel || `無効属性/ステート/弱体`
		),
	...
	class EnemyBook {
		...
		percentRegisteredEnemy() {
			...
			const registeredEnemyCount = this._pages.filter((page, enemyId) => {
				return (
					page &&
					$dataEnemies[enemyId] &&
					isRegisterableEnemy($dataEnemies[enemyId]) &&
					page.isRegistered
				)
			}).length
		...
		percentRegisteredDropItem() {
			const registerableDropItemCount = registerableEnemies().reduce(
				(previous, current) =>
					previous + current.dropItems.filter((dropItem) => dropItem.kind > 0).length,
				0
			)
	...
	class EnemyBookPage {
		...
		registeredDropItemCount(enemy) {
			return this._dropItems.filter(
				(dropItem, index) => dropItem && enemy.dropItems[index].kind > 0
			).length
		}
		register() {
	...
	class Window_EnemyBookPercent extends Window_LabelAndValueTexts {
		labelAndValueTexts() {
			return [
				new LabelAndValueText(
					settings.enemyPercentLabel,
					`${$gameSystem.percentCompleteEnemy().toFixed(1)}％`
				),
				new LabelAndValueText(
					settings.dropItemPercentLabel,
					`${$gameSystem.percentCompleteDrop().toFixed(1)}％`
				)
			]
		}
	}
	...
	class Window_EnemyBookStatus extends Window_Base {
		...
		descriptionX() {
			return settings.devideResistAndNoEffect
				? this.contentsWidth() / 2 + this.itemPadding() / 2
				: 0
		}
		descriptionY() {
		...
		elementRate(elementId) {
			return this._enemy.traits
				.filter(
					(trait) =>
						trait.code === Game_BattlerBase.TRAIT_ELEMENT_RATE && trait.dataId === elementId
				)
		...
		stateRate(stateId) {
			...
			return this._enemy.traits
				.filter(
					(trait) => trait.code === Game_BattlerBase.TRAIT_STATE_RATE && trait.dataId === stateId
				)
				.reduce((r, trait) => r * trait.value, 1)
		}
		/**
		...
		debuffRate(statusId) {
			return (
				this._enemy.traits
					.filter(
						(trait) =>
							trait.code === Game_BattlerBase.TRAIT_DEBUFF_RATE && trait.dataId === statusId
					)
			...
			const targetIcons = $dataSystem.elements
				...
				.concat(
					$dataStates
						.filter(
							(state) =>
								state && this.stateRate(state.id) > 1 && !this.isExcludedWeakState(state.id)
						)
			...
			const targetIcons = $dataSystem.elements
				...
				.concat(
					$dataStates
						.filter(
							(state) =>
								state && this.stateRate(state.id) <= 0 && !this.isExcludedResistState(state.id)
						)
						.map((state) => state.iconIndex)
				)
				.concat(
