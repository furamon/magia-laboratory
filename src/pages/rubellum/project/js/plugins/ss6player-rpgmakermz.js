...
;(function (loaders, display, meshExtras, ticker, filterColorMatrix, constants) {
	...
	class AnimationInitialData {
		...
		static getRootAsAnimationInitialData(bb, obj) {
			return (obj || new AnimationInitialData()).__init(
		...
		static createAnimationInitialData(
			builder,
			index,
			lowflag,
			highflag,
	...
	class userDataItem {
		...
		data(index, obj) {
			const offset = this.bb.__offset(this.bb_pos, 10)
			return offset
				? this.bb.__union(obj, this.bb.__vector(this.bb_pos + offset) + index * 4)
				: null
		}
		dataLength() {
			const offset = this.bb.__offset(this.bb_pos, 10)
	...
	class AnimationData {
		...
		defaultData(index, obj) {
			const offset = this.bb.__offset(this.bb_pos, 6)
			return offset
				? (obj || new AnimationInitialData()).__init(
						this.bb.__indirect(this.bb.__vector(this.bb_pos + offset) + index * 4),
						this.bb
					)
				: null
		...
		userData(index, obj) {
			const offset = this.bb.__offset(this.bb_pos, 10)
			return offset
				? (obj || new userDataPerFrame()).__init(
						this.bb.__indirect(this.bb.__vector(this.bb_pos + offset) + index * 4),
						this.bb
					)
				: null
		...
		meshsDataIndices(index, obj) {
			const offset = this.bb.__offset(this.bb_pos, 16)
			return offset
				? (obj || new meshDataIndices()).__init(
						this.bb.__indirect(this.bb.__vector(this.bb_pos + offset) + index * 4),
						this.bb
					)
				: null
	...
	class Cell {
		...
		cellMap(obj) {
			const offset = this.bb.__offset(this.bb_pos, 6)
			return offset
				? (obj || new CellMap()).__init(this.bb.__indirect(this.bb_pos + offset), this.bb)
				: null
		}
		indexInCellMap() {
			const offset = this.bb.__offset(this.bb_pos, 8)
		...
		LoadCellResources() {
			...
			for (let i = 0; i < self.fbObj.cellsLength(); i++) {
				if (
					...
					loader.add(
						self.fbObj.cells(i).cellMap().name(),
						self.rootPath + this.fbObj.cells(i).cellMap().imagePath()
					)
				}
			}
	...
	class SS6Player extends display.Container {
		...
		SetAnimationSpeed(fpsRate, _skipEnabled = true) {
			...
			this.updateInterval =
				1e3 / (this.playerLib.animationData.fps() * fpsRate * this.playDirection)
			this.skipEnabled = _skipEnabled
		}
		/**
		 * アニメーション再生設定
		...
		GetColorMatrixFilter(blendType, rate, argb32) {
			...
			if (blendType === 0) {
				...
				colorMatrix.matrix = [cr, 0, 0, 0, 0, 0, cg, 0, 0, 0, 0, 0, cb, 0, 0, 0, 0, 0, ca, 0]
			} else if (blendType === 2) {
				colorMatrix.matrix = [1, 0, 0, 0, cr, 0, 1, 0, 0, cg, 0, 0, 1, 0, cb, 0, 0, 0, ca, 0]
			} else if (blendType === 3) {
				colorMatrix.matrix = [1, 0, 0, 0, -cr, 0, 1, 0, 0, -cg, 0, 0, 1, 0, -cb, 0, 0, 0, ca, 0]
			}
			...
			for (let ii = 0; ii < l; ii = (ii + 1) | 0) {
				...
				switch (partType) {
					case SsPartType.Instance: {
						...
						this._instancePos = this.playerLib.TransformPositionLocal(
							this._instancePos,
							data.index,
							frameNumber
						)
					...
					case SsPartType.Mask: {
						...
						if (partType === SsPartType.Mesh) {
							if (data.meshIsBind === 0) {
								verts = this.playerLib.TransformMeshVertsLocal(
									Player.GetMeshVerts(cell, data, mesh.vertices),
									data.index,
									frameNumber
								)
							} else {
							...
							verts = this.playerLib.TransformVertsLocal(
								Player.GetVerts(cell, data, verts),
								data.index,
								frameNumber
							)
						}
						if (data.flag1 & PART_FLAG.VERTEX_TRANSFORM) {
							...
							const vec2 = Player.CoordinateGetDiagonalIntersection(
								verts[0],
								verts[1],
								CoordinateLURUx,
								CoordinateLURUy,
								...
								CoordinateLDRDx,
								CoordinateLDRDy,
								this._CoordinateGetDiagonalIntersectionVec2
							)
							verts[0] = vec2[0]
							verts[1] = vec2[1]
						}
						...
						mesh.vertices = verts
						if (
							data.flag1 & PART_FLAG.U_MOVE ||
							data.flag1 & PART_FLAG.V_MOVE ||
							data.flag1 & PART_FLAG.U_SCALE ||
							data.flag1 & PART_FLAG.V_SCALE ||
							data.flag1 & PART_FLAG.UV_ROTATION
						) {
						...
						if (data.useColorMatrix) {
							const colorMatrix = this.GetColorMatrixFilter(
								data.colorBlendType,
								data.colorRate,
								data.colorArgb32
							)
					...
					case SsPartType.Nulltype: {
						...
						const verts = this.playerLib.TransformVerts(
							Player.GetDummyVerts(),
							data.index,
							frameNumber
						)
						const px = verts[0]
		...
		MakeCellMesh(id) {
			...
			return new meshExtras.SimpleMesh(
				this.resources[cell.cellMap().name()].texture,
				verts,
				uvs,
				indices,
		...
		MakeMeshCellMesh(partID, cellID) {
			...
			if (uvLength > 0) {
				...
				return new meshExtras.SimpleMesh(
					this.resources[this.playerLib.fbObj.cells(cellID).cellMap().name()].texture,
					verts,
					uvs,
					indices,
					constants.DRAW_MODES.TRIANGLES
	...
	const _PluginParameters = class _PluginParameters {
		...
		constructor() {
			...
			this.replaceSVActorSpriteFlag =
				this.pluginParameters['replaceSVActorSpriteFlag'] === 'true' || false
			...
			this.svActorHideWeaponGraphics =
				this.pluginParameters['svActorHideWeaponGraphics'] === 'true' || false
			this.replaceSVEnemySpriteFlag =
				this.pluginParameters['replaceSVEnemySpriteFlag'] === 'true' || false
		...
		if (picture && picture.mzkpSS6Player) {
			if (picture.mzkpSS6Player instanceof SS6Player) {
				...
				if (player.loop !== -1) {
					...
					console.warn(
						'pictureId: ' + pictureId + ' can not wait SS6Player because setting infinity loop.'
					)
				}
			}
		} else {
		...
		if (PluginParameters.getInstance().replaceSVActorSpriteFlag) {
			if (changed) {
				...
				if (
					...
					Imported.VisuMZ_1_BattleCore &&
					this instanceof Sprite_SvEnemy
				) {
					actorId = this._actor.enemyId()
					ssfbId = 'sv_enemy_' + actorId
					ssfbPath =
						PluginParameters.getInstance().svActorDir +
						'svenemy' +
						String(actorId) +
						'/' +
						String(actorId) +
	...
	Sprite_Actor.prototype.updateBitmap = function () {
		if (PluginParameters.getInstance().replaceSVActorSpriteFlag) {
			...
			if (
				SS6ProjectManager.getInstance().isExist(ssfbId) &&
				this._actor._svActorSS6Player !== void 0
			) {
	...
	Sprite_Actor.prototype.updateSS6Player = function () {
		...
		if (
			this._actor._svActorSS6Player === null ||
			this._actor._svActorSS6Player.animeName !== motionName
		) {
	...
	Sprite_Enemy.prototype.updateBitmap = function () {
		if (PluginParameters.getInstance().replaceSVEnemySpriteFlag) {
			...
			if (
				SS6ProjectManager.getInstance().isExist(ssfbId) &&
				this._enemy._svEnemySS6Player !== void 0
			) {
				Sprite_Battler.prototype.updateBitmap.call(this)
				this.bitmap = null
				this.updateSS6Player()
