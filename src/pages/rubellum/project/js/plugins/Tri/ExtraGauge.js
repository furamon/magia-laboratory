...
;(() => {
	...
	Scene_Base.prototype.addChildExtraGauge = function (extraGauge) {
		...
		if (parentName && this._windowLayer) {
			const win = this._windowLayer.children.find(
				(window) => window instanceof Window && window.findWindowClassName() === parentName
			)
			if (win) {
		...
		currentMaxValue() {
			...
			if (!isFinite(value)) {
				PluginManagerEx.throwError(
					`Invalid current max value[${value}] id[${this._data.Id}]`,
					script
				)
			}
			return value
