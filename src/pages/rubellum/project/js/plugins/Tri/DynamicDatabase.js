...
;(() => {
	...
	DynamicDatabaseManager._setColumnMapperDynamic = function () {
		...
		this._times(this._getMaxLength('actions'), (i) => {
			this._columnMapperEnemies['actions_%1_conditionParam1'.format(i)] = '行動%1_条件1'.format(
				i + 1
			)
			this._columnMapperEnemies['actions_%1_conditionParam2'.format(i)] = '行動%1_条件2'.format(
				i + 1
			)
			this._columnMapperEnemies['actions_%1_conditionType'.format(i)] = '行動%1_条件タイプ'.format(
				i + 1
			)
