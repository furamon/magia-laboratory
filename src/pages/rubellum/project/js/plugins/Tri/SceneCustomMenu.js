...
;(() => {
	...
	class Scene_CustomMenu extends Scene_MenuBase {
		...
		stop() {
			super.stop()
			if (SceneManager.isNextScene(Scene_Battle) && !SceneManager.isPreviousScene(Scene_Battle)) {
				this.launchBattle()
			}
		}
	...
	class Window_CustomMenu extends Window_StatusBase {
		constructor(data, actor, windowMap) {
			super(
				new Rectangle(data.x, data.y, data.width || Graphics.boxWidth - data.x, data.height),
				data
			)
			this._actor = actor
	...
	class Window_CustomMenuCommand extends Window_CustomMenu {
		...
		isEnabledSub(item) {
			return (
				item && this.isScriptValid(item.IsEnableScript) && this.isSwitchValid(item.EnableSwitchId)
			)
		}
