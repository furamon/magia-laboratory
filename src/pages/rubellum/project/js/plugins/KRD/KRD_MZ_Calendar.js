...
;(() => {
	...
	PluginManager.registerCommand(PLUGIN_NAME, 'getFullYear', (args) => {
		$gameVariables.setValue(args.result, SceneManager._scene._calendarWindow.getFullYear())
	})

	PluginManager.registerCommand(PLUGIN_NAME, 'getMonth', (args) => {
		$gameVariables.setValue(args.result, SceneManager._scene._calendarWindow.getMonth())
	})

	PluginManager.registerCommand(PLUGIN_NAME, 'getDate', (args) => {
		$gameVariables.setValue(args.result, SceneManager._scene._calendarWindow.getDate())
	})
	...
	Window_Calendar = class extends Window_Table {
		...
		fittingHeight(numLines) {
			return !!LINE_HEIGHT
				? numLines * LINE_HEIGHT + $gameSystem.windowPadding() * 2
				: super.fittingHeight(...arguments)
		}
